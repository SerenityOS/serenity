...
if [ -z "$SERENITY_HOST_TELNET_PORT" ]; then 
    SERENITY_HOST_TELNET_PORT="8823"
fi

if [ -z "$SERENITY_HOST_WEBDRIVER_PORT" ]; then 
    SERENITY_HOST_WEBDRIVER_PORT="8000"
fi

if [ -z "$SERENITY_HOST_SSH_PORT" ]; then 
...
if [ "$SERENITY_ARCH" = "aarch64" ]; then
    ...
    SERENITY_NETFLAGS="-netdev user,id=breh,hostfwd=tcp:${SERENITY_HOST_IP}:${SERENITY_HOST_8888_PORT}-10.0.2.15:8888,hostfwd=tcp:${SERENITY_HOST_IP}:${SERENITY_HOST_TELNET_PORT}-10.0.2.15:23,hostfwd=tcp:${SERENITY_HOST_IP}:${SERENITY_HOST_WEBDRIVER_PORT}-10.0.2.15:8000,hostfwd=tcp:${SERENITY_HOST_IP}:${SERENITY_HOST_SSH_PORT}-10.0.2.15:22"
    SERENITY_NETFLAGS_WITH_DEFAULT_DEVICE="
    $SERENITY_NETFLAGS
