...
if (( ${#files[@]} )); then
    ...
    if command -v clang-format-19 >/dev/null 2>&1 ; then
        ...
        if ! "${CLANG_FORMAT}" --version | awk '{ if (substr($NF, 1, index($NF, ".") - 1) < 19) exit 1; }'; then
            echo "You are using '$("${CLANG_FORMAT}" --version)', which appears to not be clang-format 19 or later."
        ...
        echo "(If you install a package 'clang-format', please make sure it's version 19 or later.)"
        exit 1
    fi
