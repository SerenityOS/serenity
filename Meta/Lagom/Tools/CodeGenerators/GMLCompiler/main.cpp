...
static Optional<StringView> map_class_to_file(StringView clazz)
{
    ...
    return class_file_mappings.get(clazz);
}
...
static Optional<StringView> map_property_to_type(StringView property)
{
    ...
    if (property_to_type_mappings.is_empty()) {
        property_to_type_mappings.set("container_margins"sv, "GUI::Margins"sv);
        property_to_type_mappings.set("margins"sv, "GUI::Margins"sv);
    }
...
{
    String pathed_name = TRY(TRY(String::from_utf8(class_name)).replace("::"sv, "/"sv, ReplaceMode::All));
...
{
    RefPtr<@main_class_name@> main_class;

)~~~";

static char const footer[] = R"~~~(
    return main_class.release_nonnull();
}
)~~~";
...
static ErrorOr<String> escape_string(JsonValue to_escape)
{
    ...
    // Other commonly-escaped characters are hopefully Unicode and therefore fine to include verbatim in UTF-8 coded strings.
...
static ErrorOr<String> generate_initializer_for(Optional<StringView> property_name, JsonValue value)
{
    if (value.is_string()) {
        ...
        return String::formatted(R"~~~(TRY("{}"_string))~~~", TRY(escape_string(value)));
    }
...
static ErrorOr<void> generate_loader_for_object(GUI::GML::Object const& gml_object, SourceGenerator generator, String object_name, size_t indentation, UseObjectConstructor use_object_constructor)
{
    ...
    if (use_object_constructor == UseObjectConstructor::Yes)
        ...
        TRY(append(generator, "@object_name@ = TRY(try_make_ref_counted<@class_name@>());"));
...
static ErrorOr<String> generate_cpp(NonnullRefPtr<GUI::GML::GMLFile> gml)
{
    ...
    TRY(generate_loader_for_object(main_class, generator.fork(), TRY(String::from_utf8("main_class"sv)), 2, UseObjectConstructor::No));
