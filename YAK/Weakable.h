...
public:
...
#endif
            if (!(m_consumers.fetch_add(1u << 1, YAK::MemoryOrder::memory_order_acquire) & 1u)) {
                T* ptr = (T*)m_ptr.load(YAK::MemoryOrder::memory_order_acquire);
                if (ptr && ptr->try_ref())
                    ref = adopt_ref(*ptr);
            }
            m_consumers.fetch_sub(1u << 1, YAK::MemoryOrder::memory_order_release);
        }
    ...
    T* unsafe_ptr() const
    {
        if (m_consumers.load(YAK::MemoryOrder::memory_order_relaxed) & 1u)
            return nullptr;
        ...
        // reference...
        return (T*)m_ptr.load(YAK::MemoryOrder::memory_order_acquire);
    }
    ...
    void revoke()
    {
        auto current_consumers = m_consumers.fetch_or(1u, YAK::MemoryOrder::memory_order_relaxed);
        ...
        while (current_consumers > 0) {
            ...
            // TODO: yield?
#endif
            current_consumers = m_consumers.load(YAK::MemoryOrder::memory_order_acquire) & ~1u;
        }
        // No one is trying to use it (anymore)
        m_ptr.store(nullptr, YAK::MemoryOrder::memory_order_release);
    }
...
protected:
    ...
    ~Weakable()
    {
        m_being_destroyed.store(true, YAK::MemoryOrder::memory_order_release);
        revoke_weak_ptrs();
    }
