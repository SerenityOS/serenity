...
public:
    ...
    ALWAYS_INLINE ~NonnullRefPtr()
    {
        assign(nullptr);
#ifdef SANITIZE_PTRS
        m_bits.store(explode_byte(0xb0), YAK::MemoryOrder::memory_order_relaxed);
#endif
    }
...
private:
    ...
    {
        return (T*)(m_bits.load(YAK::MemoryOrder::memory_order_relaxed) & ~(FlatPtr)1);
    }

    ALWAYS_INLINE RETURNS_NONNULL T* as_nonnull_ptr() const
    {
        T* ptr = (T*)(m_bits.load(YAK::MemoryOrder::memory_order_relaxed) & ~(FlatPtr)1);
        VERIFY(ptr);
        return ptr;
    }
    ...
    void do_while_locked(F f) const
    {
        ...
        FlatPtr bits;
        for (;;) {
            bits = m_bits.fetch_or(1, YAK::MemoryOrder::memory_order_acq_rel);
            if (!(bits & 1))
                break;
#ifdef KERNEL
        ...
        VERIFY(!(bits & 1));
        f((T*)bits);
        m_bits.store(bits, YAK::MemoryOrder::memory_order_release);
    }
    ...
    ALWAYS_INLINE T* exchange(T* new_ptr)
    {
        ...
        FlatPtr expected = m_bits.load(YAK::MemoryOrder::memory_order_relaxed);
        for (;;) {
            expected &= ~(FlatPtr)1; // only if lock bit is not set
            if (m_bits.compare_exchange_strong(expected, (FlatPtr)new_ptr, YAK::MemoryOrder::memory_order_acq_rel))
                break;
#ifdef KERNEL
            Kernel::Processor::wait_check();
#endif
    ...
    T* add_ref() const
    {
        ...
        for (;;) {
            expected &= ~(FlatPtr)1; // only if lock bit is not set
            if (m_bits.compare_exchange_strong(expected, expected | 1, YAK::MemoryOrder::memory_order_acq_rel))
                break;
#ifdef KERNEL
            Kernel::Processor::wait_check();
#endif
        ...
        m_bits.store(expected, YAK::MemoryOrder::memory_order_release);
        return (T*)expected;
    }
