...
private:
    ...
    consteval static bool check_format_parameter_consistency(const char (&fmt)[N])
    {
...

        {
            ...
            auto has_all_referenced_arguments = !YAK::any_of(begin, end, [](auto& entry) { return entry >= param_count; });
        ...
        if (check.has_explicit_argument_references) {
            ...
            auto references_all_arguments = YAK::all_of(
                all_parameters,
                [&](auto& entry) {
                    return contains(
