...
struct RefPtrTraits {
    ...
    ALWAYS_INLINE static FlatPtr exchange(Atomic<FlatPtr>& atomic_var, FlatPtr new_value)
    {
        ...
        VERIFY(!(new_value & 1));
        FlatPtr expected = atomic_var.load(YAK::MemoryOrder::memory_order_relaxed);
        for (;;) {
            expected &= ~(FlatPtr)1; // only if lock bit is not set
            if (atomic_var.compare_exchange_strong(expected, new_value, YAK::MemoryOrder::memory_order_acq_rel))
                break;
#ifdef KERNEL
    ...
    ALWAYS_INLINE static FlatPtr lock(Atomic<FlatPtr>& atomic_var)
    {
        ...
        FlatPtr bits;
        for (;;) {
            bits = atomic_var.fetch_or(1, YAK::MemoryOrder::memory_order_acq_rel);
            if (!(bits & 1))
                break;
#ifdef KERNEL
            Kernel::Processor::wait_check();
    ...
    {
        VERIFY(!(new_value & 1));
        atomic_var.store(new_value, YAK::MemoryOrder::memory_order_release);
    }
...
public:
    ...
    ALWAYS_INLINE ~RefPtr()
    {
        clear();
#ifdef SANITIZE_PTRS
        m_bits.store(explode_byte(0xe0), YAK::MemoryOrder::memory_order_relaxed);
#endif
    }
    ...
    bool operator!() const { return PtrTraits::is_null(m_bits.load(YAK::MemoryOrder::memory_order_relaxed)); }
    ...
    ALWAYS_INLINE bool is_null() const { return PtrTraits::is_null(m_bits.load(YAK::MemoryOrder::memory_order_relaxed)); }
    ...
    typename PtrTraits::NullType null_value() const
    {
        ...
        FlatPtr bits = m_bits.load(YAK::MemoryOrder::memory_order_relaxed);
        VERIFY(PtrTraits::is_null(bits));
...
private:
    ...
    {
        return PtrTraits::as_ptr(m_bits.load(YAK::MemoryOrder::memory_order_relaxed));
    }

    ALWAYS_INLINE T* as_nonnull_ptr() const
    {
        return as_nonnull_ptr(m_bits.load(YAK::MemoryOrder::memory_order_relaxed));
    }
...
using YAK::adopt_ref_if_nonnull;
using YAK::RefPtr;
using YAK::static_ptr_cast;
using YAK::try_make_ref_counted;

#ifdef KERNEL
using YAK::adopt_nonnull_ref_or_enomem;
#endif
