...
class RefCountedBase {
    YAK_MAKE_NONCOPYABLE(RefCountedBase);
    YAK_MAKE_NONMOVABLE(RefCountedBase);
...
public:
    ...
    void ref() const
    {
        auto old_ref_count = m_ref_count.fetch_add(1, YAK::MemoryOrder::memory_order_relaxed);
    ...
    {
        RefCountType expected = m_ref_count.load(YAK::MemoryOrder::memory_order_relaxed);
        for (;;) {
            if (expected == 0)
                return false;
            VERIFY(!Checked<RefCountType>::addition_would_overflow(expected, 1));
            if (m_ref_count.compare_exchange_strong(expected, expected + 1, YAK::MemoryOrder::memory_order_acquire))
                return true;
        }
    }
    ...
    {
        return m_ref_count.load(YAK::MemoryOrder::memory_order_relaxed);
    }
...
protected:
    ...
    ~RefCountedBase()
    {
        VERIFY(m_ref_count.load(YAK::MemoryOrder::memory_order_relaxed) == 0);
    }

    RefCountType deref_base() const
    {
        auto old_ref_count = m_ref_count.fetch_sub(1, YAK::MemoryOrder::memory_order_acq_rel);
        VERIFY(old_ref_count > 0);
...
using YAK::RefCounted;
using YAK::RefCountedBase;
