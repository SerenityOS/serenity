...
class Singleton {
    YAK_MAKE_NONCOPYABLE(Singleton);
    YAK_MAKE_NONMOVABLE(Singleton);
...
public:
    ...
    {
        T* obj = obj_var.load(YAK::memory_order_acquire);
        if (FlatPtr(obj) <= 0x1) {
            ...
            if constexpr (allow_create) {
                if (obj == nullptr && obj_var.compare_exchange_strong(obj, (T*)0x1, YAK::memory_order_acq_rel)) {
                    // We're the first one
                    obj = InitFunction();
                    obj_var.store(obj, YAK::memory_order_release);
                    return obj;
                }
            }
            ...
            while (obj == (T*)0x1) {
                ...
                // TODO: yield
#endif
                obj = obj_var.load(YAK::memory_order_acquire);
            }
            if constexpr (allow_create) {
    ...
    {
        T* obj = m_obj.load(YAK::MemoryOrder::memory_order_consume);
