...
struct VisitImpl {
    ...
    ALWAYS_INLINE static constexpr decltype(auto) visit(IndexType id, void const* data, Visitor&& visitor) requires(CurrentIndex < sizeof...(Ts))
    {
...
public:
    ...
    template<typename T>
    T const* get_pointer() const requires(can_contain<T>())
    {
        if (index_of<T>() == m_index)
    ...
    template<typename T>
    T const& get() const requires(can_contain<T>())
    {
        VERIFY(has<T>());
        return *bit_cast<T const*>(&m_data);
    }
