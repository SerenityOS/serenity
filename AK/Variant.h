...
union VariantStorage<CurrentIndex, F, Ts...> {
    ...
    // Note: STL impls seem to wrap this into its own struct,
    //       which just forwards the constructor, not sure why though.
    F value;
    Rest rest;
};
...
struct AllOverloads<IndexSequence<Indices...>, Ts...> : public Overload<Indices, Ts>... {
    // If you are here because of a long list of redeclarations of this function
...
public:
    ...
    constexpr Variant(T&& t)
    {
        ...
        // FIXME: Is this replacement over the trivial empty union free,
        //        and should we try to construct it directly instead?
