...
// `move` and `forward` are visible from the contruct_at headers, so the similar applies to them.
// They are also often special cased in compilers and static analyzers,
// which may make it faster to use a `std::` sourced version
// So for non-Kernel code we will use the std versions of these functions
