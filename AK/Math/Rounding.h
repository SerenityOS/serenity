...
constexpr T round(T x)
{
    ...
    // FIXME: Is there a nice way of getting these values?
    //        The "correct" way is `nextafter(0.5,0)`,
    //        but while clang seems to fold that it does not treat it as constexpr
    //        GCC seems fine with it (tested on v24 and v15 respectively)
    //        (1. - EPSILON/2)/2 seems to also work,
...
constexpr I round_to(P value)
{
    ...
    // FIXME: As we use a static_cast we dont get the
    //        "If x is a NaN or an infinity [...] the return value is unspecified"
    //        for free, so we currently just invoke UB,
    //        If we would use the correct instruction
    //        On x86 we get a ~0 (or a 0x8000'0000 when separating rint and ltrunc as below),
    //        Arm  uses a saturation, and generate a 0 for Nan,
    //        RiscV uses a saturation, and treats NaN the same as +infty
