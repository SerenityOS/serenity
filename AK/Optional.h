...
requires(!IsLvalueReference<T>) class [[nodiscard]] Optional<T> {
    ...
    //        Note this could be an AK::Array, but can't as that depends on Optional
    //        it uses a anonymous struct with a dummy member instead
...
public:
    ...
    {
        if (other.has_value())
            construct_at<T>(&m_storage, other.value());
        else
            construct_null();
    }
    ...
    {
        if (other.has_value())
            construct_at<T>(&m_storage, other.release_value());
        else
            construct_null();
    }
    ...
    requires(!IsSame<RemoveCVReference<U>, Optional<T>> && IsConstructible<T, U &&>)
        : m_has_value(true)
    {
        construct_at<T>(&m_storage, forward<U>(value));
    }
    ...
    constexpr ALWAYS_INLINE void emplace(Parameters&&... parameters)
    {
        clear();
        m_has_value = true;
        construct_at<T>(&m_storage, forward<Parameters>(parameters)...);
    }
    ...
    constexpr ALWAYS_INLINE void lazy_emplace(Callable callable)
    {
        clear();
        m_has_value = true;
        construct_at<T>(&m_storage, callable());
    }
