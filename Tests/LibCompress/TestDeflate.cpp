...
{
    auto original = MUST(ByteBuffer::create_uninitialized(1024));
...
{
    auto original = MUST(ByteBuffer::create_zeroed(2048));
...
TEST_CASE(deflate_round_trip_compress_large)
{
    ...
    auto original = MUST(ByteBuffer::create_uninitialized(size)); // Compress a buffer larger than the maximum block size to test the sliding window mechanism
