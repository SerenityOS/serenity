...
static PDF::PDFErrorOr<NonnullRefPtr<PDF::Function>> make_function(int type, ReadonlyBytes data, Vector<float> domain, Vector<float> range, Function<void(HashMap<FlyByteString, PDF::Value>&)> extra_keys = nullptr)
{
    HashMap<FlyByteString, PDF::Value> map;
