...
TEST_CASE(little_endian_bit_stream_input_output_match)
{
    ...
    LittleEndianOutputBitStream bit_write_stream { MaybeOwned<Core::Stream>(*memory_stream) };
    LittleEndianInputBitStream bit_read_stream { MaybeOwned<Core::Stream>(*memory_stream) };
...
TEST_CASE(big_endian_bit_stream_input_output_match)
{
    ...
    // Note: The bit stream only ever reads from/writes to the underlying stream in one byte chunks,
    // so testing with sizes that will not trigger a write will yield unexpected results.
    BigEndianOutputBitStream bit_write_stream { MaybeOwned<Core::Stream>(*memory_stream) };
    BigEndianInputBitStream bit_read_stream { MaybeOwned<Core::Stream>(*memory_stream) };
