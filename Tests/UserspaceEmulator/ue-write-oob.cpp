...
int main(int argc, char** argv)
{
    ...
    args_parser.add_option(do_static, "Use a static region instead of an mmap'ed region. Fixes 'size' to 10*PAGESIZE = 40960. (Default: false)", "static", 'S').release_value_but_fixme_should_propagate_errors();
    args_parser.add_option(size, "The size of the region to allocate. (Default: 10*PAGESIZE = 40960)", "size", 's', "size").release_value_but_fixme_should_propagate_errors();
    args_parser.add_option(offset, "The signed offset at which to start writing. (Default: 10*PAGESIZE-1 = 40959)", "offset", 'o', "offset").release_value_but_fixme_should_propagate_errors();
    args_parser.add_option(bits, "Amount of bits to write in a single instruction. (Default: 16)", "bits", 'b', "bits").release_value_but_fixme_should_propagate_errors();
