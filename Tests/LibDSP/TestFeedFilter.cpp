...
TEST_CASE(oscillating)
{
    // Filter that oscillates with constant input.
    Filter<2, 2> oscillation_filter;
    oscillation_filter.set_feedforward_coefficients({ 0, 1 });
    oscillation_filter.set_feedback_coefficients({ 1, 1 });
    Array<int, chunk_size> input { 1, 1, 1, 1, 1 };
    Array<int, chunk_size> output {};
    auto output_span = output.span();
    oscillation_filter.filter(input.span(), output_span);

    for (size_t i = 0; i < chunk_size; ++i)
        EXPECT_EQ(output[i], static_cast<int>(i % 2));

    oscillation_filter.filter(input.span(), output_span);
    for (size_t i = 0; i < chunk_size; ++i)
        EXPECT_EQ(output[i], static_cast<int>((i + chunk_size) % 2));
}
