...
{
    auto mask = Core::StreamPermissionsMask::from_symbolic_notation(""sv);
    ...
    mask = Core::StreamPermissionsMask::from_symbolic_notation("u+rwx"sv);
    EXPECT(!mask.is_error());
    EXPECT_EQ(mask.value().clear_mask(), 0);
    ...
    mask = Core::StreamPermissionsMask::from_symbolic_notation("g+rwx"sv);
    EXPECT(!mask.is_error());
    EXPECT_EQ(mask.value().clear_mask(), 0);
    ...
    mask = Core::StreamPermissionsMask::from_symbolic_notation("o+rwx"sv);
    EXPECT(!mask.is_error());
    EXPECT_EQ(mask.value().clear_mask(), 0);
    ...
    mask = Core::StreamPermissionsMask::from_symbolic_notation("a=rx"sv);
    EXPECT(!mask.is_error());
    ...
    mask = Core::StreamPermissionsMask::from_symbolic_notation("ugo=rx"sv);
    EXPECT(!mask.is_error());
    EXPECT_EQ(mask.value().clear_mask(), 0777);
    EXPECT_EQ(mask.value().write_mask(), 0555);
    EXPECT_EQ(mask.value().apply(0), 0555);
    ...
    mask = Core::StreamPermissionsMask::from_symbolic_notation("u+rw,g=rx,o-rwx"sv);
    EXPECT(!mask.is_error());
    ...
    mask = Core::StreamPermissionsMask::from_symbolic_notation("+r"sv);
    EXPECT(!mask.is_error());
    EXPECT_EQ(mask.value().clear_mask(), 0);
    ...
    mask = Core::StreamPermissionsMask::from_symbolic_notation("=rx"sv);
    EXPECT(!mask.is_error());
    EXPECT_EQ(mask.value().clear_mask(), 0777);
    EXPECT_EQ(mask.value().write_mask(), 0555);
    EXPECT_EQ(mask.value().apply(0), 0555);
    ...
    mask = Core::StreamPermissionsMask::from_symbolic_notation("a+X"sv);
    EXPECT(!mask.is_error());
    EXPECT_EQ(mask.value().clear_mask(), 0);
    EXPECT_EQ(mask.value().write_mask(), 0);
    ...
    mask = Core::StreamPermissionsMask::from_symbolic_notation("z+rw"sv);
    EXPECT(mask.is_error());
    EXPECT(mask.error().string_literal().starts_with("invalid class"sv));

    mask = Core::StreamPermissionsMask::from_symbolic_notation("u*rw"sv);
    EXPECT(mask.is_error());
    EXPECT(mask.error().string_literal().starts_with("invalid operation"sv));

    mask = Core::StreamPermissionsMask::from_symbolic_notation("u+rz"sv);
    EXPECT(mask.is_error());
    EXPECT(mask.error().string_literal().starts_with("invalid symbolic permission"sv));

    mask = Core::StreamPermissionsMask::from_symbolic_notation("u+rw;g+rw"sv);
    EXPECT(mask.is_error());
    EXPECT(mask.error().string_literal().starts_with("invalid symbolic permission"sv));
}
...
{
    auto numeric_mask = Core::StreamPermissionsMask::parse("750"sv);
    auto symbolic_mask = Core::StreamPermissionsMask::parse("u=rwx,g=rx,o-rwx"sv);
    ...
    auto mask = Core::StreamPermissionsMask::parse("888"sv);
    EXPECT(mask.is_error());

    mask = Core::StreamPermissionsMask::parse("z+rw"sv);
    EXPECT(mask.is_error());
}
...
{
    {
        auto mask = Core::StreamPermissionsMask::parse("750"sv);
        EXPECT(!mask.is_error());
        EXPECT_EQ(mask.value().apply(07000), 07750);
    }

    {
        auto mask = Core::StreamPermissionsMask::parse("7750"sv);
        EXPECT(!mask.is_error());
        EXPECT_EQ(mask.value().apply(0), 07750);
    }

    {
        auto mask = Core::StreamPermissionsMask::parse("0750"sv);
        EXPECT(!mask.is_error());
