...
{
    EXPECT(test_single<double>({ LITERAL("xxxxxx"), "|%f|", NAN, 5, LITERAL("|nan|\0") }));
    EXPECT(test_single<double>({ LITERAL("xxxxxxx"), "|%+f|", NAN, 6, LITERAL("|+nan|\0") }));

    EXPECT(test_single<double>({ LITERAL("xxxxxxx"), "|%f|", -INFINITY, 6, LITERAL("|-inf|\0") }));
    EXPECT(test_single<double>({ LITERAL("xxxxxxx"), "|%+f|", -INFINITY, 6, LITERAL("|-inf|\0") }));

    EXPECT(test_single<double>({ LITERAL("xxxxxxxxxxx"), "|%08f|", NAN, 10, LITERAL("|     nan|\0") }));
    EXPECT(test_single<double>({ LITERAL("xxxxxxxxxxx"), "|%+08f|", NAN, 10, LITERAL("|    +nan|\0") }));
    EXPECT(test_single<double>({ LITERAL("xxxxxxxxxxx"), "|%08f|", -INFINITY, 10, LITERAL("|    -inf|\0") }));
    EXPECT(test_single<double>({ LITERAL("xxxxxxxxxxx"), "|%+08f|", -INFINITY, 10, LITERAL("|    -inf|\0") }));

    EXPECT(test_single<double>({ LITERAL("xxxxxxxxxxx"), "|%8f|", NAN, 10, LITERAL("|     nan|\0") }));
    EXPECT(test_single<double>({ LITERAL("xxxxxxxxxxx"), "|%+8f|", NAN, 10, LITERAL("|    +nan|\0") }));
    EXPECT(test_single<double>({ LITERAL("xxxxxxxxxxx"), "|%8f|", -INFINITY, 10, LITERAL("|    -inf|\0") }));
