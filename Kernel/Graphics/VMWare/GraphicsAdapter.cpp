...
UNMAP_AFTER_INIT RefPtr<VMWareGraphicsAdapter> VMWareGraphicsAdapter::try_initialize(PCI::DeviceIdentifier const& pci_device_identifier)
{
    ...
    if (id.device_id != 0x0405)
        return {};
    // auto fifo_registers_region = MUST(MM.allocate_kernel_region(PhysicalAddress(PCI::get_BAR2(device_identifier.address())).page_base(), PAGE_SIZE, "VMWare FIFO Display", Memory::Region::Access::ReadWrite));
...
{
    write_io_register(VMWareDisplayRegistersOffset::ID, SVGA_ID_2);
    ...
    if (read_io_register(VMWareDisplayRegistersOffset::ID) == SVGA_ID_2)
        return {};
