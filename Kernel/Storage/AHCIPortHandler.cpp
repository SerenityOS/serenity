...
NonnullRefPtr<AHCIPortHandler> AHCIPortHandler::create(AHCIController& controller, u8 irq, AHCI::MaskedBitField taken_ports, NonnullRefPtr<AHCIPort> (*ahci_port_creator)(const AHCIPortHandler&, volatile AHCI::PortRegisters&, u32 port_index))
{
    return adopt_ref(*new AHCIPortHandler(controller, irq, taken_ports, ahci_port_creator));
}

AHCIPortHandler::AHCIPortHandler(AHCIController& controller, u8 irq, AHCI::MaskedBitField taken_ports, NonnullRefPtr<AHCIPort> (*ahci_port_creator)(const AHCIPortHandler&, volatile AHCI::PortRegisters&, u32 port_index))
    : IRQHandler(irq)
    ...
    , m_pending_ports_interrupts(create_pending_ports_interrupts_bitfield())
{
    ...
    if (kernel_command_line().ahci_reset_mode() == AHCIResetMode::Complete) {
        for (auto index : taken_ports.to_vector()) {
            auto port = ahci_port_creator(*this, static_cast<volatile AHCI::PortRegisters&>(controller.hba().port_regs[index]), index);
