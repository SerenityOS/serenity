...
RAMFSInode::RAMFSInode(RAMFS& fs, Optional<u64> max_inode_size, InodeMetadata const& metadata, LockWeakPtr<RAMFSInode> parent)
...
ErrorOr<NonnullRefPtr<RAMFSInode>> RAMFSInode::try_create(RAMFS& fs, Optional<u64> max_inode_size, InodeMetadata const& metadata, LockWeakPtr<RAMFSInode> parent)
{
    return adopt_nonnull_ref_or_enomem(new (nothrow) RAMFSInode(fs, max_inode_size, metadata, move(parent)));
}
...
ErrorOr<void> RAMFSInode::ensure_safely_allocated_blocks(size_t last_block_index)
{
    ...
    TRY(fs().current_storage_usage_size({}).with_exclusive([&](auto& current_storage_usage_size) -> ErrorOr<void> {
        ...
        if (storage_diff > 0 && fs().max_size().has_value()) {
            VERIFY((fs().max_size().value() % PAGE_SIZE) == 0);
            ...
            VERIFY(current_storage_usage_size >= static_cast<u64>(AK::abs(storage_diff)));
            storage_diff = (m_blocks.size() * DataBlock::block_size) - (last_block_index * DataBlock::block_size);
            current_storage_usage_size -= static_cast<u64>(AK::abs(storage_diff));
        }
...
ErrorOr<void> RAMFSInode::ensure_allocated_blocks(size_t offset, size_t io_size)
{
    ...
    if (m_max_inode_size.has_value()) {
        VERIFY((m_max_inode_size % PAGE_SIZE) == 0);
...
ErrorOr<NonnullRefPtr<Inode>> RAMFSInode::create_child(StringView name, mode_t mode, dev_t dev, UserID uid, GroupID gid)
{
    ...
    auto child = TRY(RAMFSInode::try_create(fs(), fs().max_inode_size(), metadata, *this));
