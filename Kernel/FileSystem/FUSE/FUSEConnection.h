...
public:
    static ErrorOr<NonnullRefPtr<FUSEConnection>> try_create(NonnullRefPtr<OpenFileDescription> description)
    {
        ...
        return TRY(adopt_nonnull_ref_or_enomem(new (nothrow) FUSEConnection(description)));
    }

    static ErrorOr<NonnullOwnPtr<KBuffer>> create_request(int opcode, int nodeid, u32 unique, ReadonlyBytes request_body)
    {
    ...
    ErrorOr<NonnullOwnPtr<KBuffer>> send_request_and_wait_for_a_reply(int opcode, int nodeid, ReadonlyBytes request_body)
    {
        ...
        if (validate_response(response.ptr(), m_unique++).is_error())
...
private:
    ...
    ErrorOr<void> validate_response(KBuffer* response, u32 unique)
    {
    ...
    ErrorOr<void> handle_init()
    {
        ...
        auto request = TRY(create_request(FUSE_INIT, 0, 0, { &init_request, sizeof(init_request) }));
        auto response = TRY(device->send_request_and_wait_for_a_reply(m_description, request->bytes()));

        if (validate_response(response.ptr(), 0).is_error())
            return Error::from_errno(EIO);
