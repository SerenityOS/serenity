...
static u64 reinterpret_as_octal(u64 decimal)
{
    u64 result = 0;
    u64 n = 0;
    while (decimal > 0) {
        result += AK::pow<u64>(8, n++) * (decimal % 10);
        decimal /= 10;
    }
    return result;
}

ErrorOr<NonnullRefPtr<FileSystem>> FUSE::try_create(ProcessID pid, ReadonlyBytes mount_flags)
{
    ...
    auto description = TRY(process->open_file_description(fuse_mount_flags->device_inode));
    auto connection = TRY(FUSEConnection::try_create(description));

    u64 rootmode = reinterpret_as_octal(fuse_mount_flags->rootmode);
