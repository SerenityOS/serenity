...
ErrorOr<void> Ext2FSInode::remove_child_impl(StringView name, bool remove_misc)
{
    MutexLocker locker(m_inode_lock);
    VERIFY(is_directory());
    ...
    if (child_inode->is_directory() && remove_misc) {
        TRY(static_cast<Ext2FSInode&>(*child_inode).remove_child_impl("."sv, false));
        TRY(static_cast<Ext2FSInode&>(*child_inode).remove_child_impl(".."sv, false));
    }
