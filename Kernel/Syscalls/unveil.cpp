...
ErrorOr<FlatPtr> Process::sys$unveil(Userspace<Syscall::SC_unveil_params const*> user_params)
{
    ...
    auto custody_or_error = VirtualFileSystem::the().resolve_path_without_veil(vfs_root_context(), credentials(), path->view(), Process::current().vfs_root_context_custody(), &parent_custody);
