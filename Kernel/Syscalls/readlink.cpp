...
ErrorOr<FlatPtr> Process::sys$readlink(Userspace<Syscall::SC_readlink_params const*> user_params)
{
    ...
    auto description = TRY(VirtualFileSystem::open(vfs_root_context(), credentials(), unresolved_path, O_RDONLY | O_NOFOLLOW_NOERROR, 0));
    ...
    auto link_target = TRY(description->inode()->read_as_link());
