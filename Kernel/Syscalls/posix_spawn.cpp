...
ErrorOr<FlatPtr> Process::sys$posix_spawn(Userspace<Syscall::SC_posix_spawn_params const*> user_params)
{
    ...
    auto child_and_first_thread = TRY(Process::create(name.representable_view(), credentials->uid(), credentials->gid(), pid(), m_is_kernel_process, vfs_root_context(), hostname_context(), current_directory(), nullptr, tty(), nullptr));
    ...
    TRY(child->exec(move(path), move(arguments), move(environment), reinterpret_cast<Thread*&>(child_first_thread), previous_interrupts_state));
