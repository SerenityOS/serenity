            ...
            .on_node_end = [&global_data, &state](StringView node_name) -> ErrorOr<IterationDecision> {
                ...
                case State::InMemory:
                    for (auto const& reg_entry : state.reg) {
                        ...
                        global_data.physical_memory_ranges.append(PhysicalMemoryRange { PhysicalMemoryRangeType::Usable, PhysicalAddress { reg_entry.start_addr }, reg_entry.size });
                    }
                ...
                case State::InReservedMemoryChild:
                    ...
                    for (auto const& reg_entry : state.reg) {
                        ...
                        global_data.physical_memory_ranges.append(PhysicalMemoryRange { PhysicalMemoryRangeType::Reserved, PhysicalAddress { reg_entry.start_addr }, reg_entry.size });
                        // FIXME: Not all of these are "used", only those in "memory" are actually "used"
                        //        There might be for example debug DMA control registers, which are marked as reserved
                        global_data.used_memory_ranges.append(UsedMemoryRange { UsedMemoryRangeType::BootModule, PhysicalAddress { reg_entry.start_addr }, PhysicalAddress { reg_entry.start_addr + reg_entry.size } });
                    }
