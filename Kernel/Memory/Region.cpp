...
bool Region::should_dirty_on_write(size_t page_index, ShouldLockVMObject should_lock_vmobject) const
{
    ...
    if (should_lock_vmobject == ShouldLockVMObject::Yes) {
        ...
        return should_dirty_on_write_impl(page_index);
    } else {
        return should_dirty_on_write_impl(page_index);
    }
}
...
ErrorOr<void> Region::map(PageDirectory& page_directory, ShouldLockVMObject should_lock_vmobject, ShouldFlushTLB should_flush_tlb)
{
...
void Region::remap_impl(ShouldLockVMObject should_lock_vmobject)
{
    ...
    if (m_vmobject->is_mmio())
        ...
        result = map(*m_page_directory, should_lock_vmobject);
    if (result.is_error())
        TODO();
}
