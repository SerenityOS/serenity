...
public:
    ...
    {
        process.profiling_data().with([&](auto& data) {
            if (auto* event_buffer = data.current_perf_events_buffer()) {
    ...
    static void add_thread_created_event(Thread& thread)
    {
        if (thread.is_profiling_suppressed())
            return;
        thread.process().profiling_data().with([&](auto& data) {
            if (auto* event_buffer = data.current_perf_events_buffer()) {
    ...
    static void add_cpu_sample_event(Thread& current_thread, RegisterState const& regs, u32 lost_time)
    {
        if (current_thread.is_profiling_suppressed())
            return;
        current_thread.process().profiling_data().with([&](auto& data) {
            if (auto* event_buffer = data.current_perf_events_buffer()) {
    ...
    {
        current_process.profiling_data().with([&](auto& data) {
            if (auto* event_buffer = data.current_perf_events_buffer()) {
