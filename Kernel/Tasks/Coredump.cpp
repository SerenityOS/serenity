...
ErrorOr<NonnullRefPtr<OpenFileDescription>> Coredump::try_create_target_file(Process const& process, StringView output_path)
{
    ...
    auto current_vfs_root_context_custody = current_vfs_root_context->custody().with([](auto& custody) -> NonnullRefPtr<Custody> {
        return custody;
    });
    auto dump_directory = TRY(VirtualFileSystem::the().open_directory(current_vfs_root_context, Process::current().credentials(), output_directory, *current_vfs_root_context_custody));
