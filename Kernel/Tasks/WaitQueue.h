...
#include <Kernel/Library/Interrupted.h>
...
class WaitQueue {
    ...
    public:
        ...
        InterruptedOr<void> wait_until(WaitQueue& wait_queue, F should_wake)
        {
        ...
        InterruptedOr<void> wait_until(WaitQueue& wait_queue, Spinlock<Rank>& lock, F should_wake)
        {
            return wait_until_impl(wait_queue, should_wake, lock);
        }

        template<typename T, LockRank Rank, CallableAs<bool, T&> F>
        InterruptedOr<void> wait_until(WaitQueue& wait_queue, SpinlockProtected<T, Rank>& spinlock_protected, F should_wake)
        {
    ...
    private:
        template<LockRank Rank, CallableAs<bool> F>
        InterruptedOr<void> wait_until_impl(WaitQueue& wait_queue, F should_wake, Optional<Spinlock<Rank>&> user_lock)
        {
...
public:
    ...
    template<CallableAs<bool> F>
    InterruptedOr<void> wait_until(F should_wake)
    {
        Waiter waiter;
    ...
    template<LockRank Rank, CallableAs<bool> F>
    InterruptedOr<void> wait_until(Spinlock<Rank>& lock, F should_wake)
    {
        Waiter waiter;
    ...
    template<typename T, LockRank Rank, CallableAs<bool, T&> F>
    InterruptedOr<void> wait_until(SpinlockProtected<T, Rank>& spinlock_protected, F should_wake)
    {
        Waiter waiter;
