...
ErrorOr<Process::ProcessAndFirstThread> Process::create_userland_init_process(StringView path, Vector<NonnullOwnPtr<KString>> arguments, NonnullRefPtr<VFSRootContext> vfs_root_context, NonnullRefPtr<HostnameContext> hostname_context, RefPtr<TTY> tty)
{
    ...
    // NOTE: We must create a new Path, because the path in the VFSRootContext has a fake OptimizedPath
    auto current_directory_path = TRY(vfs_root_context->root_path().with([path](auto& root_path) -> ErrorOr<Path> {
        ...
        auto canonicalized_current_directory_path = TRY(OptimizedPath::create(root_path.custody(), path));
...
ErrorOr<Process::ProcessAndFirstThread> Process::create_kernel_process(StringView name, void (*entry)(void*), void* entry_data, u32 affinity, RegisterProcess do_register)
{
    ...
    auto& empty_canonicalized_path = OptimizedPath::fake_root_path();
...
Path Process::current_directory()
{
    ...
    VERIFY(&path.optimized_path() != &OptimizedPath::fake_root_path());
    return path;
}
