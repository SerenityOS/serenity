...
private:
    ...
    /**
     * @brief Performs the system shutdown or reboot operation.
     *
     * This function handles the process of shutting down or rebooting the system.
     * It ensures that all necessary steps are taken to transition the system to the
     * desired power state.
     *
     * @param do_reboot Specifies whether the operation should result in a reboot (`DoReboot::Yes`)
     * or a shutdown (`DoReboot::No`).
     * @return An `ErrorOr<void>` indicating the success or failure of the operation.
     */
    static ErrorOr<void> perform_shutdown(DoReboot do_reboot);
};
