...
static bool match_table_signature(PhysicalAddress table_header, StringView signature)
{
    ...
    auto check_in_region = [&](size_t length, auto&& callback) -> bool {
        auto mapping_length = MUST(Memory::page_round_up(table_header.offset_in_page() + length));
        auto region = MUST(MM.allocate_mmio_kernel_region(table_header.page_base(), mapping_length, {}, Memory::Region::Access::Read));
        u8 const* base = region->vaddr().offset(table_header.offset_in_page()).as_ptr();
        return callback(base);
    };

    u32 length = 0;
    auto check_signature_and_get_length = [&](u8 const* base) -> bool {
        if (strncmp(reinterpret_cast<char const*>(base), signature.characters_without_null_termination(), 4) != 0)
            return false;
