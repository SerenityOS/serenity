...
ALWAYS_INLINE int gpu_connector_get_properties(int fd, GPUConnectorProperties* info)
{
    return ioctl(fd, GPU_IOCTL_GET_PROPERTIES, info);
}

ALWAYS_INLINE int gpu_connector_get_head_edid(int fd, GPUHeadEDID* info)
{
...
ALWAYS_INLINE int gpu_connector_set_responsible(int fd)
{
    return ioctl(fd, GPU_IOCTL_SET_RESPONSIBLE, nullptr);
}

ALWAYS_INLINE int gpu_connector_unset_responsible(int fd)
{
    return ioctl(fd, GPU_IOCTL_UNSET_RESPONSIBLE, nullptr);
}

ALWAYS_INLINE int fb_get_head_vertical_offset_buffer(int fd, GPUHeadVerticalOffset* vertical_offset)
{
    return ioctl(fd, GPU_IOCTL_GET_HEAD_VERTICAL_OFFSET_BUFFER, vertical_offset);
}

ALWAYS_INLINE int fb_set_head_vertical_offset_buffer(int fd, GPUHeadVerticalOffset* vertical_offset)
{
...
ALWAYS_INLINE int gpu_connector_set_safe_head_mode_setting(int fd)
{
...
ALWAYS_INLINE int fb_flush_buffers(int fd, int index, FBRect const* rects, unsigned count)
{
    ...
    return ioctl(fd, GPU_IOCTL_FLUSH_HEAD_BUFFERS, &fb_flush_rects);
}

ALWAYS_INLINE int fb_flush_head(int fd)
{
    return ioctl(fd, GPU_IOCTL_FLUSH_HEAD, nullptr);
}
