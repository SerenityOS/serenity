...
ErrorOr<NonnullOwnPtr<KBuffer>> FUSEDevice::send_request_and_wait_for_a_reply(OpenFileDescription const& description, Bytes bytes)
{
    ...
    ErrorOr<KBuffer*> reply_or_error = nullptr;
    auto receive_reply = [&](auto& instances) -> bool {
        ...
        for (size_t i = 0; i < requests_for_instance.size(); ++i) {
            ...
            if (!requests_for_instance[i].response_ready)
                continue;
            auto buffer_or_error = KBuffer::try_create_with_bytes("FUSEDevice: Response"sv, requests_for_instance[i].response->bytes());
            if (buffer_or_error.is_error())
                reply_or_error = buffer_or_error.release_error();
            else
                reply_or_error = buffer_or_error.release_value().leak_ptr();
            requests_for_instance.remove(i);
            return true;
        }
    ...
    return reply_or_error.is_error() ? reply_or_error.release_error() : adopt_nonnull_own_or_enomem(reply_or_error.release_value());
}
