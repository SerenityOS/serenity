...
static NS16550* s_the = nullptr;

NS16550& PCINS16550::the()
{
    VERIFY(s_the);
    return *s_the;
}

bool PCINS16550::is_available()
{
    return s_the;
}
...
struct BoardDefinition {
    ...
    u32 first_offset { 0 };
    u32 port_size { 0 };
    NS16550::Baud baud_rate { NS16550::Baud::Baud38400 };
};

static constexpr BoardDefinition board_definitions[] = {
    { { PCI::VendorID::RedHat, 0x0002 }, "QEMU PCI 16550A"sv, 1, 0, 0, 8, NS16550::Baud::Baud115200 },
    { { PCI::VendorID::RedHat, 0x0003 }, "QEMU PCI Dual-port 16550A"sv, 2, 0, 0, 8, NS16550::Baud::Baud115200 },
    { { PCI::VendorID::RedHat, 0x0004 }, "QEMU PCI Quad-port 16550A"sv, 4, 0, 0, 8, NS16550::Baud::Baud115200 },
    { { PCI::VendorID::WCH, 0x2273 }, "WCH CH351"sv, 2, 0, 0, 8, NS16550::Baud::Baud115200 },
    { { PCI::VendorID::WCH, 0x3253 }, "WCH CH382 2S"sv, 2, 0, 0xC0, 8, NS16550::Baud::Baud115200 }
};
...
static constexpr BoardDefinition generic_board_definition = {
    ...
    .first_offset = 0,
    .port_size = 8,
    .baud_rate = NS16550::Baud::Baud115200,
};

size_t s_current_device_minor = 4;

PCI_DRIVER(NS16550Driver);

ErrorOr<void> NS16550Driver::probe(PCI::DeviceIdentifier const& pci_device_identifier) const
{
    ...
    auto initialize_serial_device = [&pci_device_identifier](BoardDefinition const& board_definition) {
        ...
        for (size_t i = 0; i < board_definition.port_count; i++) {
            ...
            auto serial_device = Device::try_create_device<NS16550>(move(port_registers_io_window), s_current_device_minor++).release_value_but_fixme_should_propagate_errors();
            if (board_definition.baud_rate != NS16550::Baud::Baud38400) // non-default baud
        ...
        dmesgln("NS16550: Found {} @ {}", board_definition.name, pci_device_identifier.address());
    };
