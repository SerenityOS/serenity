...
ErrorOr<void> Device::initialize()
{
    for (auto const& application_collection : m_parsed_report_descriptor.application_collections) {
        ...
        case Keyboard: {
            auto keyboard_handler = TRY(HID::KeyboardDriver::create(*this, application_collection));
            m_application_collection_handlers.append(move(keyboard_handler));
            break;
        }

        case Mouse: {
            auto mouse_handler = TRY(HID::MouseDriver::create(*this, application_collection));
            m_application_collection_handlers.append(move(mouse_handler));
            break;
        }
    ...
    TRY(m_transport_interface->start_receiving_input_reports([this](ReadonlyBytes report_data) {
        for (auto& application_collection_handler : m_application_collection_handlers) {
            // FIXME: Rate limit this message?
            if (auto result = application_collection_handler.on_report(report_data); result.is_error())
