...
{
    return { (&m_in_memory_buffer[x * sizeof(u32) * (m_glyph_columns + m_glyph_spacing) + y * m_glyph_rows * framebuffer_pitch()]) };
}
...
void GenericFramebufferConsoleImpl::scroll_up()
{
    ...
    memcpy(framebuffer_data(), m_in_memory_buffer, height() * framebuffer_pitch());
}
