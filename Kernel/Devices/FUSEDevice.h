...
private:
    ...
    FUSEInstance const* get_instance_from_fd(OpenFileDescription const& fd) const;
    FUSEInstance* get_mutable_instance_from_fd(OpenFileDescription const& fd);
    void remove_instance(OpenFileDescription const&);

    Mutex m_lock { "FUSEDevice"sv };
    Vector<FUSEInstance> m_instances;
};
