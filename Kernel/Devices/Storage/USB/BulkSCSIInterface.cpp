...
ErrorOr<void> BulkSCSIInterface::do_read(u32 block_index, u32 block_count, UserOrKernelBuffer& buffer, size_t)
{
    ...
    while (blocks_read < block_count) {
        ...
        auto status = TRY(send_scsi_command<SCSIDataDirection::DataToHost>(*m_out_pipe, *m_in_pipe, read_command, destination_buffer, transfer_length_bytes));
...
ErrorOr<void> BulkSCSIInterface::do_write(u32 block_index, u32 block_count, UserOrKernelBuffer& buffer, size_t)
{
    ...
    while (blocks_read < block_count) {
        ...
        auto status = TRY(send_scsi_command<SCSIDataDirection::DataToDevice>(*m_out_pipe, *m_in_pipe, read_command, source_buffer, transfer_length_bytes));
