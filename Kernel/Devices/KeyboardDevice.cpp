...
void KeyboardDevice::key_state_changed(u8 raw, bool pressed)
{
    ...
    char character = (m_modifiers & Mod_Shift) ? shift_map[raw]: 
                     (m_modifiers & Mod_Alt)   ?   alt_map[raw]: 
                                                       map[raw];
    ...
    else
    {
        if(m_has_e0_prefix) {
            if(key == Key_Slash) {
                character = '/'; // On Turkish-QWERT Keyboard Key_Slash mapped to '.' char, if e0 prefix is true remap to '/' char
            }
        }
    }
