...
public:
    ...
    void attach_standalone_hid_mouse_device(HIDDevice&);
    void detach_standalone_hid_mouse_device(HIDDevice&);
...
private:
    ...
    // (which are not attached via HIDController in the sense that there's no specific
    // hardware controller to coordinate their usage).
