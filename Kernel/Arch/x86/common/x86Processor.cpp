...
{
    // NOTE: This is called during x86Processor::early_initialize, we cannot
...
extern "C" void enter_thread_context(Thread* from_thread, Thread* to_thread)
{
    ...
    bool has_fxsr = x86Processor::current().has_feature(CPUFeature::FXSR);
    bool has_xsave_avx_support = x86Processor::current().has_feature(CPUFeature::XSAVE) && x86Processor::current().has_feature(CPUFeature::AVX);
...
extern "C" FlatPtr do_init_context(Thread* thread, u32 flags)
{
    ...
    return x86Processor::current().init_context(*thread, true);
}
