...
safe_memset_faulted:
                 ...
                 : [dest_ptr] "+&r"(dest_ptr), [result] "+&r"(result)
                 : [n] "r"(n), [c] "r"(c), "r"(fault_at_in_x3)
                 : "memory", "x4");
    return result != 0;
}
...
NEVER_INLINE FLATTEN ssize_t safe_strnlen(char const* str, unsigned long max_n, void*& fault_at)
{
    ...
    asm volatile(R"(
    cbz %[max_n], 2f
    mov x3, %[str]               // x3: original string pointer
    mov %[result], #0

1:
.global safe_strnlen_ins
safe_strnlen_ins:
    ldrb w4, [x3, %[result]]     // w4: current char
    cbz w4, 2f
    add %[result], %[result], #1
    cmp %[result], %[max_n]
    b.ne 1b
...
safe_strnlen_faulted:
                 ...
                 : [str] "+&r"(str), [result] "+&r"(result)
                 : [max_n] "r"(max_n), "r"(fault_at_in_x2)
                 : "memory", "x3", "w4");
    return result;
}
...
safe_memcpy_faulted:
                 ...
                 : [result] "+&r"(result)
                 : [dest_ptr] "r"(dest_ptr), [src_ptr] "r"(src_ptr), [n] "r"(n), "r"(fault_at_in_x3)
                 : "memory", "x4", "w5");
    return result != 0;
}
