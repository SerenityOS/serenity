...
bool I8042Controller::handle_irq(Badge<I8042ControllerIRQHandler>, u8 irq_number)
{
    ...
    PS2PortIndex port_index = I8042PS2PortIndex::FirstPort;
    if (irq_number == IRQ_SECOND_PORT) {
        port_index = I8042PS2PortIndex::SecondPort;
    }
...
UNMAP_AFTER_INIT ErrorOr<void> I8042Controller::detect_devices(EnableFirstPortTranslation enable_first_port_translation)
{
    u8 configuration;
    {
        SpinlockLocker lock(m_lock);
        ...
        if (enable_first_port_translation == EnableFirstPortTranslation::Yes)
    ...
    if (m_first_port_available) {
        ...
        auto keyboard_device_scan_code_set = enable_first_port_translation == EnableFirstPortTranslation::Yes ? ScanCodeSet::Set1 : ScanCodeSet::Set2;
        auto error_or_device = PS2KeyboardDevice::try_to_initialize(*this, I8042PS2PortIndex::FirstPort, keyboard_device_scan_code_set, *keyboard_device);
        if (error_or_device.is_error()) {
            ...
            TRY(send_command(I8042PS2PortIndex::FirstPort, I8042Command::DisablePacketStreaming));
...
bool I8042Controller::irq_process_input_buffer(PS2PortIndex port_index)
{
    ...
    else if (port_index == I8042PS2PortIndex::SecondPort)
...
ErrorOr<void> I8042Controller::do_reset_device(PS2PortIndex port_index)
{
    VERIFY(m_lock.is_locked());

    VERIFY(!Processor::current_in_irq());
    auto reset_result = TRY(do_send_command(port_index, I8042Command::Reset));
...
ErrorOr<u8> I8042Controller::do_send_command(PS2PortIndex port_index, u8 command)
{
    VERIFY(m_lock.is_locked());

    VERIFY(!Processor::current_in_irq());

    return do_write_to_device(port_index, command);
}

ErrorOr<u8> I8042Controller::do_send_command(PS2PortIndex port_index, u8 command, u8 data)
{
    VERIFY(m_lock.is_locked());

    VERIFY(!Processor::current_in_irq());

    u8 response = TRY(do_write_to_device(port_index, command));
    if (response == I8042Response::Acknowledge)
        response = TRY(do_write_to_device(port_index, data));
    return response;
}

ErrorOr<u8> I8042Controller::do_write_to_device(PS2PortIndex port_index, u8 data)
{
    VERIFY(m_lock.is_locked());
    ...
    do {
        if (port_index != I8042PS2PortIndex::FirstPort) {
            VERIFY(port_index == I8042PS2PortIndex::SecondPort);
...
ErrorOr<void> I8042Controller::prepare_for_input(PS2PortIndex port_index)
{
    VERIFY(m_lock.is_locked());
    u8 const second_port_flag = port_index == I8042PS2PortIndex::FirstPort ? 0 : I8042StatusFlag::SecondPS2PortOutputBuffer;
