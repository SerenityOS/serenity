...
public:
    ...
    virtual ErrorOr<u8> send_command(PS2PortIndex port_index, u8 command) override
    {
        SpinlockLocker lock(m_lock);
        return do_send_command(port_index, command);
    }
    virtual ErrorOr<u8> send_command(PS2PortIndex port_index, u8 command, u8 data) override
    {
        SpinlockLocker lock(m_lock);
        return do_send_command(port_index, command, data);
    }
...
private:
    ...
    ErrorOr<u8> do_send_command(PS2PortIndex port_index, u8 data);
    ErrorOr<u8> do_send_command(PS2PortIndex port_index, u8 command, u8 data);
    ErrorOr<u8> do_write_to_device(PS2PortIndex port_index, u8 data);
