...
public class compareto001 {
    ...
    private final static String prefix = "nsk.jdi.ShortValue.compareTo";
    ...
    private static void execTest() {
        ...
        for (int i = 0; i < fieldList.size(); i++ ) {
            ...
            display("Checking compateTo(Object object) method for ShortValue: " + value);
    ...
    private static boolean PerformComparing(ShortValue value, Object object ) {
        ...
        if (object instanceof ShortValue) {
            ShortValue shortObject = (ShortValue)object;
            try {
                // assertion [ x.compareTo(y) == 0 <==> x.equals(y) ]
                if ( ((value.equals(object)) && (value.compareTo(shortObject) != 0)) ||
                     (!(value.equals(object)) && (value.compareTo(shortObject) == 0)) ) {
                ...
                if ( ((value.compareTo(shortObject) == 0) && (shortObject.compareTo(value) != 0)) ||
                     ((value.compareTo(shortObject) != 0) && (shortObject.compareTo(value) == 0)) ) {
                    complain("Failed assertion [ (x.compareTo(y) == 0) is identical to (y.compareTo(x) == 0) ] \n\t" +
                             "where 'x' is ShortValue: " + value + " and 'y' is ShortValue : " + shortObject + " \n\t" +
                             "result of (x.compareTo(y)): " + value.compareTo(shortObject) + "\n\t" +
                             "result of (y.compareTo(x)): " + shortObject.compareTo(value) );
                    result = false;
                }
                if (value.compareTo(shortObject) != 0) {
                    // assertion [ if (x.compareTo(y) == i) then (y.compareTo(x) == -i) ]
                    if (value.compareTo(shortObject) != -(shortObject.compareTo(value))) {
                ...
                if (value.compareTo(shortObject) > 0) {
                     ShortValue lessValue = FindLessShortValue(shortObject);
                     if (lessValue != null) {
                ...
                complain("Caught unexpected " + e + " when comparing \n\t" +
                         "ShortValue: " + value + " and ShortValue argument: " + object);
                result = false;
            }
            ...
            try {
                ...
                complain("Does not throw expected NullPointerException when comparing \n\t" +
                         "ShortValue: " + value + " and null argument");
                result = false;
            ...
            try {
                value.compareTo((ShortValue)object);
                complain("Does not throw expected ClassCastException when comparing \n\t" +
                         "ShortValue: " + value + " and argument: " + object);
                result = false;
     ...
     *  to find another ShortValue field which is less then method's argument via
     *  <i>compareTo</i> method.
     */

    private static ShortValue FindLessShortValue (ShortValue currentValue) {
        ShortValue result = null;

        for (int i = 0; i < fieldList.size(); i++ ) {

            Field field = (Field )fieldList.get(i);
            ShortValue newValue = (ShortValue)((ObjectReference )objectValue).getValue(field);
