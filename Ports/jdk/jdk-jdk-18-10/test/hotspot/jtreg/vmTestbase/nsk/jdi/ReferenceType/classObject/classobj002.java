...
public class classobj002 extends Log {
    ...
    private final static String
        ...
        thisClassName = package_prefix + "classobj002",
    ...
    public static int run (String argv[], PrintStream out) {
        ...
        int v_test_result = new classobj002().runThis(argv,out_stream);
        if ( v_test_result == 2/*STATUS_FAILED*/ ) {
            out_stream.println
                ("\n==> nsk/jdi/ReferenceType/classObject/classobj002 test FAILED");
        }
        else {
            out_stream.println
                ("\n==> nsk/jdi/ReferenceType/classObject/classobj002 test PASSED");
        }
    ...
    private int runThis (String argv[], PrintStream out) {
        ...
        out_stream.println("==> nsk/jdi/ReferenceType/classObject/classobj002 test LOG:");
        ...
        print_log_on_verbose("--> classobj002: classobj002a debugee launched");
        debugee.resume();

        String debugee_signal = pipe.readln();
        if (debugee_signal == null) {
            out_stream.println
                ("##> classobj002: UNEXPECTED debugee's signal (not \"ready0\") - " + debugee_signal);
        ...
        if (!debugee_signal.equals("ready0")) {
            ...
            print_log_on_verbose("--> classobj002: debugee's \"ready0\" signal recieved!");
        }
        ...
        if (debugee_signal == null) {
            out_stream.println
                ("##> classobj002: UNEXPECTED debugee's signal (not \"ready1\") - " + debugee_signal);
            return 2/*STATUS_FAILED*/;
        }
        if (!debugee_signal.equals("ready1")) {
            ...
            print_log_on_verbose("--> classobj002: debugee's \"ready1\" signal recieved!");
        }

        boolean class_not_found_error = false;
        boolean classObject_method_error = false;
        while ( true ) {
            print_log_on_verbose
                ("--> classobj002: getting ReferenceType object for loaded checked class...");
            ReferenceType refType = debugee.classByName(checked_class);
            if (refType == null) {
                print_log_without_verbose
                    ("--> classobj002: getting ReferenceType object for loaded checked class...");
                out_stream.println("##> classobj002: FAILED: Could NOT FIND checked class: " + checked_class);
                class_not_found_error = true;
                break;
            }
            else {
                print_log_on_verbose("--> classobj002: checked class FOUND: " + checked_class);
            }
            print_log_on_verbose
                ("--> classobj002: waiting for \"ready2\" or \"not_unloaded\" signal from debugee...");
            pipe.println("continue");
            debugee_signal = pipe.readln();
            if (debugee_signal == null) {
                out_stream.println
                    ("##> classobj002: UNEXPECTED debugee's signal - " + debugee_signal);
                return 2/*STATUS_FAILED*/;
            }
            if ( debugee_signal.equals("not_unloaded")) {
                out_stream.println
                    ("--> classobj002: debugee's \"not_unloaded\" signal recieved!");
                print_log_without_verbose
                    ("-->              checked class may be NOT unloaded!");
                out_stream.println
                    ("-->              ReferenceType.classObject() method can NOT be checked!");
                break;
            }
            if (!debugee_signal.equals("ready2")) {
                out_stream.println
                    ("##> classobj002: UNEXPECTED debugee's signal (not \"ready2\") - " + debugee_signal);
                return 2/*STATUS_FAILED*/;
            }
            else {
                print_log_on_verbose("--> classobj002: debugee's \"ready2\" signal recieved!");
            }
            print_log_on_verbose
                ("--> classobj002: check that checked class has been unloaded realy...");
            ReferenceType refType2 = debugee.classByName(checked_class);
            if (refType2 == null) {
                print_log_on_verbose
                    ("--> classobj002: checked class has been unloaded realy: " + checked_class);
            }
            else {
                print_log_without_verbose
                    ("--> classobj002: check that checked class has been unloaded realy...");
                out_stream.println
                    ("--> classobj002: checked class FOUND: " + checked_class
            ...
            out_stream.println
                ("--> classobj002: check ReferenceType.classObject() method for unloaded class...");
            ClassObjectReference class_obj_ref = null;
            try {
                class_obj_ref = refType.classObject();
            }
            catch (Exception expt) {
                if (expt instanceof com.sun.jdi.ObjectCollectedException) {
                    out_stream.println
                        ("--> classobj002: PASSED: expected Exception thrown - " + expt.toString());
                }
                else {
                    out_stream.println
                        ("##> classobj002: FAILED: unexpected Exception thrown - " + expt.toString());
                    out_stream.println
                        ("##>              expected Exception - com.sun.jdi.ObjectCollectedException");
                    classObject_method_error = true;
                }
            }
            break;
        }
        int v_test_result = 0/*STATUS_PASSED*/;
        if ( class_not_found_error || classObject_method_error ) {
            v_test_result = 2/*STATUS_FAILED*/;
        }

        print_log_on_verbose("--> classobj002: waiting for debugee finish...");
        pipe.println("quit");
        debugee.waitFor();
        ...
        if (status != 0/*STATUS_PASSED*/ + 95/*STATUS_TEMP*/) {
            out_stream.println
                ("##> classobj002: UNEXPECTED Debugee's exit status (not 95) - " + status);
            v_test_result = 2/*STATUS_FAILED*/;
        }
        else {
            print_log_on_verbose
                ("--> classobj002: expected Debugee's exit status - " + status);
        }

        return v_test_result;
    }
}  // end of classobj002 class
