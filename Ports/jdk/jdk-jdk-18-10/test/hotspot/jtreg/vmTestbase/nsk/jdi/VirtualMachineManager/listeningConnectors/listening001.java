 ...
 * <code>com.sun.jdi.VirtualMachineManager.listeningConnectors()</code> <BR>
 * complies with its specification.                             <BR>
 * <BR>
 * The test checks up that invoking the method                  <BR>
 *   VirtualMachineManager.listeningConnectors() doesn't throw  <BR>
 *  an exception and returnes a List of non-null values of      <BR>
 *  the ListeningConnector type each.                           <BR>
 */

public class listening001 {
    ...
    public static int run (String argv[], PrintStream out) {
        return new listening001().runThis(argv, out);
    }
    ...
    //------------------------------------------------------  testing section
        ...
        if (vmm == null) {
            ...
            try {
                log2("......call to vmm.listeningConnectors()");
                List connectors = vmm.listeningConnectors();
                log2("       size of List == " + connectors.size());

                log2("......checking up on nulls and casts to ListeningConnector");
                ListIterator li = connectors.listIterator();
                for (; li.hasNext(); ) {
                    ListeningConnector listeningConnector = (ListeningConnector) li.next();
                    if (listeningConnector == null) {
                        log3("ERROR: listeningConnector == null");
                        testExitCode = FAILED;
                    }
                }
            } catch ( Exception e) {
