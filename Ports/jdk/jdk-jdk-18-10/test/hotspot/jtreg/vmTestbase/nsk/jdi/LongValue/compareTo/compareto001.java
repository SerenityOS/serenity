...
public class compareto001 {
    ...
    private final static String prefix = "nsk.jdi.LongValue.compareTo";
    ...
    private static void execTest() {
        ...
        for (int i = 0; i < fieldList.size(); i++ ) {
            ...
            display("Checking compateTo(Object object) method for LongValue: " + value);
    ...
    private static boolean PerformComparing(LongValue value, Object object ) {
        ...
        if (object instanceof LongValue) {
            LongValue longObject = (LongValue)object;
            try {
                // assertion [ x.compareTo(y) == 0 <==> x.equals(y) ]
                if ( ((value.equals(object)) && (value.compareTo(longObject) != 0)) ||
                     (!(value.equals(object)) && (value.compareTo(longObject) == 0)) ) {
                ...
                if ( ((value.compareTo(longObject) == 0) && (longObject.compareTo(value) != 0)) ||
                     ((value.compareTo(longObject) != 0) && (longObject.compareTo(value) == 0)) ) {
                    complain("Failed assertion [ (x.compareTo(y) == 0) is identical to (y.compareTo(x) == 0) ] \n\t" +
                             "where 'x' is LongValue: " + value + " and 'y' is LongValue : " + longObject + " \n\t" +
                             "result of (x.compareTo(y)): " + value.compareTo(longObject) + "\n\t" +
                             "result of (y.compareTo(x)): " + longObject.compareTo(value) );
                    result = false;
                }
                if (value.compareTo(longObject) != 0) {
                    // assertion [ if (x.compareTo(y) == i) then (y.compareTo(x) == -i) ]
                    if (value.compareTo(longObject) != -(longObject.compareTo(value))) {
                ...
                if (value.compareTo(longObject) > 0) {
                     LongValue lessValue = FindLessLongValue(longObject);
                     if (lessValue != null) {
                ...
                complain("Caught unexpected " + e + " when comparing \n\t" +
                         "LongValue: " + value + " and LongValue argument: " + object);
                result = false;
            }
            ...
            try {
                ...
                complain("Does not throw expected NullPointerException when comparing \n\t" +
                         "LongValue: " + value + " and null argument");
                result = false;
            ...
            try {
                value.compareTo((LongValue)object);
                complain("Does not throw expected ClassCastException when comparing \n\t" +
                         "LongValue: " + value + " and argument: " + object);
                result = false;
     ...
     *  to find another LongValue field which is less then method's argument via
     *  <i>compareTo</i> method.
     */

    private static LongValue FindLessLongValue (LongValue currentValue) {
        LongValue result = null;

        for (int i = 0; i < fieldList.size(); i++ ) {

            Field field = (Field )fieldList.get(i);
            LongValue newValue = (LongValue)((ObjectReference )objectValue).getValue(field);
