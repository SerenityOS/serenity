...
public class isinit002 {
    static ArgumentHandler argsHandler;
    static Log test_log_handler;
    static boolean verbose_mode = false;  // test argument -verbose switches to true
                                          // - for more easy failure evaluation

    /** The main class isInitializeds of the debugger & debugee applications. */
    private final static String
        package_prefix = "nsk.jdi.ReferenceType.isInitialized.",
        thisClassName = package_prefix + "isinit002",
    ...
    public static int run (String argv[], PrintStream out) {
        ...
        int v_test_result = new isinit002().runThis(argv,out);
    ...
    private int runThis (String argv[], PrintStream out) {
        ...
        print_log_anyway("==> nsk/jdi/ReferenceType/isInitialized/isinit002 test LOG:");
        print_log_anyway("--> test checks isInitialized() method of ReferenceType interface ");
        ...
        print_log_on_verbose("--> isinit002: isinit002a debugee launched");
        debugee.resume();

        String debugee_signal = pipe.readln();
        if (debugee_signal == null) {
            print_log_anyway
                ("##> isinit002: UNEXPECTED debugee's signal (not \"ready0\") - " + debugee_signal);
        ...
        if (!debugee_signal.equals("ready0")) {
            ...
            print_log_on_verbose("--> isinit002: debugee's \"ready0\" signal recieved!");
        }
        ...
        if (debugee_signal == null) {
            print_log_anyway
                ("##> isinit002: UNEXPECTED debugee's signal (not \"ready1\") - " + debugee_signal);
            return 2/*STATUS_FAILED*/;
        }
        if (!debugee_signal.equals("ready1")) {
            ...
            print_log_on_verbose("--> isinit002: debugee's \"ready1\" signal recieved!");
        }

        boolean class_not_found_error = false;
        boolean isInitialized_method_error = false;
        while ( true ) {
            ...
            if (refType == null) {
                ...
                print_log_on_verbose("--> isinit002: checked class FOUND: " + checked_class);
            }
            print_log_on_verbose
                ("--> isinit002: waiting for \"ready2\" or \"not_unloaded\" signal from debugee...");
            pipe.println("continue");
            debugee_signal = pipe.readln();
            if (debugee_signal == null) {
                print_log_anyway
                    ("##> isinit002: UNEXPECTED debugee's signal - " + debugee_signal);
                return 2/*STATUS_FAILED*/;
            }
            ...
            if (!debugee_signal.equals("ready2")) {
                print_log_anyway
                    ("##> isinit002: UNEXPECTED debugee's signal (not \"ready2\") - " + debugee_signal);
                return 2/*STATUS_FAILED*/;
            }
            else {
                print_log_on_verbose("--> isinit002: debugee's \"ready2\" signal recieved!");
            }
            print_log_on_verbose
            ...
            print_log_anyway
                ("--> isinit002: check ReferenceType.isInitialized() method for unloaded class...");
            boolean ref_type_is_initialized = false;
            try {
                ref_type_is_initialized = refType.isInitialized();
            }
            catch (Exception expt) {
                if (expt instanceof com.sun.jdi.ObjectCollectedException) {
                    print_log_anyway
                        ("--> isinit002: PASSED: expected Exception thrown - " + expt.toString());
                }
                else {
                    print_log_anyway
                        ("##> isinit002: FAILED: unexpected Exception thrown - " + expt.toString());
                    print_log_anyway
                        ("##>            expected Exception - com.sun.jdi.ObjectCollectedException");
                    isInitialized_method_error = true;
                }
            }
            break;
        }
        int v_test_result = 0/*STATUS_PASSED*/;
        if ( class_not_found_error || isInitialized_method_error ) {
            v_test_result = 2/*STATUS_FAILED*/;
        }

        print_log_on_verbose("--> isinit002: waiting for debugee finish...");
        pipe.println("quit");
        debugee.waitFor();
        ...
        if (status != 0/*STATUS_PASSED*/ + 95/*STATUS_TEMP*/) {
            print_log_anyway
                ("##> isinit002: UNEXPECTED Debugee's exit status (not 95) - " + status);
            v_test_result = 2/*STATUS_FAILED*/;
        }
        else {
            print_log_on_verbose
                ("--> isinit002: expected Debugee's exit status - " + status);
        }

        return v_test_result;
    }
}  // end of isinit002 class
