...
public class superclass002 {
    ...
    private final static String prefix = "nsk.jdi.ClassType.superclass.";
    private final static String className = "superclass002";
    ...
    private static void check (String fieldName) {
        try {
            ...
            if (checkedClass.superclass().name().equals("java.lang.Enum")) {
                display("ClassType.superclass() returned expected java.lang.Enum for type: " + className);
            } else {
                complain("ClassType.superclass() returned unexpected type: " + checkedClass.superclass().name());
                exitStatus = Consts.TEST_FAILED;
            }
        } catch (Exception e) {
