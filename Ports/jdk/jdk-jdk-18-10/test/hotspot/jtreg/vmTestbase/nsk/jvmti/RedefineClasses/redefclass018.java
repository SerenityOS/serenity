...
public class redefclass018 {
    ...
    static {
        try {
            System.loadLibrary("redefclass018");
        } catch (UnsatisfiedLinkError ule) {
            System.err.println("Could not load redefclass018 library");
    ...
    public static int run(String args[], PrintStream out) {
        ...
        String fileName = fileDir + File.separator + "newclass" + File.separator +
            redefclass018a.class.getName().replace('.', File.separatorChar) +
            ".class";
        try {
        ...
        getReady(redefclass018a.class, bytes);

        redefclass018a thr = new redefclass018a();
        thr.sout = out;
        thr.start();
        try {
            thr.join();
