    ...
    for (size_t i = 0; i < source.length(); ++i) {
        ...
        case State::InNumber:
            if (source[i] != ' ') {
                ...
                switch (curr_field_name) {
                case Field::UID:
                    result.m_uid = std::stoul(curr_value.string_view());
                    break;
                default:
        ...
        case State::InBodyText:
            if (source[i] != ')') {
                ...
                if (curr_field_name == Field::BodyText && !curr_value.is_empty()) {
                    const auto body_start = curr_value.find('\n');
                    result.m_text = curr_value.substr(body_start + 1);
                }
                curr_value.clear();
                curr_state = State::Start;
            }
...
void Message::load_flags(StringView flag_list)
{
    ...
    for (size_t i = 0; i < flag_list.length(); ++i) {
        switch (curr_state) {
        case State::Start:
            if (std::isalpha(flag_list[i]) || flag_list[i] == '\\') {
        ...
        case State::InFlag:
            if (flag_list[i] != ' ') {
                ...
                if (!curr_flag_value.is_empty() && curr_flag_value[0] == '\\') {
                    const auto flag = parse_system_flag(curr_flag_value.string_view());
                    if (flag.has_value())
                    ...
                    m_keyword_flags.append(curr_flag);
                }
                curr_flag.clear();
                curr_state = State::Start;
            }
