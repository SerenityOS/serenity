...
void GuideTool::on_context_menu(Layer*, GUI::ContextMenuEvent& event)
{
    ...
    if (!m_context_menu) {
        ...
        m_context_menu->add_action(GUI::Action::create(
            "Set &Offset", Gfx::Bitmap::try_request_resource("gear").release_value_but_fixme_should_propagate_errors(), [this](auto&) {
            ...
            editor()));
        m_context_menu->add_action(GUI::Action::create(
            "&Delete Guide", Gfx::Bitmap::try_request_resource("delete").release_value_but_fixme_should_propagate_errors(), [this](auto&) {
                if (!m_context_menu_guide)
                    return;
