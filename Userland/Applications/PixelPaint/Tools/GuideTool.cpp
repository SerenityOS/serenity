...
void GuideTool::on_context_menu(Layer*, GUI::ContextMenuEvent& event)
{
    ...
    if (!m_context_menu) {
        ...
        m_context_menu->add_action(GUI::Action::create(
            "Set &Offset"_string.release_value_but_fixme_should_propagate_errors(), Gfx::Bitmap::load_from_file("/res/icons/16x16/gear.png"sv).release_value_but_fixme_should_propagate_errors(), [this](auto&) {
            ...
            editor()));
        m_context_menu->add_action(GUI::Action::create(
            "&Delete Guide"_string.release_value_but_fixme_should_propagate_errors(), Gfx::Bitmap::load_from_file("/res/icons/16x16/delete.png"sv).release_value_but_fixme_should_propagate_errors(), [this](auto&) {
                if (!m_context_menu_guide)
                    return;
