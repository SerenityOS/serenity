...
ErrorOr<int> serenity_main(Main::Arguments arguments)
{
    ...
    auto kill_action = GUI::Action::create(
        "&Kill Process"_string, { Mod_Ctrl, Key_K }, { Key_Delete }, TRY(Gfx::Bitmap::load_from_file("/res/icons/16x16/kill.png"sv)), [&](const GUI::Action&) {
    ...
    auto stop_action = GUI::Action::create(
        "&Stop Process"_string, { Mod_Ctrl, Key_S }, TRY(Gfx::Bitmap::load_from_file("/res/icons/16x16/stop-hand.png"sv)), [&](const GUI::Action&) {
            pid_t pid = selected_id(ProcessModel::Column::PID);
            if (pid == -1)
                return;
    ...
    auto continue_action = GUI::Action::create(
        "&Continue Process"_string, { Mod_Ctrl, Key_C }, TRY(Gfx::Bitmap::load_from_file("/res/icons/16x16/continue.png"sv)), [&](const GUI::Action&) {
            pid_t pid = selected_id(ProcessModel::Column::PID);
            if (pid != -1)
                kill(pid, SIGCONT);
        },
    ...
    auto profile_action = GUI::Action::create(
        "&Profile Process"_string, { Mod_Ctrl, Key_P },
    ...
    auto debug_action = GUI::Action::create(
        "Debug in HackStudio"_string, { Mod_Ctrl, Key_D }, TRY(Gfx::Bitmap::load_from_file("/res/icons/16x16/app-hack-studio.png"sv)), [&](const GUI::Action&) {
            pid_t pid = selected_id(ProcessModel::Column::PID);
            if (pid == -1)
                return;
            auto pid_string = DeprecatedString::number(pid);
