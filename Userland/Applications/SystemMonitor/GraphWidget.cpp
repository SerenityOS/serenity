    ...
    if (!m_values.is_empty() && !m_value_format.is_empty()) {
        ...
        for (size_t i = 0; i < min(m_value_format.size(), current_values.size()); i++) {
            ...
            if (format.text_shadow_color != Color::Transparent)
                painter.draw_text(text_rect.translated(1, 1), text.characters(), Gfx::Alignment::CenterRight, format.text_shadow_color);
            painter.draw_text(text_rect, text.characters(), Gfx::Alignment::CenterRight, graph_color);
