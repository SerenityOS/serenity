...
NetworkStatisticsWidget::NetworkStatisticsWidget()
{
    on_first_show = [this](auto&) {
        ...
        m_network_connected_bitmap = MUST(Gfx::Bitmap::load_from_uri("resource://icons/16x16/network-connected.png"sv));
        m_network_disconnected_bitmap = MUST(Gfx::Bitmap::load_from_uri("resource://icons/16x16/network-disconnected.png"sv));
        ...
        m_adapter_context_menu->add_action(GUI::Action::create(
            "Open in Network Settings...", MUST(Gfx::Bitmap::load_from_uri("resource://icons/16x16/network.png"sv)), [this](GUI::Action&) {
