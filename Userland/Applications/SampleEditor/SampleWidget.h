...
public:
    ...
    bool is_empty() const { return m_samples.is_empty(); }
    ...
    void must_repaint()
    {
        m_painter.invalidate_scene_and_repaint(*this);
    }
    void drag_repaint()
    {
        m_painter.invalidate_drag_overlay_and_repaint(*this);
    }

protected:
    SampleWidget();
    void zoom();
    intptr_t audio_thread_main();
    void reset_view_state();
    AK::Optional<double> current_playback_position_ratio();
    SampleDocument m_samples;
    SamplePainter m_painter;
    double m_cursor { 0.0 };
    double m_start { 0.0 };
    double m_scale { 1.0 };
    ...
    RefPtr<Threading::Thread> m_audio_thread;
    Atomic<bool> m_audio_should_stop { false };
    Atomic<bool> m_audio_finished { false };
