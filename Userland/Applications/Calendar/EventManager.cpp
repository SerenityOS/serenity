...
ErrorOr<void> EventManager::save(FileSystemAccessClient::File& file)
{
    ...
    TRY(stream->write_some(json.to_deprecated_string().bytes()));
    stream->close();
...
ErrorOr<JsonArray> EventManager::serialize_events()
{
    ...
    for (auto const& event : m_events) {
        JsonObject object;
        object.set("start", JsonValue(event.start.to_deprecated_string(DATE_FORMAT)));
        object.set("end", JsonValue(event.end.to_deprecated_string(DATE_FORMAT)));
...
ErrorOr<Vector<Event>> EventManager::deserialize_events(JsonArray const& json)
{
    ...
    for (auto const& value : json.values()) {
        ...
        auto summary = TRY(String::from_deprecated_string(object.get("summary"sv).release_value().as_string()));
