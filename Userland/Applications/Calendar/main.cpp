...
ErrorOr<int> serenity_main(Main::Arguments arguments)
{
    ...
    auto prev_date_action = GUI::Action::create({}, TRY(Gfx::Bitmap::try_request_resource("go-back")), [&](const GUI::Action&) {
    ...
    auto next_date_action = GUI::Action::create({}, TRY(Gfx::Bitmap::try_request_resource("go-forward")), [&](const GUI::Action&) {
        unsigned view_month = calendar->view_month();
        unsigned view_year = calendar->view_year();
        if (calendar->mode() == GUI::Calendar::Month) {
            view_month++;
    ...
    auto add_event_action = GUI::Action::create("&Add Event", {}, TRY(Gfx::Bitmap::try_request_resource("add-event")), [&](const GUI::Action&) {
        AddEventDialog::show(calendar->selected_date(), window);
    });

    auto jump_to_action = GUI::Action::create("Jump to &Today", {}, TRY(Gfx::Bitmap::try_request_resource("calendar-date")), [&](const GUI::Action&) {
        calendar->set_selected_date(Core::DateTime::now());
        calendar->update_tiles(Core::DateTime::now().year(), Core::DateTime::now().month());
    });

    auto view_month_action = GUI::Action::create_checkable("&Month View", { Mod_Ctrl, KeyCode::Key_1 }, TRY(Gfx::Bitmap::try_request_resource("calendar-month-view")), [&](const GUI::Action&) {
        if (calendar->mode() == GUI::Calendar::Year)
            calendar->toggle_mode();
    });
    view_month_action->set_checked(true);

    auto view_year_action = GUI::Action::create_checkable("&Year View", { Mod_Ctrl, KeyCode::Key_2 }, TRY(Gfx::Bitmap::try_request_resource("icon-view")), [&](const GUI::Action&) {
        if (calendar->mode() == GUI::Calendar::Month)
            calendar->toggle_mode();
    });
    ...
    file_menu.add_action(GUI::Action::create("&Add Event", { Mod_Ctrl | Mod_Shift, Key_E }, TRY(Gfx::Bitmap::try_request_resource("add-event")),
        [&](const GUI::Action&) {
            AddEventDialog::show(calendar->selected_date(), window);
        }));
