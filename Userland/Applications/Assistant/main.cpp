        ...
        on_context_menu_request = [this](auto& event) {
            if (!m_context_menu) {
                ...
                if (LexicalPath path { text().to_deprecated_string() }; path.is_absolute()) {
                    m_context_menu->add_action(GUI::Action::create("&Show in File Manager"_string.release_value_but_fixme_should_propagate_errors(), MUST(Gfx::Bitmap::load_from_file("/res/icons/16x16/app-file-manager.png"sv)), [=](auto&) {
                        Desktop::Launcher::open(URL::create_with_file_scheme(path.dirname(), path.basename()));
                    }));
                    m_context_menu->add_separator();
                }

                m_context_menu->add_action(GUI::Action::create("&Copy as Text"_string.release_value_but_fixme_should_propagate_errors(), MUST(Gfx::Bitmap::load_from_file("/res/icons/16x16/edit-copy.png"sv)), [&](auto&) {
