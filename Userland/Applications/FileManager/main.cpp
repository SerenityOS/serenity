...
ErrorOr<int> run_in_desktop_mode()
{
    ...
    auto desktop_icon = TRY(Gfx::Bitmap::load_from_uri("resource://icons/16x16/desktop.png"sv));
    ...
    auto create_archive_action
        = GUI::Action::create(
            "Create &Archive",
            TRY(Gfx::Bitmap::load_from_uri("resource://icons/16x16/filetype-archive.png"sv)),
            [&](GUI::Action const&) {
                auto paths = directory_view->selected_file_paths();
                if (paths.is_empty())
                    return;
    ...
    auto set_wallpaper_action
        = GUI::Action::create(
            ...
            TRY(Gfx::Bitmap::load_from_uri("resource://icons/16x16/app-display-settings.png"sv)),
            [&](GUI::Action const&) {
                auto paths = directory_view->selected_file_paths();
                if (paths.is_empty())
                    return;
    ...
    auto file_manager_action = GUI::Action::create("Open in File &Manager", {}, TRY(Gfx::Bitmap::load_from_uri("resource://icons/16x16/app-file-manager.png"sv)), [&](auto&) {
        auto paths = directory_view->selected_file_paths();
        if (paths.is_empty()) {
    ...
    auto open_terminal_action = GUI::Action::create("Open in &Terminal", {}, TRY(Gfx::Bitmap::load_from_uri("resource://icons/16x16/app-terminal.png"sv)), [&](auto&) {
        auto paths = directory_view->selected_file_paths();
        if (paths.is_empty()) {
    ...
    auto display_properties_action = GUI::Action::create("&Display Settings", {}, TRY(Gfx::Bitmap::load_from_uri("resource://icons/16x16/app-display-settings.png"sv)), [&](GUI::Action const&) {
...
ErrorOr<int> run_in_windowed_mode(ByteString const& initial_location, ByteString const& entry_focused_on_init)
{
    ...
    auto open_in_new_window_action
        = GUI::Action::create(
            "Open in New &Window",
            {},
            TRY(Gfx::Bitmap::load_from_uri("resource://icons/16x16/app-file-manager.png"sv)),
    ...
    auto open_in_new_terminal_action
        = GUI::Action::create(
            "Open in &Terminal",
            {},
            TRY(Gfx::Bitmap::load_from_uri("resource://icons/16x16/app-terminal.png"sv)),
            [&](GUI::Action const& action) {
                Vector<ByteString> paths;
                if (action.activator() == tree_view_directory_context_menu)
                    paths = tree_view_selected_file_paths();
    ...
    auto shortcut_action
        = GUI::Action::create(
            ...
            {},
            TRY(Gfx::Bitmap::load_from_uri("resource://icons/16x16/filetype-symlink.png"sv)),
            [&](GUI::Action const&) {
                auto paths = directory_view->selected_file_paths();
                if (paths.is_empty()) {
                    return;
    ...
    auto directory_open_action = GUI::Action::create("Open", TRY(Gfx::Bitmap::load_from_uri("resource://icons/16x16/open.png"sv)), [&](auto&) {
