...
ErrorOr<int> serenity_main(Main::Arguments arguments)
{
    ...
    if (!initial_location.is_empty()) {
        ...
        if (!FileSystem::is_directory(initial_location)) {
            ...
            if (path.has_extension(".zip"sv)) {
                ...
                return run_in_windowed_mode(temp_directory_path, String::from_utf8(path.basename()).release_value_but_fixme_should_propagate_errors());
            }
    ...
    if (is_selection_mode) {
        ...
        focused_entry = String::from_utf8(path.basename()).release_value_but_fixme_should_propagate_errors();
    }

    return run_in_windowed_mode(String::from_deprecated_string(initial_location).release_value_but_fixme_should_propagate_errors(), focused_entry);
}
...
void do_copy(Vector<String> const& selected_file_paths, FileOperation file_operation)
{
    ...
    GUI::Clipboard::the().set_data(copy_text.to_string().release_value_but_fixme_should_propagate_errors().bytes(), "text/uri-list");
}
...
void do_paste(String const& target_directory, GUI::Window* window)
{
    ...
    auto copied_lines = String::from_utf8(data_and_type.data).release_value_but_fixme_should_propagate_errors().split('\n').release_value_but_fixme_should_propagate_errors();
    ...
    for (auto& uri_as_string : copied_lines) {
        ...
        source_paths.append(String::from_deprecated_string(url.serialize_path()).release_value_but_fixme_should_propagate_errors());
    }
...
void do_create_link(Vector<String> const& selected_file_paths, GUI::Window* window)
{
    ...
    auto destination = String::formatted("{}/{}", Core::StandardPaths::desktop_directory(), LexicalPath::basename(path.to_deprecated_string())).release_value_but_fixme_should_propagate_errors();
    if (auto result = FileSystem::link_file(destination, path); result.is_error()) {
        GUI::MessageBox::show(window, String::formatted("Could not create desktop shortcut:\n{}", result.error()).release_value_but_fixme_should_propagate_errors(), "File Manager"sv,
...
void do_create_archive(Vector<String> const& selected_file_paths, GUI::Window* window)
{
    ...
    auto output_path = path_builder.to_string().release_value_but_fixme_should_propagate_errors();
    ...
    if (!zip_pid) {
        ...
        arg_list.append("-r");
        arg_list.append("-f");
        arg_list.append(output_path.to_deprecated_string().characters());
        for (auto const& path : selected_file_paths) {
            relative_paths.append(String::from_deprecated_string(LexicalPath::relative_path(path.bytes_as_string_view(), output_directory_path.dirname())).release_value_but_fixme_should_propagate_errors());
...
{
    auto show_error = [&] {
        GUI::MessageBox::show(window, String::formatted("Failed to set {} as wallpaper.", file_path).release_value_but_fixme_should_propagate_errors(), "Failed to set wallpaper"sv, GUI::MessageBox::Type::Error);
    };
...
void do_unzip_archive(Vector<String> const& selected_file_paths, GUI::Window* window)
{
    ...
    String output_directory_path = String::from_deprecated_string(archive_file_path.to_deprecated_string().substring(0, archive_file_path.code_points().length() - 4)).release_value_but_fixme_should_propagate_errors();
    ...
    if (!unzip_pid) {
        int rc = execlp("/bin/unzip", "/bin/unzip", "-d", output_directory_path.bytes_as_string_view(), archive_file_path.to_deprecated_string().characters(), nullptr);
        if (rc < 0) {
            perror("execlp");
            _exit(1);
        }
...
bool add_launch_handler_actions_to_menu(RefPtr<GUI::Menu>& menu, DirectoryView const& directory_view, String const& full_path, RefPtr<GUI::Action>& default_action, Vector<NonnullRefPtr<LauncherHandler>>& current_file_launch_handlers)
{
    ...
    if (default_file_handler) {
        auto file_open_action = default_file_handler->create_launch_action([&, full_path = move(full_path)](auto& launcher_handler) {
            directory_view.launch(URL::create_with_file_scheme(full_path.to_deprecated_string()), launcher_handler);
        });
        if (default_file_handler->details().launcher_type == Desktop::Launcher::LauncherType::Application)
            file_open_action->set_text(String::formatted("Run {}", file_open_action->text()).release_value_but_fixme_should_propagate_errors().to_deprecated_string());
        else
            file_open_action->set_text(String::formatted("Open in {}", file_open_action->text()).release_value_but_fixme_should_propagate_errors().to_deprecated_string());
...
ErrorOr<int> run_in_desktop_mode()
{
    ...
    auto paste_action = GUI::CommonActions::make_paste_action(
        ...
        window);
    paste_action->set_enabled(GUI::Clipboard::the().fetch_mime_type() == "text/uri-list" && access(directory_view->path().to_deprecated_string().characters(), W_OK) == 0);

    GUI::Clipboard::the().on_change = [&](DeprecatedString const& data_type) {
        paste_action->set_enabled(data_type == "text/uri-list" && access(directory_view->path().to_deprecated_string().characters(), W_OK) == 0);
    };
    ...
    auto file_manager_action = GUI::Action::create("Open in File &Manager", {}, TRY(Gfx::Bitmap::load_from_file("/res/icons/16x16/app-file-manager.png"sv)), [&](auto&) {
        auto paths = directory_view->selected_file_paths();
        if (paths.is_empty()) {
            Desktop::Launcher::open(URL::create_with_file_scheme(directory_view->path().to_deprecated_string()));
            return;
        }
    ...
    directory_view->on_context_menu_request = [&](GUI::ModelIndex const& index, GUI::ContextMenuEvent const& event) {
        if (index.is_valid()) {
            ...
            if (node.is_directory()) {
                ...
                bool added_open_menu_items = add_launch_handler_actions_to_menu(file_context_menu, directory_view, String::from_deprecated_string(node.full_path()).release_value_but_fixme_should_propagate_errors(), file_context_menu_action_default_action, current_file_handlers);
...
ErrorOr<int> run_in_windowed_mode(String const& initial_location, String const& entry_focused_on_init)
{
    ...
    auto refresh_tree_view = [&] {
        ...
        while (stat(current_path.to_deprecated_string().characters(), &st) != 0) {
            directory_view->open_parent_directory();
            current_path = directory_view->path();
            if (current_path.to_deprecated_string() == directories_model->root_path()) {
                break;
            }
        }
    ...
    auto tree_view_selected_file_paths = [&] {
        ...
        view.selection().for_each_index([&](GUI::ModelIndex const& index) {
            paths.append(String::from_deprecated_string(directories_model->full_path(index)).release_value_but_fixme_should_propagate_errors());
        });
        return paths;
    };
    ...
    auto properties_action = GUI::CommonActions::make_properties_action(
        [&](auto& action) {
            ...
            if (action.activator() == directory_context_menu || directory_view->active_widget()->is_focused()) {
                ...
                path = String::from_deprecated_string(directories_model->full_path(tree_view.selection().first())).release_value_but_fixme_should_propagate_errors();
                container_dir_path = String::from_deprecated_string(LexicalPath::basename(path.to_deprecated_string())).release_value_but_fixme_should_propagate_errors();
    ...
    auto go_home_action = GUI::CommonActions::make_go_home_action(
        [&](auto&) {
            directory_view->open(String::from_deprecated_string(Core::StandardPaths::home_directory()).release_value_but_fixme_should_propagate_errors());
        },
        window);

    GUI::Clipboard::the().on_change = [&](DeprecatedString const& data_type) {
        auto current_location = directory_view->path();
        paste_action->set_enabled(data_type == "text/uri-list" && access(current_location.to_deprecated_string().characters(), W_OK) == 0);
    };
    ...
    breadcrumbbar.on_path_change = [&](auto selected_path) {
        if (FileSystem::is_directory(selected_path)) {
            directory_view->open(String::from_utf8(selected_path).release_value_but_fixme_should_propagate_errors());
        } else {
    ...
    directory_view->on_path_change = [&](DeprecatedString const& new_path, bool can_read_in_path, bool can_write_in_path) {
        ...
        window->set_title(String::formatted("{} - File Manager", new_path).release_value_but_fixme_should_propagate_errors().to_deprecated_string());
    ...
    directory_view->on_selection_change = [&](GUI::AbstractView& view) {
        ...
        cut_action->set_enabled(!selection.is_empty() && access(directory_view->path().to_deprecated_string().characters(), W_OK) == 0);
        ...
        focus_dependent_delete_action->set_enabled((!tree_view.selection().is_empty() && tree_view.is_focused())
            || (!directory_view->current_view().selection().is_empty() && access(directory_view->path().to_deprecated_string().characters(), W_OK) == 0));
    };
    ...
    directory_view->on_context_menu_request = [&](GUI::ModelIndex const& index, GUI::ContextMenuEvent const& event) {
        if (index.is_valid()) {
            ...
            if (node.is_directory()) {
                ...
                bool added_launch_file_handlers = add_launch_handler_actions_to_menu(file_context_menu, directory_view, String::from_deprecated_string(node.full_path()).release_value_but_fixme_should_propagate_errors(), file_context_menu_action_default_action, current_file_handlers);
    ...
    tree_view.on_selection_change = [&] {
        ...
        auto path = String::from_deprecated_string(directories_model->full_path(index)).release_value_but_fixme_should_propagate_errors();
    ...
    breadcrumbbar.on_paths_drop = [&](auto path, GUI::DropEvent const& event) {
        bool const has_accepted_drop = handle_drop(event, String::from_utf8(path).release_value_but_fixme_should_propagate_errors(), window).release_value_but_fixme_should_propagate_errors();
    ...
    tree_view.on_drop = [&](GUI::ModelIndex const& index, GUI::DropEvent const& event) {
        ...
        bool const has_accepted_drop = handle_drop(event, String::from_deprecated_string(target_node.full_path()).release_value_but_fixme_should_propagate_errors(), window).release_value_but_fixme_should_propagate_errors();
        if (has_accepted_drop) {
            refresh_tree_view();
    ...
    paste_action->set_enabled(GUI::Clipboard::the().fetch_mime_type() == "text/uri-list" && access(initial_location.to_deprecated_string().characters(), W_OK) == 0);
    ...
    directory_view->set_view_mode_from_string(String::from_deprecated_string(Config::read_string("FileManager"sv, "DirectoryView"sv, "ViewMode"sv, "Icon"sv)).release_value_but_fixme_should_propagate_errors());
