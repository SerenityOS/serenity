...
ErrorOr<void> PresenterWidget::initialize_menubar()
{
    ...
    m_next_slide_action = GUI::Action::create("&Next"_string, { KeyCode::Key_Right }, TRY(Gfx::Bitmap::load_from_file("/res/icons/16x16/go-forward.png"sv)), [this](auto&) {
        if (m_current_presentation) {
    ...
    m_previous_slide_action = GUI::Action::create("&Previous"_string, { KeyCode::Key_Left }, TRY(Gfx::Bitmap::load_from_file("/res/icons/16x16/go-back.png"sv)), [this](auto&) {
        if (m_current_presentation) {
    ...
    m_present_from_first_slide_action = GUI::Action::create("Present from First &Slide"_string, { KeyCode::Key_F5 }, TRY(Gfx::Bitmap::load_from_file("/res/icons/16x16/play.png"sv)), [this](auto&) {
        if (m_current_presentation) {
    ...
    m_full_screen_action = GUI::Action::create("Toggle &Full Screen"_string, { KeyModifier::Mod_Shift, KeyCode::Key_F5 }, { KeyCode::Key_F11 }, TRY(Gfx::Bitmap::load_from_file("/res/icons/16x16/fullscreen.png"sv)), [this](auto&) {
        auto* window = this->window();
        window->set_fullscreen(!window->is_fullscreen());
    });
    m_resize_to_fit_content_action = GUI::Action::create("Resize to Fit &Content"_string, { KeyModifier::Mod_Alt, KeyCode::Key_C }, TRY(Gfx::Bitmap::load_from_file("/res/icons/16x16/scale.png"sv)), [this](auto&) {
        if (m_current_presentation) {
