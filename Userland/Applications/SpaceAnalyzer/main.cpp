...
ErrorOr<int> serenity_main(Main::Arguments arguments)
{
    ...
    auto delete_action = GUI::CommonActions::make_delete_action([&](auto&) {
        ...
        while (try_again) {
            ...
            auto deletion_result = Core::Stream::remove(selected_node_path, Core::Stream::RecursionMode::Allowed);
    ...
    tree_map_widget.on_context_menu_request = [&](const GUI::ContextMenuEvent& event) {
        ...
        if (selected_node_path.is_empty())
            return;
        delete_action->set_enabled(Core::Stream::can_delete_or_move(selected_node_path));
        if (Core::Stream::is_directory(selected_node_path)) {
