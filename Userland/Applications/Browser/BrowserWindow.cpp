    ...
    , m_window_actions(*this)
{
    ...
    set_title("Ladybird"_string.release_value_but_fixme_should_propagate_errors());
...
{
    auto title = String::from_deprecated_string(tab.title()).release_value_but_fixme_should_propagate_errors();
    auto url = tab.url();
    set_title(String::formatted("{} - Ladybird", title.is_empty() ? url.to_string().release_value_but_fixme_should_propagate_errors() : title).release_value_but_fixme_should_propagate_errors());
}
