...
WindowActions::WindowActions(GUI::Window& window)
{
    ...
    m_create_new_tab_action = GUI::Action::create(
        "&New Tab"_string.release_value_but_fixme_should_propagate_errors(), { Mod_Ctrl, Key_T }, g_icon_bag.new_tab, [this](auto&) {
    ...
    m_create_new_window_action = GUI::Action::create(
        "&New Window"_string.release_value_but_fixme_should_propagate_errors(), { Mod_Ctrl, Key_N }, g_icon_bag.new_window, [this](auto&) {
    ...
    m_next_tab_action = GUI::Action::create(
        "&Next Tab"_string.release_value_but_fixme_should_propagate_errors(), { Mod_Ctrl, Key_PageDown }, [this](auto&) {
            if (on_next_tab)
                on_next_tab();
        },
        &window);
    ...
    m_previous_tab_action = GUI::Action::create(
        "&Previous Tab"_string.release_value_but_fixme_should_propagate_errors(), { Mod_Ctrl, Key_PageUp }, [this](auto&) {
            if (on_previous_tab)
                on_previous_tab();
        },
        &window);
    ...
    m_tab_actions.append(GUI::Action::create(
        "Last tab"_string.release_value_but_fixme_should_propagate_errors(), { Mod_Ctrl, Key_9 }, [this](auto&) {
            if (on_tabs[8])
                on_tabs[8]();
        },
        &window));
    ...
    m_about_action = GUI::CommonActions::make_about_action("Ladybird"_string.release_value_but_fixme_should_propagate_errors(), GUI::Icon::default_icon("app-browser"sv), &window);

    m_show_bookmarks_bar_action = GUI::Action::create_checkable(
        "&Bookmarks Bar"_string.release_value_but_fixme_should_propagate_errors(), { Mod_Ctrl, Key_B },
        [this](auto& action) {
    ...
    m_vertical_tabs_action = GUI::Action::create_checkable(
        "&Vertical Tabs"_string.release_value_but_fixme_should_propagate_errors(), { Mod_Ctrl, Key_Comma },
        [this](auto& action) {
            if (on_vertical_tabs)
