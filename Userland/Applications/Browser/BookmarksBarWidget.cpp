...
BookmarksBarWidget::BookmarksBarWidget(DeprecatedString const& bookmarks_file, bool enabled)
{
    ...
    auto default_action = GUI::Action::create(
        "&Open"_string, g_icon_bag.go_to, [this](auto&) {
    ...
    m_context_menu->add_action(GUI::Action::create(
        "Open in New &Tab"_string, g_icon_bag.new_tab, [this](auto&) {
            if (on_bookmark_click)
                on_bookmark_click(m_context_menu_url, Open::InNewTab);
        },
        this));
    m_context_menu->add_action(GUI::Action::create(
        "Open in New Window"_string, g_icon_bag.new_window, [this](auto&) {
            if (on_bookmark_click) {
    ...
    m_context_menu->add_action(GUI::Action::create(
        "&Edit..."_string, g_icon_bag.rename, [this](auto&) {
