    ...
    : m_workbook(make<Workbook>(move(sheets), parent_window))
{
    ...
    m_tab_context_menu->add_action(GUI::Action::create("Add New Sheet..."_string.release_value_but_fixme_should_propagate_errors(), Gfx::Bitmap::load_from_file("/res/icons/16x16/new-tab.png"sv).release_value_but_fixme_should_propagate_errors(), [this](auto&) {
        String name;
    ...
    m_new_action = GUI::Action::create("Add New Sheet"_string.release_value_but_fixme_should_propagate_errors(), Gfx::Bitmap::load_from_file("/res/icons/16x16/new-tab.png"sv).release_value_but_fixme_should_propagate_errors(), [&](auto&) {
        add_sheet();
    });
    ...
    m_import_action = GUI::Action::create("Import Sheets..."_string.release_value_but_fixme_should_propagate_errors(), [&](auto&) {
        FileSystemAccessClient::OpenFileOptions options {
            .allowed_file_types = Vector {
                { "Spreadsheets", { { "sheets", "csv" } } },
                GUI::FileTypeFilter::all_files(),
    ...
    m_change_background_color_action = GUI::Action::create(
        "&Change Background Color"_string.release_value_but_fixme_should_propagate_errors(), { Mod_Ctrl, Key_B }, Gfx::Bitmap::load_from_file("/res/icons/pixelpaint/bucket.png"sv).release_value_but_fixme_should_propagate_errors(), [&](auto&) {
            change_cell_static_color_format(Spreadsheet::FormatType::Background);
        },
        window());

    m_change_foreground_color_action = GUI::Action::create(
        "&Change Foreground Color"_string.release_value_but_fixme_should_propagate_errors(), { Mod_Ctrl, Key_T }, Gfx::Bitmap::load_from_file("/res/icons/16x16/text-color.png"sv).release_value_but_fixme_should_propagate_errors(), [&](auto&) {
    ...
    m_functions_help_action = GUI::Action::create(
        "&Functions Help"_string.release_value_but_fixme_should_propagate_errors(), Gfx::Bitmap::load_from_file("/res/icons/16x16/app-help.png"sv).release_value_but_fixme_should_propagate_errors(), [&](auto&) {
    ...
    m_about_action = GUI::CommonActions::make_about_action("Spreadsheet"_string.release_value_but_fixme_should_propagate_errors(), GUI::Icon::default_icon("app-spreadsheet"sv), &parent_window);
