...
{
    menu.add_action(GUI::Action::create("&Add Track", { Mod_Ctrl, Key_T }, TRY(Media::ImageDecoder::load_from_file("/res/icons/16x16/plus.png"sv)), [&](auto&) {
        m_player_widget->add_track();
    }));

    menu.add_action(GUI::Action::create("&Next Track", { Mod_Ctrl, Key_N }, TRY(Media::ImageDecoder::load_from_file("/res/icons/16x16/go-last.png"sv)), [&](auto&) {
