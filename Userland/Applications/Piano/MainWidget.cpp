...
{
    menu.add_action(GUI::Action::create("&Add Track", { Mod_Ctrl, Key_T }, Gfx::Bitmap::try_request_resource("plus").release_value_but_fixme_should_propagate_errors(), [&](auto&) {
        m_player_widget->add_track();
    }));

    menu.add_action(GUI::Action::create("&Next Track", { Mod_Ctrl, Key_N }, Gfx::Bitmap::try_request_resource("go-last").release_value_but_fixme_should_propagate_errors(), [&](auto&) {
