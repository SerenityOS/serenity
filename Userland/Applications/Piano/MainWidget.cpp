...
ErrorOr<NonnullRefPtr<MainWidget>> MainWidget::create(TrackManager& manager, AudioPlayerLoop& loop)
{
    auto widget = TRY(adopt_nonnull_ref_or_enomem(new (nothrow) MainWidget(manager, loop)));
    widget->m_player_widget = TRY(PlayerWidget::create(widget->m_track_manager, widget->m_audio_loop));
    widget->initialize();
    return widget;
}
...
void MainWidget::initialize()
{
    ...
    m_roll_widget = m_tab_widget->add_tab<RollWidget>("Piano Roll", m_track_manager);

    m_roll_widget->set_fixed_height(300);

    m_tab_widget->add_tab<SamplerWidget>("Sampler", m_track_manager);
    ...
    m_knobs_widget = m_keys_and_knobs_container->add<KnobsWidget>(m_track_manager, *this);
...
{
    menu.add_action(GUI::Action::create("&Add Track", { Mod_Ctrl, Key_T }, TRY(Gfx::Bitmap::try_load_from_file("/res/icons/16x16/plus.png"sv)), [&](auto&) {
        m_player_widget->add_track();
    }));

    menu.add_action(GUI::Action::create("&Next Track", { Mod_Ctrl, Key_N }, TRY(Gfx::Bitmap::try_load_from_file("/res/icons/16x16/go-last.png"sv)), [&](auto&) {
