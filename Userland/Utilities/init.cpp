...
static ErrorOr<void> populate_device_node_with_symlink(DeviceNodeType device_node_type, StringView path, mode_t mode, Kernel::MajorAllocation::CharacterDeviceFamily major, unsigned minor)
{
    ...
    if (device_node_type == DeviceNodeType::Block)
        TRY(Core::System::create_block_device(path, mode, to_underlying(major), minor));
    else
        TRY(Core::System::create_char_device(path, mode, to_underlying(major), minor));
    auto symlink_path = LexicalPath("/tmp/system/devicemap/nodes/")
                            .append(device_node_type == DeviceNodeType::Block ? "block"sv : "char"sv)
                            .append(String::number(to_underlying(major)))
...
static ErrorOr<void> prepare_bare_minimum_devtmpfs_directory_structure()
{
    ...
    TRY(populate_device_node_with_symlink(DeviceNodeType::Character, "/dev/devctl"sv, 0660, Kernel::MajorAllocation::CharacterDeviceFamily::DeviceControl, 10));
    TRY(populate_device_node_with_symlink(DeviceNodeType::Character, "/dev/zero"sv, 0666, Kernel::MajorAllocation::CharacterDeviceFamily::Generic, 5));
    TRY(populate_device_node_with_symlink(DeviceNodeType::Character, "/dev/mem"sv, 0600, Kernel::MajorAllocation::CharacterDeviceFamily::Generic, 1));
    TRY(populate_device_node_with_symlink(DeviceNodeType::Character, "/dev/null"sv, 0666, Kernel::MajorAllocation::CharacterDeviceFamily::Generic, 3));
    TRY(populate_device_node_with_symlink(DeviceNodeType::Character, "/dev/full"sv, 0666, Kernel::MajorAllocation::CharacterDeviceFamily::Generic, 7));
    TRY(populate_device_node_with_symlink(DeviceNodeType::Character, "/dev/random"sv, 0666, Kernel::MajorAllocation::CharacterDeviceFamily::Generic, 8));
    TRY(populate_device_node_with_symlink(DeviceNodeType::Character, "/dev/console"sv, 0666, Kernel::MajorAllocation::CharacterDeviceFamily::Console, 1));
    TRY(populate_device_node_with_symlink(DeviceNodeType::Character, "/dev/ptmx"sv, 0666, Kernel::MajorAllocation::CharacterDeviceFamily::Console, 2));
    TRY(populate_device_node_with_symlink(DeviceNodeType::Character, "/dev/tty"sv, 0666, Kernel::MajorAllocation::CharacterDeviceFamily::Console, 0));
    TRY(populate_device_node_with_symlink(DeviceNodeType::Character, "/dev/fuse"sv, 0666, Kernel::MajorAllocation::CharacterDeviceFamily::Generic, 229));
#ifdef ENABLE_KERNEL_COVERAGE_COLLECTION
    TRY(populate_device_node_with_symlink(DeviceNodeType::Block, "/dev/kcov"sv, 0666, Kernel::MajorAllocation::BlockDeviceFamily::KCOV, 0));
#endif
    umask(old_mask);
