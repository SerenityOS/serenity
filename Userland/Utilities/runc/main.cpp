...
{
    return TRY(Core::System::create_unshare(Kernel::UnshareType::VFSRootContext, 0));
}
...
static ErrorOr<void> create_custom_vfs_root_context_layout(JsonArray const& layout_creation_sequence)
{
    ...
    TRY(Core::System::attach_unshare(Kernel::UnshareType::VFSRootContext, vfs_root_context_index));

    // NOTE: Finally, apply changes to current working directory.
    (void)chdir("/");
    return {};
}
...
{
    auto scoped_process_list_index = TRY(Core::System::create_unshare(Kernel::UnshareType::ScopedProcessList, 0));
    TRY(Core::System::attach_unshare(Kernel::UnshareType::ScopedProcessList, scoped_process_list_index));
    return {};
}

static ErrorOr<void> create_and_attach_hostname_context(StringView hostname)
{
    auto hostname_context_index = TRY(Core::System::create_unshare(Kernel::UnshareType::HostnameContext, 0));
    TRY(Core::System::attach_unshare(Kernel::UnshareType::HostnameContext, hostname_context_index));
...
static ErrorOr<void> enter_jail_mode()
{
...
static ErrorOr<void> deploy_container_based_on_config_file(NonnullOwnPtr<Core::File> file)
{
    ...
    TRY(Core::System::pledge_remove_capabilities("unshare"sv));

    if (enforce_jail)
        TRY(enter_jail_mode());
    TRY(Core::System::pledge_remove_capabilities("proc"sv));
...
ErrorOr<int> serenity_main(Main::Arguments arguments)
{
    ...
    if (!config_file.is_null() && !config_file.is_empty()) {
        ...
        auto file = TRY(Core::File::open(config_file, Core::File::OpenMode::Read));
        TRY(deploy_container_based_on_config_file(move(file)));
        return 0;
    }
