...
static void aes_ctr_test_encrypt()
{
    auto test_it = [](auto key, auto ivec, auto in, auto out_expected) {
        ...
        ByteBuffer out_actual = MUST(ByteBuffer::create_zeroed(in.size()));
...
{
    {
        ...
        Bytes out;
        auto tag = MUST(ByteBuffer::create_uninitialized(16));
    ...
    }
    {
        ...
        auto tag = MUST(ByteBuffer::create_uninitialized(16));
        auto out = MUST(ByteBuffer::create_uninitialized(16));
    ...
    }
    {
        ...
        auto tag = MUST(ByteBuffer::create_uninitialized(16));
        auto out = MUST(ByteBuffer::create_uninitialized(64));
        auto out_bytes = out.bytes();
        cipher.encrypt(
...
static void tls_test_client_hello()
{
    ...
    tls->on_tls_ready_to_read = [&](TLS::TLSv12& tls) {
        ...
        if (!data.has_value()) {
            ...
            if (contents.append(data.value().data(), data.value().size()).is_error()) {
