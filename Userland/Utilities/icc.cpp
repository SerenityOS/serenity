...
static ErrorOr<void> convert_color(StringView color, Gfx::ICC::Profile const& profile)
{
    auto split = color.split_view(',');
    if (split.size() != 3 && split.size() != 4)
        return Error::from_string_literal("expected 3 or 4 channels for color convertion");
    ...
    if (channels.size() == 3)
        outln("({}, {}, {}) in sRGB is {}", channels[0], channels[1], channels[2], out_color);
    else
        outln("({}, {}, {}, {}) in sRGB is {}", channels[0], channels[1], channels[2], channels[3], out_color);
    return {};
}
...
ErrorOr<int> serenity_main(Main::Arguments arguments)
{
    ...
    if (!color_to_convert.is_empty()) {
        TRY(convert_color(color_to_convert, *profile));
        return 0;
    }
