...
#include <Userland/Shell/Escape.h>
#include <ctype.h>
#include <dirent.h>
#include <errno.h>
#include <fcntl.h>
...
static int print_escaped(StringView name)
{
    auto const escaped_string = [name]() -> String {
        ...
        for (auto const c : name) {
            if (Shell::special_character_escape_mode(c) != Shell::SpecialCharacterEscapeMode::Untouched && c != '\'' && c != '"')
                general_escape = true;
            if (c == '\'')
        ...
        if (contains_double_quotes && !contains_single_quotes)
            return Shell::escape_token_for_single_quotes(name);
        if (general_escape || contains_single_quotes)
