...
auto string_offset_format = StringOffsetFormat::None;
...
static void print_characters(Vector<u8> const& characters, size_t string_offset_position)
{
...
static ErrorOr<void> process_strings_in_file(StringView path)
{
    ...
    while (!file->is_eof()) {
        ...
        while (!buffer_span.is_empty()) {
            ...
            if (output_characters.size() >= minimum_string_length && should_print_characters(output_characters)) {
                print_characters(output_characters, string_offset_position);
            }
...
ErrorOr<int> serenity_main(Main::Arguments arguments)
{
    ...
    for (auto const& path : paths)
        TRY(process_strings_in_file(path));
