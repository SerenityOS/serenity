...
ErrorOr<int> serenity_main(Main::Arguments arguments)
{
    ...
    args_parser.add_option(screenshot_output_path, "Path to save a screenshot (defaults to output.png)", "shot-out", 'o', "screenshot-output-path");
    ...
    auto timer = Core::Timer::create_single_shot(
        ...
        [ page_client = move(page_client), screenshot_output_path = move(screenshot_output_path)] {
            auto screenshot_path = String(screenshot_output_path);

            dbgln("Saving to {}", screenshot_path);

            if (Core::File::exists(screenshot_path))
                [[maybe_unused]]
                auto ignored = Core::File::remove(screenshot_path, Core::File::RecursionMode::Disallowed, true);
            auto output_file = MUST(Core::Stream::File::open(screenshot_path, Core::Stream::OpenMode::Write));
