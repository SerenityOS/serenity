...
{
    auto slice_result = buf.slice(2, buf.size() - 2);
    if (slice_result.is_error())
        return slice_result.release_error();

    auto hex_string = ByteString(slice_result.value().bytes());
...
ErrorOr<int> serenity_main(Main::Arguments arguments)
{
    ...
    while (di.has_next()) {
        ...
        auto vendor_id = TRY(read_hex_string_from_bytebuffer(vendor_id_contents.value()));
        ...
        auto device_id = TRY(read_hex_string_from_bytebuffer(device_id_contents.value()));
        ...
        auto revision_id = TRY(read_hex_string_from_bytebuffer(revision_id_contents.value()));
        ...
        auto class_id = TRY(read_hex_string_from_bytebuffer(class_id_contents.value()));
        ...
        auto subclass_id = TRY(read_hex_string_from_bytebuffer(subclass_id_contents.value()));
