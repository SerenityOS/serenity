...
private:
    ...
    JS_DECLARE_NATIVE_FUNCTION(read_binary_wasm_file);
    JS_DECLARE_NATIVE_FUNCTION(parse_webassembly_module);
    JS_DECLARE_NATIVE_FUNCTION(compare_typed_arrays);
};
...
void TestRunnerGlobalObject::initialize_global_object()
{
    ...
    static FlyString parse_webassembly_module_property_name { "parseWebAssemblyModule" };
    static FlyString read_binary_wasm_file_property_name { "readBinaryWasmFile" };
    static FlyString compare_typed_arrays_property_name { "compareTypedArrays" };
    ...
    define_native_function(parse_webassembly_module_property_name, parse_webassembly_module);
    define_native_function(read_binary_wasm_file_property_name, read_binary_wasm_file);
    define_native_function(compare_typed_arrays_property_name, compare_typed_arrays);
}
...
JS_DEFINE_NATIVE_FUNCTION(TestRunnerGlobalObject::read_binary_wasm_file)
{
...
JS_DEFINE_NATIVE_FUNCTION(TestRunnerGlobalObject::parse_webassembly_module)
{
    ...
    auto& array = dynamic_cast<JS::Uint8Array&>(*object);
...
JS_DEFINE_NATIVE_FUNCTION(TestRunnerGlobalObject::compare_typed_arrays)
{
    ...
    auto& lhs_array = dynamic_cast<JS::TypedArrayBase&>(*lhs);
    auto rhs = vm.argument(1).to_object(global_object);
    if (!is<JS::TypedArrayBase>(rhs)) {
        vm.throw_exception<JS::TypeError>(global_object, "Expected a TypedArray");
        return {};
    }
    auto& rhs_array = dynamic_cast<JS::TypedArrayBase&>(*rhs);
...
{
    dbgln("test-wasm: SIGABRT received, cleaning up.");
    cleanup_and_exit();
}
...
static void iterate_directory_recursively(const String& directory_path, Callback callback)
{
    ...
    while (directory_iterator.has_next()) {
        ...
        if (Core::File::is_directory(file_path) && !file_path.contains("/Fixtures")) {
...
int main(int argc, char** argv)
{
...
#ifdef __serenity__
        test_root = "/home/anon/wasm-tests";
#else
        char* serenity_source_dir = getenv("SERENITY_SOURCE_DIR");
        if (!serenity_source_dir) {
            warnln("No test root given, test-wasm requires the SERENITY_SOURCE_DIR environment variable to be set");
            return 1;
        }
        test_root = String::formatted("{}/Userland/Libraries/LibWasm/Tests", serenity_source_dir);
    ...
    if (common_path.is_empty())
        common_path = String::formatted("{}/test-common.js", test_root);
    ...
    TestRunner(test_root, common_path, print_times, print_progress).run();
