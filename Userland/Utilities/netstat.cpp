...
int main(int argc, char** argv)
{
    ...
    HashMap<pid_t, Process> programs;
    ...
    if (flag_program) {
        ...
        for (auto& proc : processes.value()) {
            programs.set(proc.pid, { proc.name, proc.uid });
        }
    }
    ...
    auto get_formatted_program = [&](pid_t pid) {
        ...
        if (current_uid != 0 && current_uid != program.value().uid) {
            return String("-");
        }

        return String::formatted("{}/{}", pid, program.value().name);
    };

    if (!has_protocol_flag || flag_tcp || flag_udp) {
        if (flag_program && current_uid != 0) {
            outln("(Some processes may not be identified, non-owned process info will not be shown)");
        }
    ...
    if (!has_protocol_flag || flag_tcp) {
        ...
        for (auto& value : sorted_regions) {
            ...
            auto origin_pid = if_object.get("origin_pid").to_u32();
