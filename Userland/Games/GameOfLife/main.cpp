...
ErrorOr<int> serenity_main(Main::Arguments arguments)
{
    ...
    auto paused_icon = Gfx::Bitmap::try_request_resource("pause").release_value_but_fixme_should_propagate_errors();
    auto play_icon = Gfx::Bitmap::try_request_resource("play").release_value_but_fixme_should_propagate_errors();
    ...
    auto run_one_generation_action = GUI::Action::create("Run &Next Generation", { Mod_Ctrl, Key_Equal }, Gfx::Bitmap::try_request_resource("go-forward").release_value_but_fixme_should_propagate_errors(), [&](const GUI::Action&) {
        statusbar.set_text(click_tip);
        board_widget->run_generation();
    });
    (void)TRY(main_toolbar.try_add_action(run_one_generation_action));

    auto clear_board_action = GUI::Action::create("&Clear board", { Mod_Ctrl, Key_N }, Gfx::Bitmap::try_request_resource("delete").release_value_but_fixme_should_propagate_errors(), [&](auto&) {
        statusbar.set_text(click_tip);
    ...
    auto randomize_cells_action = GUI::Action::create("&Randomize board", { Mod_Ctrl, Key_R }, Gfx::Bitmap::try_request_resource("reload").release_value_but_fixme_should_propagate_errors(), [&](auto&) {
        statusbar.set_text(click_tip);
        board_widget->randomize_cells();
        board_widget->update();
    });
    ...
    auto rotate_pattern_action = GUI::Action::create("&Rotate pattern", { 0, Key_R }, Gfx::Bitmap::try_request_resource("redo").release_value_but_fixme_should_propagate_errors(), [&](auto&) {
