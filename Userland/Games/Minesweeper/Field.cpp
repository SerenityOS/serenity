...
ErrorOr<NonnullRefPtr<Field>> Field::create(GUI::Label& flag_label, GUI::Label& time_label, GUI::Button& face_button)
{
    ...
    field->m_mine_bitmap = TRY(Media::ImageDecoder::load_from_file("/res/graphics/minesweeper/mine.png"sv));
    field->m_flag_bitmap = TRY(Media::ImageDecoder::load_from_file("/res/graphics/minesweeper/flag.png"sv));
    field->m_badflag_bitmap = TRY(Media::ImageDecoder::load_from_file("/res/graphics/minesweeper/badflag.png"sv));
    field->m_consider_bitmap = TRY(Media::ImageDecoder::load_from_file("/res/graphics/minesweeper/consider.png"sv));
    field->m_default_face_bitmap = TRY(Media::ImageDecoder::load_from_file("/res/graphics/minesweeper/face-default.png"sv));
    field->m_good_face_bitmap = TRY(Media::ImageDecoder::load_from_file("/res/graphics/minesweeper/face-good.png"sv));
    field->m_bad_face_bitmap = TRY(Media::ImageDecoder::load_from_file("/res/graphics/minesweeper/face-bad.png"sv));
    for (int i = 0; i < 8; ++i)
        field->m_number_bitmap[i] = TRY(Media::ImageDecoder::load_from_file(DeprecatedString::formatted("/res/graphics/minesweeper/{}.png", i + 1)));
    field->initialize();
    return field;
}
