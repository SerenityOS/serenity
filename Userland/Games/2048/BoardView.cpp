...
void BoardView::paint_event(GUI::PaintEvent& event)
{
    ...
    if (slide_animation_frame < animation_duration) {
        ...
        for (auto& sliding_tile : m_board->sliding_tiles()) {
            ...
            painter.draw_text(rect, String::number(sliding_tile.value_from).release_value_but_fixme_should_propagate_errors(), Gfx::TextAlignment::Center, text_color_for_cell(sliding_tile.value_from));
        }
    } else {
        for (size_t column = 0; column < columns(); ++column) {
            for (size_t row = 0; row < rows(); ++row) {
