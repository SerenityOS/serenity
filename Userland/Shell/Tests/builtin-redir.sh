...
pushd /tmp/shell-test
    ...
    if not not internal:string_equal "" "$(cat timeerr)" { fail "'time' stderr output not redirected correctly" }
    if not test -e timeout { fail "'time' stdout output not redirected correctly" }

    time ls 2> /dev/null | head > timeout
    if not not internal:string_equal "" "$(cat timeout)" { fail "'time' stdout not piped correctly" }
