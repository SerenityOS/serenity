...
this is a test
test
if not internal:string_equal "$(cat sh.doc.test)" "this is a test" {
...
contents for test2
test2
if not internal:string_equal "$(cat sh.doc.test)" "contents for test contents for test2" {
    fail "Could not use two heredocs"
}

# Why would you do this you crazy person?
if not internal:string_equal "$(echo <<~text)" "test" {
                test
                text
...
if not internal:string_equal "$(echo <<~test1)" "$(echo <<~test2)" { fail "The parser forgot about heredocs after a block, oops" }
test
test1
test
test2
