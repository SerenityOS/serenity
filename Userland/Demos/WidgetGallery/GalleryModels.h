    ...
    virtual void invalidate() override
    {
        ...
        while (big_iterator.has_next()) {
            ...
            if (!path.contains("filetype-"sv) && !path.contains("app-"sv))
                continue;
            IconSet icon_set;
            icon_set.big_icon = Media::ImageDecoder::load_from_file(path).release_value_but_fixme_should_propagate_errors();
        ...
        while (little_iterator.has_next()) {
            ...
            if (!path.contains("filetype-"sv) && !path.contains("app-"sv))
                continue;
            IconSet icon_set;
            icon_set.little_icon = Media::ImageDecoder::load_from_file(path).release_value_but_fixme_should_propagate_errors();
            icon_set.name = LexicalPath::basename(path);
