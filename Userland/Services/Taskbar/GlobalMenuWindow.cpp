...
GlobalMenuWindow::GlobalMenuWindow()
{
    ...
    set_rect({ rect.x(), rect.y(), rect.width(), global_menu_height() + 1 });
    ...
    m_global_menu_area_container->set_frame_shape(Gfx::FrameShape::NoFrame);
    m_global_menu_area_container->set_frame_shadow(Gfx::FrameShadow::Plain);
    m_global_menu_area_container->set_frame_thickness(0);

    m_enabled = Config::read_bool("Taskbar"sv, "GlobalMenu"sv, "Enabled"sv, false);
    update_global_menu_enabled();
}

void GlobalMenuWindow::config_bool_did_change(DeprecatedString const& domain, DeprecatedString const& group, DeprecatedString const& key, bool value)
{
    if (domain == "Taskbar" && group == "GlobalMenu") {
        if (key == "Enabled") {
            m_enabled = value;
        }
