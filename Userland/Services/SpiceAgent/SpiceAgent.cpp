...
void SpiceAgent::on_message_received()
{
    ...
    auto buffer = MUST(ByteBuffer::create_uninitialized(header.size)); // FIXME: Handle possible OOM situation.
    ...
    case (u32)MessageType::Clipboard: {
        ...
        auto data_buffer = MUST(ByteBuffer::create_uninitialized(message->size - sizeof(u32))); // FIXME: Handle possible OOM situation.
...
ByteBuffer SpiceAgent::AnnounceCapabilities::make_buffer(bool request, const Vector<Capability>& capabilities)
{
    ...
    auto buffer = MUST(ByteBuffer::create_uninitialized(required_size)); // FIXME: Handle possible OOM situation.
    u8* data = buffer.data();
