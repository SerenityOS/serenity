...
{
    lseek(m_display_connector_fd, 0, SEEK_SET);
    write(m_display_connector_fd, scanline(0, 0), m_virtual_rect.height() * m_pitch);
    if (m_can_set_buffer) {
        if (lseek(m_display_connector_fd, m_virtual_rect.height() * m_pitch, SEEK_SET) < 0) {
            dbgln("{}", errno);
            VERIFY_NOT_REACHED();
        }

        if (write(m_display_connector_fd, scanline(0, 0), m_virtual_rect.height() * m_pitch) < 0)
            VERIFY_NOT_REACHED();
    }
}
...
void Screen::flush_display(int buffer_index)
{
    ...
    fb_flush_everything(m_display_connector_fd);
