...
RefPtr<Cursor> Cursor::create(StringView const& filename, StringView const& default_filename)
{
...
bool Cursor::load(StringView const& filename, StringView const& default_filename)
{
    bool did_load_any = false;

    auto load_bitmap = [&](StringView const& path, int scale_factor) {
