...
{
    s_the = this;
    m_ladyball = Gfx::Bitmap::try_load_from_file("/res/icons/16x16/ladyball.png"sv).release_value_but_fixme_should_propagate_errors();
}
GlobalMenu::~GlobalMenu()
{
}
...
void GlobalMenu::set_enabled(bool enabled)
{
    ...
    if (enabled) {
        ...
        m_window->set_automatic_cursor_tracking_enabled(true);
...
int GlobalMenu::paint_title(bool paint)
{
    ...
    Gfx::IntRect text_rect = { icon_width, 0, font.width(text) + 2, m_window->height() };
    if (paint)
