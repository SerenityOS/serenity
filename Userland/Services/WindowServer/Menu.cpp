...
Window& Menu::ensure_menu_window()
{
    ...
    int window_height_available = Screen::the().virtual_height() - frame_thickness() * 2;
...
void Menu::do_popup(const Gfx::IntPoint& position, bool make_input, bool as_submenu)
{
    ...
    if (adjusted_pos.x() + window.width() >= Screen::the().virtual_width() - margin) {
        adjusted_pos = adjusted_pos.translated(-window.width(), 0);
    } else {
        adjusted_pos.set_x(adjusted_pos.x() + 1);
    }
    if (adjusted_pos.y() + window.height() >= Screen::the().virtual_height() - margin) {
