    ...
    , m_ptm_notifier(Core::Notifier::construct(ptm_fd, Core::Notifier::Read))
{
    ...
    m_parser.on_command = [this](Command const& command) { handle_command(command); };
    m_parser.on_data = [this](StringView const& data) { handle_data(data); };
...
void Client::handle_data(StringView const& data)
{
    write(m_ptm_fd, data.characters_without_null_termination(), data.length());
}

void Client::handle_command(Command const& command)
{
