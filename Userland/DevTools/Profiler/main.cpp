...
ErrorOr<int> serenity_main(Main::Arguments arguments)
{
    ...
    auto disassembly_action = GUI::Action::create_checkable("Show &Disassembly", { Mod_Ctrl, Key_D }, Gfx::Bitmap::try_request_resource("x86").release_value_but_fixme_should_propagate_errors(), [&](auto& action) {
        disassembly_view->set_visible(action.is_checked());
        update_disassembly_model();
    });

    auto source_action = GUI::Action::create_checkable("Show &Source", { Mod_Ctrl, Key_S }, Gfx::Bitmap::try_request_resource("x86").release_value_but_fixme_should_propagate_errors(), [&](auto& action) {
...
static bool prompt_to_stop_profiling(pid_t pid, String const& process_name)
{
    ...
    window->set_icon(Gfx::Bitmap::try_request_resource("app-profiler").release_value_but_fixme_should_propagate_errors());
...
{
    if (!pid) {
        auto process_chooser = GUI::ProcessChooser::construct("Profiler", "Profile", Gfx::Bitmap::try_request_resource("app-profiler").release_value_but_fixme_should_propagate_errors());
