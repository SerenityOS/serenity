    ...
    auto delete_action = GUI::CommonActions::make_delete_action([this](const GUI::Action&) {
        ...
        for (auto& file : files) {
            ...
            if (auto result = Core::File::remove(file, Core::File::RecursionMode::Allowed, false); result.is_error()) {
                ...
                if (is_directory) {
                    GUI::MessageBox::show(window(),
                        DeprecatedString::formatted("Removing directory {} from the project failed: {}", error.file, error),
                        "Removal failed"sv,
                        GUI::MessageBox::Type::Error);
                } else {
                    GUI::MessageBox::show(window(),
                        DeprecatedString::formatted("Removing file {} from the project failed: {}", error.file, error),
                        "Removal failed"sv,
                        GUI::MessageBox::Type::Error);
                }
            }
