...
void ServerConnection::declarations_in_document(String const& filename, const Vector<GUI::AutocompleteProvider::Declaration>& declarations)
{
...
void ServerConnectionInstances::set_instance_for_language(String const& language_name, NonnullOwnPtr<ServerConnectionWrapper>&& connection_wrapper)
{
    s_instance_for_language.set(language_name, move(connection_wrapper));
}

void ServerConnectionInstances::remove_instance_for_language(String const& language_name)
{
    s_instance_for_language.remove(language_name);
}

ServerConnectionWrapper* ServerConnectionInstances::get_instance_wrapper(String const& language_name)
{
...
ServerConnectionWrapper::ServerConnectionWrapper(String const& language_name, Function<NonnullRefPtr<ServerConnection>()> connection_creator)
...
void ServerConnectionWrapper::try_respawn_connection()
{
    ...
    project().for_each_text_file([this](ProjectFile const& file) {
