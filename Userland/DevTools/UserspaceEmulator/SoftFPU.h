...
private:
    ...
    void fpu_dump_env()
    {
        ...
        for (u8 i = 0; i < 8; ++i) {
            reportln("\t{} ({}): {} ({})", i, (u8)((m_fpu_stack_top + i) % 8), m_st[(m_fpu_stack_top + i) % 8], fpu_is_set(i) ? "set" : "free");
        }
    }
    ...
    // FIXME: Maybe change the structure of this
    // do not pack, we need pointers from this
    union {
        ...
        struct
        {
            ...
            // 10 bytes big, as they are supposed to be
    ...
    void MOVQ_mm1_m64_mm2(const X86::Instruction&);
