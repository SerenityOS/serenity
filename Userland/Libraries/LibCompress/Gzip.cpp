...
ErrorOr<NonnullOwnPtr<GzipDecompressor::Member>> GzipDecompressor::Member::construct(BlockHeader header, Core::Stream& stream)
{
    auto deflate_stream = TRY(DeflateDecompressor::construct(MaybeOwned<Core::Stream>(stream)));
...
GzipDecompressor::GzipDecompressor(NonnullOwnPtr<Core::Stream> stream)
...
GzipCompressor::GzipCompressor(MaybeOwned<Core::Stream> stream)
...
ErrorOr<ByteBuffer> GzipCompressor::compress_all(ReadonlyBytes bytes)
{
    ...
    GzipCompressor gzip_stream { MaybeOwned<Core::Stream>(*output_stream) };
