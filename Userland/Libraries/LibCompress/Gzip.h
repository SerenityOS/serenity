...
class GzipDecompressor final : public Core::Stream {
public:
    GzipDecompressor(NonnullOwnPtr<Core::Stream>);
    ~GzipDecompressor();
...
private:
    class Member {
    public:
        static ErrorOr<NonnullOwnPtr<Member>> construct(BlockHeader header, Core::Stream&);
    ...
    NonnullOwnPtr<Core::Stream> m_input_stream;
...
class GzipCompressor final : public Core::Stream {
public:
    GzipCompressor(MaybeOwned<Core::Stream>);
