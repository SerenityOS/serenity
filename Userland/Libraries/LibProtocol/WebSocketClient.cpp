...
RefPtr<WebSocket> WebSocketClient::connect(URL url, const String& origin, const Vector<String>& protocols, const Vector<String>& extensions, const HashMap<String, String>& request_headers)
{
...
void WebSocketClient::handle(const Messages::WebSocketClient::Connected& message)
{
    ...
    if ((connection = m_connections.get(message.connection_id()).value_or(nullptr))) {
        connection->did_open({});
    }
}
...
void WebSocketClient::handle(const Messages::WebSocketClient::Received& message)
{
    RefPtr<WebSocket> connection;
    if ((connection = m_connections.get(message.connection_id()).value_or(nullptr))) {
        connection->did_receive({}, message.data(), message.is_text());
    }
}
...
void WebSocketClient::handle(const Messages::WebSocketClient::Errored& message)
{
    RefPtr<WebSocket> connection;
    if ((connection = m_connections.get(message.connection_id()).value_or(nullptr))) {
        connection->did_error({}, message.message());
    }
}
...
void WebSocketClient::handle(const Messages::WebSocketClient::Closed& message)
{
    RefPtr<WebSocket> connection;
    if ((connection = m_connections.get(message.connection_id()).value_or(nullptr))) {
        connection->did_close({}, message.code(), message.reason(), message.clean());
    }
}
