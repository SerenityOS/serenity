...
public:
    ...
    static NonnullRefPtr<WebSocket> create_from_id(Badge<WebSocketClient>, WebSocketClient& client, i32 m_connection_id)
    {
        return adopt_ref(*new WebSocket(client, m_connection_id));
    }
    ...
    void send(ByteBuffer, bool is_text);
    void send(StringView);
...
private:
    explicit WebSocket(WebSocketClient&, i32 m_connection_id);
