...
NEVER_INLINE void Image::sort_symbols() const
{
    ...
    bool const is_aarch64_or_riscv64 = header().e_machine == EM_AARCH64 || header().e_machine == EM_RISCV;
    for_each_symbol([this, is_aarch64_or_riscv64](auto const& symbol) {
        // The AArch64 and RISC-V ABI mark the boundaries of literal pools in a function with $x/$d.
        ...
        if (is_aarch64_or_riscv64 && (symbol.name().starts_with("$x"sv) || symbol.name().starts_with("$d"sv)))
            return;
