...
public:
    ...
    class Section {
    public:
        Section(DynamicObject const& dynamic, unsigned section_offset, unsigned section_size_bytes, unsigned entry_size, StringView const& name)
            : m_dynamic(dynamic)
    ...
    class HashSymbol {
    public:
        HashSymbol(StringView const& name)
            : m_name(name)
        {
        }
    ...
    private:
        Optional<Symbol> lookup_sysv_symbol(StringView const& name, u32 hash_value) const;
        Optional<Symbol> lookup_gnu_symbol(StringView const& name, u32 hash) const;
    ...
    Optional<SymbolLookupResult> lookup_symbol(StringView const& name) const;
    ...
    void* symbol_for_name(StringView const& name);
...
inline void DynamicObject::for_each_needed_library(F func) const
{
    for_each_dynamic_entry([func, this](auto entry) {
        ...
        StringView name { (char const*)(m_base_address.offset(m_string_table_offset).offset(offset)).as_ptr() };
        func(name);
    });
}
