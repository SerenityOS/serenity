...
{
    BigEndianInputBitStream bit_input { MaybeOwned<Core::Stream>(*m_stream) };
...
#define FLAC_VERIFY(check, category, msg)                                                                                                     \
    ...
    BigEndianInputBitStream streaminfo_data { MaybeOwned<Core::Stream>(streaminfo_data_memory) };
...
MaybeLoaderError FlacLoaderPlugin::load_picture(FlacRawMetadataBlock& block)
{
    ...
    BigEndianInputBitStream picture_block_bytes { MaybeOwned<Core::Stream>(memory_stream) };
...
{
    FixedMemoryStream memory_stream { block.data.bytes() };
    BigEndianInputBitStream seektable_bytes { MaybeOwned<Core::Stream>(memory_stream) };
...
#define FLAC_VERIFY(check, category, msg)                                                                                                         \
    ...
    BigEndianInputBitStream bit_stream { MaybeOwned<Core::Stream>(*m_stream) };
