...
{
    m_bitstream = LOADER_TRY(try_make<BigEndianInputBitStream>(MaybeOwned<Core::Stream>(*m_stream)));
...
ErrorOr<MP3::MP3Frame, LoaderError> MP3LoaderPlugin::read_frame_data(MP3::Header const& header)
{
    ...
    BigEndianInputBitStream reservoir_stream { MaybeOwned<Core::Stream>(m_bit_reservoir) };
