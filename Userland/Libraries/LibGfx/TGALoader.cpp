...
struct TGAHeader {
    u8 id_length;
    u8 color_map_type;
    enum TGADataType data_type_code;
...
static_assert(sizeof(TGAPixel) == 4);
...
ErrorOr<ImageFrameDescriptor> TGAImageDecoderPlugin::frame(size_t index)
{
    ...
    if (index != 0)
        return Error::from_string_view(String::formatted("BMPImageDecoderPlugin: frame index must be 0 but found {}", index));

    if (color_map > 1)
        return Error::from_string_view(String::formatted("TGAImageDecoderPlugin: Invalid color map type {}", color_map));
    ...
    default:
        ...
        return Error::from_string_view(String::formatted("TGAImageDecoderPlugin: Can only handle 24 and 32 bits per pixel but found {}", bits_per_pixel));
    }
    switch (data_type) {
