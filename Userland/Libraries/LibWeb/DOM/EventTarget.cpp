...
Bindings::CallbackType* EventTarget::get_current_value_of_event_handler(FlyString const& name)
{
    ...
    if (event_handler.value.has<String>()) {
        ...
        // FIXME: We should be parsing it as a FunctionBody instead of a FunctionExpression, however, JS::ECMAScriptFunctionObject doesn't currently take sourceText.
        ...
        // Make sure we have an interpreter execution scope before creating an ECMAScriptFunctionObject, otherwise its realm will be null.
        // FIXME: Remove this hack once VM no longer requires this.
        auto interpreter = JS::Interpreter::create_with_existing_realm(settings_object.realm());
        JS::VM::InterpreterExecutionScope execution_scope(*interpreter);
...
void EventTarget::activate_event_handler(FlyString const& name, HTML::EventHandler& event_handler, IsAttribute is_attribute)
{
    ...
    Bindings::CallbackType callback { JS::make_handle(static_cast<JS::Object*>(callback_function)), verify_cast<HTML::EnvironmentSettingsObject>(*global_object->associated_realm()->custom_data()) };
