...
void PaintableBox::paint_background(PaintContext& context) const
{
    ...
    if (layout_box().is_root_element()) {
        ...
        background_rect = context.device_viewport_rect().to_type<float>();
...
bool PaintableWithLines::handle_mousewheel(Badge<EventHandler>, CSSPixelPoint const&, unsigned, unsigned, int wheel_delta_x, int wheel_delta_y)
{
...
Optional<HitTestResult> PaintableBox::hit_test(CSSPixelPoint const& position, HitTestType type) const
{
    if (!is_visible())
        return {};
...
Optional<HitTestResult> PaintableWithLines::hit_test(CSSPixelPoint const& position, HitTestType type) const
{
    ...
    for (auto& line_box : m_line_boxes) {
        for (auto& fragment : line_box.fragments()) {
            ...
            if (fragment_absolute_rect.top() <= position.y())
                last_good_candidate = HitTestResult { *fragment.layout_node().paintable(), fragment.text_index_at(position.x().value()) };
        }
    }
