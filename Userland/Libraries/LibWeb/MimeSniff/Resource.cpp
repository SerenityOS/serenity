...
{
    // 1. Execute the following steps for each row row in the following table:
    static Array<BytePatternTableRow, 3> pattern_table {
        // The GZIP archive signature.
        BytePatternTableRow { "\x1F\x8B\x08"sv.bytes(), "\xFF\xFF\xFF"sv.bytes(), no_ignored_bytes, "application/x-gzip"sv },

        // The string "PK" followed by ETX EOT, the ZIP archive signature.
        BytePatternTableRow { "\x50\x4B\x03\x04"sv.bytes(), "\xFF\xFF\xFF\xFF"sv.bytes(), no_ignored_bytes, "application/zip"sv },

        // The string "Rar " followed by SUB BEL NUL, the RAR archive signature.
        BytePatternTableRow { "\x52\x61\x72\x20\x1A\x07\x00"sv.bytes(), "\xFF\xFF\xFF\xFF\xFF\xFF\xFF"sv.bytes(), no_ignored_bytes, "application/x-rar-compressed"sv },
    };
