...
ErrorOr<NonnullOwnPtr<CalculationNode>> Parser::simplify_a_calculation_tree(NonnullOwnPtr<CalculationNode> root)
{
    ...
    if (root->type() == CalculationNode::Type::Numeric) {
        ...
        // NOTE: Our Numeric nodes are never <calc-constant>, so we need to handle them outside this block.
