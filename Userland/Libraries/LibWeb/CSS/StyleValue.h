...
public:
    static NonnullRefPtr<ImageStyleValue> create(URL const& url, DOM::Document& document) { return adopt_ref(*new ImageStyleValue(url, document)); }
...
private:
    ImageStyleValue(URL const&, DOM::Document&);
...
{
    if (is_identifier())
        return static_cast<IdentifierStyleValue const&>(*this).id();
    if (is_auto())
