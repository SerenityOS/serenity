...
static CSSPixels box_baseline(LayoutState const& state, Box const& box)
{
    ...
    if (!box_state.line_boxes.is_empty())
        return CSSPixels(box_state.border_box_top() + box_state.offset.y()) + box_state.line_boxes.last().baseline();
