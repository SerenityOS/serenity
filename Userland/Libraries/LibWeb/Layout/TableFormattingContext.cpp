...
void TableFormattingContext::run(Box const& box, LayoutMode, AvailableSpace const& available_space)
{
    ...
    CSSPixels total_content_width = 0;
    ...
    box.for_each_child_of_type<TableRowGroupBox>([&](auto& row_group_box) {
        ...
        CSSPixels remaining_for_max = box_state.content_width();
        CSSPixels remaining_for_min = box_state.content_width();
        ...
        if (!table_width_is_auto || (min_fits && !max_fits)) {
            CSSPixels missing_width = max_fits ? remaining_for_max : remaining_for_min;
            if (missing_width > 0) {
                ...
                if (num_auto_columns) {
                    CSSPixels extra = missing_width / (float)num_auto_columns;
                    for (auto& column_width : column_widths) {
                        if (column_width.is_auto)
        ...
        CSSPixels content_width = 0;
        CSSPixels content_height = 0;

        row_group_box.template for_each_child_of_type<TableRowBox>([&](auto& row) {
            auto& row_state = m_state.get_mutable(row);
            row_state.offset = { 0, content_height.value() };
        ...
        if (row_group_box.computed_values().width().is_auto())
            row_group_box_state.set_content_width(content_width.value());
        row_group_box_state.set_content_height(content_height.value());

        row_group_box_state.offset = { 0, total_content_height.value() };
    ...
    if (table_width_is_auto)
        box_state.set_content_width(total_content_width.value());
...
void TableFormattingContext::calculate_column_widths(Box const& row, CSS::Length const& table_width, Vector<ColumnWidth>& column_widths, AvailableSpace const& available_space)
{
    ...
    row.for_each_child_of_type<TableCellBox>([&](auto& cell) {
        ...
        if (colspan > 1) {
            ...
            auto min_width = calculate_min_content_width(cell);
            auto max_width = calculate_max_content_width(cell);
            CSSPixels missing_min = max(min_width, cell_state.border_box_width());
            CSSPixels missing_max = max(max_width, cell_state.border_box_width());
            for (size_t i = 0; i < colspan; ++i) {
                missing_min -= column_widths[column_index + i].min;
                missing_max -= column_widths[column_index + i].max;
            }
            if (missing_min > 0) {
                CSSPixels extra = missing_min / (float)colspan;
                for (size_t i = 0; i < colspan; ++i)
                    column_widths[column_index + i].min += extra;
            }
            if (missing_max > 0) {
                CSSPixels extra = missing_max / (float)colspan;
                for (size_t i = 0; i < colspan; ++i)
...
void TableFormattingContext::layout_row(Box const& row, Vector<ColumnWidth>& column_widths, AvailableSpace const& available_space)
{
    auto& row_state = m_state.get_mutable(row);
    size_t column_index = 0;
    CSSPixels tallest_cell_height = 0;
    CSSPixels content_width = 0;
    ...
    row.for_each_child_of_type<TableCellBox>([&](auto& cell) {
        ...
        cell_state.offset = row_state.offset.translated(cell_state.border_box_left() + content_width.value(), cell_state.border_box_top());
    ...
    row_state.set_content_height(tallest_cell_height.value());

    row.for_each_child_of_type<TableCellBox>([&](auto& cell) {
        auto& cell_state = m_state.get_mutable(cell);
        cell_state.set_content_height((tallest_cell_height - cell_state.border_box_top() - cell_state.border_box_bottom()).value());
    });

    if (use_auto_layout) {
        row_state.set_content_width(content_width.value());
    } else {
