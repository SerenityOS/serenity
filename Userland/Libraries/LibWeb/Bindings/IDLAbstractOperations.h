...
static JS::Completion clean_up_on_return(HTML::EnvironmentSettingsObject& stored_settings, HTML::EnvironmentSettingsObject& relevant_settings, JS::Completion& completion)
{
...
JS::Completion call_user_object_operation(Bindings::CallbackType& callback, String const& operation_name, Optional<JS::Value> this_argument, Args&&... args)
{
    ...
    auto& realm = *object.global_object().associated_realm();
...
JS::Completion invoke_callback(Bindings::CallbackType& callback, Optional<JS::Value> this_argument, Args&&... args)
{
    ...
    auto& realm = *function_object->global_object().associated_realm();
    ...
    auto call_result = realm.vm().call(verify_cast<JS::FunctionObject>(*function_object), this_argument.value(), forward<Args>(args)...);
