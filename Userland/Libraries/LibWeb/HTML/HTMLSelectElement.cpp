...
void HTMLSelectElement::activation_behavior(DOM::Event const&)
{
    ...
    for (auto const& child : children_as_vector()) {
        if (is<HTMLOptGroupElement>(*child)) {
            ...
            for (auto const& child : opt_group_element.children_as_vector()) {
                if (is<HTMLOptionElement>(*child)) {
                    ...
                    option_group_items.append(SelectItemOption { id_counter++, strip_newlines(option_element.text_content()), option_element.value(), option_element.selected(), option_element.disabled(), JS::make_handle(option_element) });
                }
            }
        ...
        if (is<HTMLOptionElement>(*child)) {
            auto& option_element = verify_cast<HTMLOptionElement>(*child);
            m_select_items.append(SelectItemOption { id_counter++, strip_newlines(option_element.text_content()), option_element.value(), option_element.selected(), option_element.disabled(), JS::make_handle(option_element) });
        }
...
void HTMLSelectElement::did_select_item(Optional<u32> id)
{
    set_is_open(false);
