...
{
    static NonnullRefPtr<ProcessManager> s_the = adopt_ref(*new ProcessManager);
    return s_the;
}
...
void ProcessManager::update_all_processes()
{
    if (s_received_sigchld) {
        ...
        ErrorOr<Core::System::WaitPidResult> result = Error::from_errno(ENOENT);
        while (result = Core::System::waitpid(-1, WNOHANG), (!result.is_error() && result.value().pid > 0)) {
