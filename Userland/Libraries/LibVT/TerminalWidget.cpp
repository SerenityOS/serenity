    ...
    , m_automatic_size_policy(automatic_size_policy)
{
    ...
    m_copy_action = GUI::Action::create("&Copy"_string, { Mod_Ctrl | Mod_Shift, Key_C }, Gfx::Bitmap::load_from_file("/res/icons/16x16/edit-copy.png"sv).release_value_but_fixme_should_propagate_errors(), [this](auto&) {
        copy();
    });
    m_copy_action->set_swallow_key_event_when_disabled(true);

    m_paste_action = GUI::Action::create("&Paste"_string, { Mod_Ctrl | Mod_Shift, Key_V }, Gfx::Bitmap::load_from_file("/res/icons/16x16/paste.png"sv).release_value_but_fixme_should_propagate_errors(), [this](auto&) {
        paste();
    });
    m_paste_action->set_swallow_key_event_when_disabled(true);

    m_clear_including_history_action = GUI::Action::create("Clear Including &History"_string, { Mod_Ctrl | Mod_Shift, Key_K }, [this](auto&) {
...
void TerminalWidget::context_menu_event(GUI::ContextMenuEvent& event)
{
    if (!m_hovered_href_id.has_value()) {
        ...
        m_context_menu_for_hyperlink->add_action(GUI::Action::create("Copy &URL"_string, [this](auto&) {
            GUI::Clipboard::the().set_plain_text(m_context_menu_href);
        }));
        m_context_menu_for_hyperlink->add_action(GUI::Action::create("Copy &Name"_string, [&](auto&) {
