        ...
        read_while_data_available([&] {
            ...
            if (m_content_length.has_value()) {
                ...
                if (m_received_size + payload.size() >= length) {
                    MUST(payload.resize(length - m_received_size)); // FIXME: Propagate error
...
void Job::finish_up()
{
    ...
    if (!m_can_stream_response) {
        auto flattened_buffer = MUST(ByteBuffer::create_uninitialized(m_buffered_size)); // FIXME: Handle possible OOM situation.
