...
static ErrorOr<ByteBuffer> handle_content_encoding(ByteBuffer const& buf, DeprecatedString const& content_encoding)
{
    ...
    //        Gzip and Deflate are implemented using Core::Stream, while Brotli uses the newer Core::Stream. The Gzip and
...
Job::Job(HttpRequest&& request, Core::Stream& output_stream)
