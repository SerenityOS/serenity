...
public:
    explicit NameObject(FlyByteString name)
        : m_name(move(name))
    {
    }
    ...
    [[nodiscard]] ALWAYS_INLINE FlyByteString const& name() const { return m_name; }
...
public:
    explicit DictObject(HashMap<FlyByteString, Value> map)
        : m_map(move(map))
    {
    }
    ...
    [[nodiscard]] ALWAYS_INLINE HashMap<FlyByteString, Value> const& map() const { return m_map; }
    ...
    ALWAYS_INLINE Optional<Value> get(FlyByteString const& key) const { return m_map.get(key).copy(); }

    Value get_value(FlyByteString const& key) const
    {
        auto value = get(key);
    ...
    PDFErrorOr<NonnullRefPtr<Object>> get_object(Document*, FlyByteString const& key) const;

#define DEFINE_GETTER(class_name, snake_name)                                                          \
    PDFErrorOr<NonnullRefPtr<class_name>> get_##snake_name(Document*, FlyByteString const& key) const; \
    NonnullRefPtr<class_name> get_##snake_name(FlyByteString const& key) const;
...
private:
    HashMap<FlyByteString, Value> m_map;
};
