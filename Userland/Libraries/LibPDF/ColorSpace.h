...
public:
    ColorSpaceFamily(FlyByteString name, bool may_be_specified_directly)
        : m_name(move(name))
        , m_may_be_specified_directly(may_be_specified_directly)
    {
    }

    FlyByteString name() const { return m_name; }
    bool may_be_specified_directly() const { return m_may_be_specified_directly; }
    static PDFErrorOr<ColorSpaceFamily> get(FlyByteString const&);
...
public:
    ...
    static PDFErrorOr<NonnullRefPtr<ColorSpace>> create(FlyByteString const&, Renderer&);
