...
Value FinalizationRegistryConstructor::construct(FunctionObject& new_target)
{
    ...
    return TRY_OR_DISCARD(ordinary_create_from_constructor<FinalizationRegistry>(global_object, new_target, &GlobalObject::finalization_registry_prototype, realm(), vm.host_make_job_callback(cleanup_callback.as_function())));
}
