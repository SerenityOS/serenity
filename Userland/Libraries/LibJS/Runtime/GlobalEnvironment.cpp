...
ThrowCompletionOr<bool> GlobalEnvironment::has_binding(FlyByteString const& name, Optional<size_t>*) const
{
...
ThrowCompletionOr<void> GlobalEnvironment::create_mutable_binding(VM& vm, FlyByteString const& name, bool can_be_deleted)
{
    // 1. Let DclRec be envRec.[[DeclarativeRecord]].
...
ThrowCompletionOr<void> GlobalEnvironment::create_immutable_binding(VM& vm, FlyByteString const& name, bool strict)
{
    // 1. Let DclRec be envRec.[[DeclarativeRecord]].
    // 2. If ! DclRec.HasBinding(N) is true, throw a TypeError exception.
    if (MUST(m_declarative_record->has_binding(name)))
...
ThrowCompletionOr<void> GlobalEnvironment::initialize_binding(VM& vm, FlyByteString const& name, Value value, InitializeBindingHint hint)
{
    // 1. Let DclRec be envRec.[[DeclarativeRecord]].
...
ThrowCompletionOr<void> GlobalEnvironment::set_mutable_binding(VM& vm, FlyByteString const& name, Value value, bool strict)
{
    // 1. Let DclRec be envRec.[[DeclarativeRecord]].
    // 2. If ! DclRec.HasBinding(N) is true, then
    if (MUST(m_declarative_record->has_binding(name))) {
...
ThrowCompletionOr<Value> GlobalEnvironment::get_binding_value(VM& vm, FlyByteString const& name, bool strict)
{
    // 1. Let DclRec be envRec.[[DeclarativeRecord]].
    // 2. If ! DclRec.HasBinding(N) is true, then
...
ThrowCompletionOr<bool> GlobalEnvironment::delete_binding(VM& vm, FlyByteString const& name)
{
    // 1. Let DclRec be envRec.[[DeclarativeRecord]].
    // 2. If ! DclRec.HasBinding(N) is true, then
    if (MUST(m_declarative_record->has_binding(name))) {
...
bool GlobalEnvironment::has_var_declaration(FlyByteString const& name) const
{
...
bool GlobalEnvironment::has_lexical_declaration(FlyByteString const& name) const
{
    // 1. Let DclRec be envRec.[[DeclarativeRecord]].
...
ThrowCompletionOr<bool> GlobalEnvironment::has_restricted_global_property(FlyByteString const& name) const
{
...
ThrowCompletionOr<bool> GlobalEnvironment::can_declare_global_var(FlyByteString const& name) const
{
    // 1. Let ObjRec be envRec.[[ObjectRecord]].
    // 2. Let globalObject be ObjRec.[[BindingObject]].
    auto& global_object = m_object_record->binding_object();
...
ThrowCompletionOr<bool> GlobalEnvironment::can_declare_global_function(FlyByteString const& name) const
{
    // 1. Let ObjRec be envRec.[[ObjectRecord]].
    // 2. Let globalObject be ObjRec.[[BindingObject]].
    auto& global_object = m_object_record->binding_object();
...
ThrowCompletionOr<void> GlobalEnvironment::create_global_var_binding(FlyByteString const& name, bool can_be_deleted)
{
    auto& vm = this->vm();
...
ThrowCompletionOr<void> GlobalEnvironment::create_global_function_binding(FlyByteString const& name, Value value, bool can_be_deleted)
{
    // 1. Let ObjRec be envRec.[[ObjectRecord]].
    // 2. Let globalObject be ObjRec.[[BindingObject]].
    auto& global_object = m_object_record->binding_object();
