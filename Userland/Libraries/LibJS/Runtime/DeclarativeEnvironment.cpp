...
ThrowCompletionOr<bool> DeclarativeEnvironment::has_binding(FlyByteString const& name, Optional<size_t>* out_index) const
{
...
ThrowCompletionOr<void> DeclarativeEnvironment::create_mutable_binding(VM&, FlyByteString const& name, bool can_be_deleted)
{
...
ThrowCompletionOr<void> DeclarativeEnvironment::create_immutable_binding(VM&, FlyByteString const& name, bool strict)
{
    // 1. Assert: envRec does not already have a binding for N.
    // NOTE: We skip this to avoid O(n) traversal of m_bindings.
...
ThrowCompletionOr<void> DeclarativeEnvironment::initialize_binding(VM& vm, FlyByteString const& name, Value value, Environment::InitializeBindingHint hint)
{
...
ThrowCompletionOr<void> DeclarativeEnvironment::set_mutable_binding(VM& vm, FlyByteString const& name, Value value, bool strict)
{
...
ThrowCompletionOr<Value> DeclarativeEnvironment::get_binding_value(VM& vm, FlyByteString const& name, [[maybe_unused]] bool strict)
{
...
ThrowCompletionOr<bool> DeclarativeEnvironment::delete_binding(VM&, FlyByteString const& name)
{
...
ThrowCompletionOr<void> DeclarativeEnvironment::initialize_or_set_mutable_binding(VM& vm, FlyByteString const& name, Value value)
{
    auto binding_and_index = find_binding_and_index(name);
    VERIFY(binding_and_index.has_value());
...
void DeclarativeEnvironment::initialize_or_set_mutable_binding(Badge<ScopeNode>, VM& vm, FlyByteString const& name, Value value)
{
