...
Value ArrayBuffer::get_value(size_t byte_index, [[maybe_unused]] bool is_typed_array, Order, bool is_little_endian)
{
    ...
    auto raw_value = MUST(buffer_impl().slice(byte_index, element_size)); // FIXME: Handle error
...
static ByteBuffer numeric_to_raw_bytes(GlobalObject& global_object, Value value, bool is_little_endian)
{
    ...
    using UnderlyingBufferDataType = Conditional<IsSame<ClampedU8, T>, u8, T>;
    ByteBuffer raw_bytes = MUST(ByteBuffer::create_uninitialized(sizeof(UnderlyingBufferDataType))); // FIXME: Handle possible OOM situation.
    auto flip_if_needed = [&]() {
        if (is_little_endian)
            return;
...
Value ArrayBuffer::get_modify_set_value(size_t byte_index, Value value, ReadWriteModifyFunction operation, bool is_little_endian)
{
    ...
    auto raw_bytes_read = MUST(buffer_impl().slice(byte_index, sizeof(T))); // FIXME: Handle error
    auto raw_bytes_modified = operation(raw_bytes_read, raw_bytes);
    raw_bytes_modified.span().copy_to(buffer_impl().span().slice(byte_index)); // FIXME: Handle error
