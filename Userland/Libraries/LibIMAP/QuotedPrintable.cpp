...
String decode_quoted_printable(StringView const& input)
{
        ...
        if (potential_character == '=') {
            ...
            if (is_ascii_hex_digit(first_escape_character)) {
                ...
                if (is_ascii_hex_digit(second_escape_character)) {
                    char actual_character = (parse_ascii_hex_digit(first_escape_character) << 4) | parse_ascii_hex_digit(second_escape_character);
    ...
    // Quoted-printable will always be in the char range (0x0-0x7F), so it's safe to put it straight into a string.
    return String { output.data(), output.size() };
}
