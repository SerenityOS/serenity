...
struct RollNote {
    ...
    constexpr RollNote operator+(u32 offset) const
    {
        return RollNote {
            on_sample + offset,
            off_sample + offset,
    ...
    Envelope to_envelope(u32 time, u32 attack_samples, u32 decay_samples, u32 release_samples) const
    {
    ...
    RollNote at_real_time(Transport& transport) const;
};
...
class ReferencedRollNotes : public OrderedHashMap<u8, RollNote>
    , public RefCounted<ReferencedRollNotes> {
};
...
constexpr u8 notes_per_octave = 12;
