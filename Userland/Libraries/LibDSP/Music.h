...
struct Signal : public Variant<Sample, Vector<RollNote>> {
    ...
    {
        return has<Sample>() ? SignalType::Sample : SignalType::Note;
    }
};

// Number of samples synthesized and enqueued per buffer.
constexpr size_t sample_count = 1 << 9;

constexpr size_t buffer_size = sample_count * sizeof(Sample);
...
constexpr size_t note_count = sizeof(note_frequencies) / sizeof(double);
