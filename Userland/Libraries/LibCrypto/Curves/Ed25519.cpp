...
constexpr Ed25519Point BASE_POINT = {
...
void select(u8* r, u8 const* a, u8 const* b, u8 c, u8 n)
{
    u8 mask = c - 1;
    for (u8 i = 0; i < n; i++) {
...
ErrorOr<ByteBuffer> Ed25519::sign(ReadonlyBytes public_key, ReadonlyBytes private_key, ReadonlyBytes message)
{
    ...
    barrett_reduce(prefix, k);
    multiply(k, k + 32, prefix, s, 32);
    barrett_reduce(prefix, k);
    add(s, prefix, r, 32);

    // modular reduction
    auto reduced_s = TRY(ByteBuffer::create_uninitialized(32));
    auto is_negative = subtract(prefix, s, Curve25519::BASE_POINT_L_ORDER, 32);
    select(reduced_s.data(), prefix, s, is_negative, 32);
