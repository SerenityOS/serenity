...
ErrorOr<bool> SECP256r1::check_signature(ReadonlyBytes hash, ReadonlyBytes pubkey, ReadonlyBytes signature)
{
    Crypto::ASN1::Decoder asn1_decoder(signature);
    if (asn1_decoder.enter().has_value())
        return Error::from_string_literal("Unable to parse signature ASN.1");

    auto r_bigint = asn1_decoder.read<Crypto::UnsignedBigInteger>(Crypto::ASN1::Class::Universal, Crypto::ASN1::Kind::Integer).release_value();
    auto s_bigint = asn1_decoder.read<Crypto::UnsignedBigInteger>(Crypto::ASN1::Class::Universal, Crypto::ASN1::Kind::Integer).release_value();
    ...
    JacobianPoint point1 {
        import_big_endian(p1.slice(1, 32)),
        import_big_endian(p1.slice(33, 32)),
        1u,
    };
    ...
    JacobianPoint point2 {
        import_big_endian(p2.slice(1, 32)),
        import_big_endian(p2.slice(33, 32)),
        1u,
    };
