...
ErrorOr<void> ReportDescriptorParser::add_report_fields(FieldType field_type, ItemData item_data)
{
    ...
    if (!m_current_item_state_table.global.logical_minimum.has_value())
        return Error::from_string_view_or_print_error_and_return_errno("Input/Output/Feature item without preceding Logical Minimum Item"sv, EINVAL);

    if (!m_current_item_state_table.global.logical_maximum.has_value())
        return Error::from_string_view_or_print_error_and_return_errno("Input/Output/Feature item without preceding Logical Maximum Item"sv, EINVAL);

    if (!m_current_item_state_table.global.report_count.has_value())
        return Error::from_string_view_or_print_error_and_return_errno("Input/Output/Feature item without preceding Report Count Item"sv, EINVAL);

    if (!m_current_item_state_table.global.report_size.has_value())
        return Error::from_string_view_or_print_error_and_return_errno("Input/Output/Feature item without preceding Report Size Item"sv, EINVAL);
