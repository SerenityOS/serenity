...
void FileSystemModel::update_node_on_selection(ModelIndex const& index, const bool selected)
{
...
const FileSystemModel::Node& FileSystemModel::node(ModelIndex const& index) const
{
    if (!index.is_valid())
        return *m_root;
...
bool FileSystemModel::accepts_drag(ModelIndex const& index, const Vector<String>& mime_types) const
{
    if (!index.is_valid())
        return false;
...
Vector<ModelIndex, 1> FileSystemModel::matches(StringView const& searching, unsigned flags, ModelIndex const& index)
{
    Node& node = const_cast<Node&>(this->node(index));
    node.reify_if_needed();
