...
void MultiView::build_actions()
{
    m_view_as_icons_action = Action::create_checkable(
        "Icon View", { Mod_Ctrl, KeyCode::Key_1 }, Media::ImageDecoder::load_from_file("/res/icons/16x16/icon-view.png"sv).release_value_but_fixme_should_propagate_errors(), [this](auto&) {
            set_view_mode(ViewMode::Icon);
        },
        this);

    m_view_as_table_action = Action::create_checkable(
        "Table View", { Mod_Ctrl, KeyCode::Key_2 }, Media::ImageDecoder::load_from_file("/res/icons/16x16/table-view.png"sv).release_value_but_fixme_should_propagate_errors(), [this](auto&) {
            set_view_mode(ViewMode::Table);
        },
        this);

    m_view_as_columns_action = Action::create_checkable(
        "Columns View", { Mod_Ctrl, KeyCode::Key_3 }, Media::ImageDecoder::load_from_file("/res/icons/16x16/columns-view.png"sv).release_value_but_fixme_should_propagate_errors(), [this](auto&) {
