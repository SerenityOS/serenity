...
void MultiView::build_actions()
{
    m_view_as_icons_action = Action::create_checkable(
        "Icon view", { Mod_Ctrl, KeyCode::Key_1 }, Gfx::Bitmap::try_request_resource("icon-view").release_value_but_fixme_should_propagate_errors(), [this](auto&) {
            set_view_mode(ViewMode::Icon);
        },
        this);

    m_view_as_table_action = Action::create_checkable(
        "Table view", { Mod_Ctrl, KeyCode::Key_2 }, Gfx::Bitmap::try_request_resource("table-view").release_value_but_fixme_should_propagate_errors(), [this](auto&) {
            set_view_mode(ViewMode::Table);
        },
        this);

    m_view_as_columns_action = Action::create_checkable(
        "Columns view", { Mod_Ctrl, KeyCode::Key_3 }, Gfx::Bitmap::try_request_resource("columns-view").release_value_but_fixme_should_propagate_errors(), [this](auto&) {
