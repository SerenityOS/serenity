...
void Desktop::set_background_color(StringView const& background_color)
{
    WindowServerConnection::the().async_set_background_color(background_color);
}

void Desktop::set_wallpaper_mode(StringView const& mode)
{
    WindowServerConnection::the().async_set_wallpaper_mode(mode);
}

bool Desktop::set_wallpaper(StringView const& path, bool save_config)
{
