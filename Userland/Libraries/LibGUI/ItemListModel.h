...
public:
    ...
    static NonnullRefPtr<ItemListModel> create(Container const& data, ColumnNamesT const& column_names, const Optional<size_t>& row_count = {}) requires(IsTwoDimensional)
    {
        return adopt_ref(*new ItemListModel<T, Container, ColumnNameListType>(data, column_names, row_count));
    }
    static NonnullRefPtr<ItemListModel> create(Container const& data, const Optional<size_t>& row_count = {}) requires(!IsTwoDimensional)
    {
