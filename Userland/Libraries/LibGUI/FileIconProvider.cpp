...
Icon FileIconProvider::icon_for_path(DeprecatedString const& path, mode_t mode)
{
    ...
    if (S_ISLNK(mode)) {
        auto raw_symlink_target_or_error = Core::Stream::read_link(path);
        ...
        if (raw_symlink_target.starts_with('/')) {
            ...
            target_path = Core::Stream::real_path_for(DeprecatedString::formatted("{}/{}", LexicalPath::dirname(path), raw_symlink_target));
        }
