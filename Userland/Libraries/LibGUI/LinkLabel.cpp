...
{
    m_open_action = GUI::Action::create("Show in File Manager", {}, Gfx::Bitmap::try_request_resource("app-file-manager").release_value_but_fixme_should_propagate_errors(), [&](const GUI::Action&) {
        if (on_click)
            on_click();
    });
