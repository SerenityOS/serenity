...
NonnullRefPtr<Action> make_about_action(DeprecatedString const& app_name, Icon const& app_icon, Window* parent)
{
    ...
    auto action = Action::create(DeprecatedString::formatted("&About {}", app_name), app_icon.bitmap_for_size(16), [=](auto&) {
        auto version = Core::Version::read_long_version_string().release_value_but_fixme_should_propagate_errors();
        AboutDialog::show(app_name, version, app_icon.bitmap_for_size(32), weak_parent.ptr()).release_value_but_fixme_should_propagate_errors();
    });
