    ...
    i32 window_type, String const& title, Gfx::IntRect const& rect, Optional<i32> const& progress)
{
    if (auto* wm = WindowManager::from_wm_id(wm_id))
        Core::EventLoop::current().post_event(*wm, make<WMWindowStateChangedEvent>(client_id, window_id, title, rect, workspace_row, workspace_column, is_active, is_blocked, static_cast<WindowType>(window_type), is_minimized, is_frameless, progress));
}

void ConnectionToWindowManagerServer::applet_area_size_changed(i32 wm_id, Gfx::IntSize const& size)
{
    if (auto* wm = WindowManager::from_wm_id(wm_id))
        Core::EventLoop::current().post_event(*wm, make<WMAppletAreaSizeChangedEvent>(size));
}

void ConnectionToWindowManagerServer::window_rect_changed(i32 wm_id, i32 client_id, i32 window_id, Gfx::IntRect const& rect)
{
    if (auto* wm = WindowManager::from_wm_id(wm_id))
        Core::EventLoop::current().post_event(*wm, make<WMWindowRectChangedEvent>(client_id, window_id, rect));
}
...
{
    if (auto* wm = WindowManager::from_wm_id(wm_id)) {
        Core::EventLoop::current().post_event(*wm, make<WMWindowIconBitmapChangedEvent>(client_id, window_id, bitmap.bitmap()));
    }
}
...
{
    if (auto* wm = WindowManager::from_wm_id(wm_id))
        Core::EventLoop::current().post_event(*wm, make<WMWindowRemovedEvent>(client_id, window_id));
}

void ConnectionToWindowManagerServer::super_key_pressed(i32 wm_id)
{
    if (auto* wm = WindowManager::from_wm_id(wm_id))
        Core::EventLoop::current().post_event(*wm, make<WMSuperKeyPressedEvent>(wm_id));
}

void ConnectionToWindowManagerServer::super_space_key_pressed(i32 wm_id)
{
    if (auto* wm = WindowManager::from_wm_id(wm_id))
        Core::EventLoop::current().post_event(*wm, make<WMSuperSpaceKeyPressedEvent>(wm_id));
}

void ConnectionToWindowManagerServer::super_d_key_pressed(i32 wm_id)
{
    if (auto* wm = WindowManager::from_wm_id(wm_id))
        Core::EventLoop::current().post_event(*wm, make<WMSuperDKeyPressedEvent>(wm_id));
}

void ConnectionToWindowManagerServer::super_digit_key_pressed(i32 wm_id, u8 digit)
{
    if (auto* wm = WindowManager::from_wm_id(wm_id))
        Core::EventLoop::current().post_event(*wm, make<WMSuperDigitKeyPressedEvent>(wm_id, digit));
}

void ConnectionToWindowManagerServer::workspace_changed(i32 wm_id, u32 row, u32 column)
{
    if (auto* wm = WindowManager::from_wm_id(wm_id))
        Core::EventLoop::current().post_event(*wm, make<WMWorkspaceChangedEvent>(wm_id, row, column));
}
