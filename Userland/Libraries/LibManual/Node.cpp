...
ErrorOr<NonnullRefPtr<PageNode>> Node::try_create_from_query(Vector<StringView, 2> const& query_parameters)
{
    ...
    if (query_parameter_iterator.is_end())
        return Error::from_string_literal("No query given");
    ...
    if (query_parameter_iterator.is_end()) {
        ...
        if (path_from_query.is_absolute()
            ...
            return try_make_ref_counted<PageNode>(section, path_from_query.title());
        }

        // [page] (in any section)
        Optional<NonnullRefPtr<PageNode>> maybe_page;
        for (auto const& section : sections) {
            auto const page = TRY(try_make_ref_counted<PageNode>(section, first_query_parameter));
            if (Core::Stream::File::exists(page->path())) {
                maybe_page = page;
                break;
            }
        }
    ...
    auto const page = TRY(try_make_ref_counted<PageNode>(section, second_query_parameter));
    if (Core::Stream::File::exists(page->path()))
        return page;
