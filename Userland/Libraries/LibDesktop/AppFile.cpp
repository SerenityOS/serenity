...
NonnullRefPtr<AppFile> AppFile::get_for_app(StringView const& app_name)
{
    auto path = String::formatted("{}/{}.af", APP_FILES_DIRECTORY, app_name);
    return open(path);
}

NonnullRefPtr<AppFile> AppFile::open(StringView const& path)
{
    return adopt_ref(*new AppFile(path));
}

void AppFile::for_each(Function<void(NonnullRefPtr<AppFile>)> callback, StringView const& directory)
{
...
AppFile::AppFile(StringView const& path)
