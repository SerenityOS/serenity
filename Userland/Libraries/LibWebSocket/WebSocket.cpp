...
void WebSocket::read_frame()
{
    ...
    auto payload = MUST(ByteBuffer::create_uninitialized(payload_length)); // FIXME: Handle possible OOM situation.
    u64 read_length = 0;
...
void WebSocket::send_frame(WebSocket::OpCode op_code, ReadonlyBytes payload, bool is_final)
{
    ...
    if (has_mask) {
        ...
        // Mask the payload
        auto buffer = MUST(ByteBuffer::create_uninitialized(payload.size())); // FIXME: Propagate error
        auto& masked_payload = buffer;
        for (size_t i = 0; i < payload.size(); ++i) {
