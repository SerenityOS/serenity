...
public:
    ...
    static bool is_word_operation(Operation);
...
class InstructionFence : public InstructionWithoutArguments {
public:
    virtual ~InstructionFence() = default;
    virtual String mnemonic() const override;
    virtual bool instruction_equals(InstructionImpl const&) const override;
    bool operator==(InstructionFence const&) const = default;

    InstructionFence() = default;
};
...
NonnullOwnPtr<ArithmeticImmediateInstruction> parse_op_imm(u32 instruction);
NonnullOwnPtr<ArithmeticImmediateInstruction> parse_op_imm_32(u32 instruction);
NonnullOwnPtr<ArithmeticInstruction> parse_op_32(u32 instruction);
NonnullOwnPtr<ArithmeticInstruction> parse_op(u32 instruction);
NonnullOwnPtr<Branch> parse_branch(u32 instruction);
