...
static Register extract_destination_register(u32 instruction)
{
...
static Register extract_source_register_1(u32 instruction)
{
...
static Register extract_source_register_2(u32 instruction)
{
...
constexpr i32 sign_extend(u32 n, u8 size)
{
    // negative
    if ((n & (1 << (size - 1))) > 0) {
        return static_cast<i32>(n | (0xffffffff << size));
    }
    // positive
...
{
    auto destination_register = extract_destination_register(instruction);
...
RawJType RawJType::parse(u32 instruction)
{
    ...
    i32 immediate = sign_extend(raw_immediate, 20);
...
{
    auto destination_register = extract_destination_register(instruction);
    auto source_register = extract_source_register_1(instruction);
    auto funct3 = extract_funct3(instruction);
    // Figure 2.4
    u32 raw_immediate = (instruction >> 20) & 0xfff;
    i32 immediate = sign_extend(raw_immediate, 12);
...
{
    auto source_register_1 = extract_source_register_1(instruction);
    auto source_register_2 = extract_source_register_2(instruction);
    auto funct3 = extract_funct3(instruction);
    // Figure 2.3
...
RawCAType RawCAType::parse(u16 instruction)
{
    ...
    return RawCAType {
        .funct6 = funct6,
        .rs1_or_rd = destination_register,
        .funct2 = funct2,
        .rs2 = source_register_2,
        .opcode = opcode,
    };
...
RawCIType RawCIType::parse(u16 instruction)
{
    ...
    case CompressedOpcode::JAL_ADDIW: {
        ...
        immediate = sign_extend(raw_immediate, 6);
        break;
    }
    // C.LUI, C.ADDI16SP
...
RawCBType RawCBType::parse(u16 instruction)
{
    ...
    if (funct3 == 0b100) {
        ...
        immediate = sign_extend(raw_immediate, 9);
    }
...
RawCJType RawCJType::parse(u16 instruction)
{
    ...
    i32 immediate = sign_extend(raw_immediate, 11);
