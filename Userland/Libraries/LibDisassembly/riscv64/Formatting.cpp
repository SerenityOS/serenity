...
String JumpAndLink::to_string(DisplayStyle display_style, u32 origin, Optional<SymbolProvider const&> symbol_provider) const
{
    ...
    if (display_style.use_aliases == DisplayStyle::UsePseudoinstructions::Yes && destination_register() == RegisterABINames::zero)
...
{
    if (display_style.use_aliases == DisplayStyle::UsePseudoinstructions::Yes && rounding_mode() == RoundingMode::DYN)
        return {};
...
{
    return MUST(String::formatted("{:10} {}, {}, {}", mnemonic(), format_register(m_rd, display_style), format_register(m_rs1, display_style), format_register(m_rs2, display_style)));
}
...
{
    return MUST(String::formatted("fs{:8} {}, {:#03x}({})", memory_width(), format_register(source_register(), display_style), immediate(), format_register(base(), display_style)));
}
