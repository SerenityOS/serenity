...
static ErrorOr<void> decode_dds(DDSLoadingContext& context)
{
    ...
    if (size_t mipmap_level = 0; mipmap_level < max(context.header.mip_map_count, 1u)) {
        ...
        context.bitmap = TRY(Gfx::Bitmap::create(Gfx::BitmapFormat::BGRA8888, { width, height }));
...
Gfx::IntSize DDSImageDecoderPlugin::size()
{
...
ErrorOr<ImageFrameDescriptor> DDSImageDecoderPlugin::frame(size_t index, Optional<Gfx::IntSize>)
{
    if (index > 0)
