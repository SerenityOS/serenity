...
ErrorOr<void> PNGWriter::add_IDAT_chunk(Gfx::Bitmap const& bitmap)
{
    ...
    for (int y = 0; y < bitmap.height(); ++y) {
        ...
        TRY(average_filter.buffer.try_ensure_capacity(sizeof(Gfx::ARGB32) * bitmap.width()));

        Filter paeth_filter { .type = PNG::FilterType::Paeth };
        TRY(paeth_filter.buffer.try_ensure_capacity(sizeof(Gfx::ARGB32) * bitmap.width()));
