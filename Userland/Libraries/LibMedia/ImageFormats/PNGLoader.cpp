...
static ErrorOr<void> decode_png_bitmap_simple(PNGLoadingContext& context, ByteBuffer& decompression_buffer)
{
    ...
    context.bitmap = TRY(Gfx::Bitmap::create(context.has_alpha() ? Gfx::BitmapFormat::BGRA8888 : Gfx::BitmapFormat::BGRx8888, { context.width, context.height }));
...
static ErrorOr<void> decode_adam7_pass(PNGLoadingContext& context, Streamer& streamer, int pass)
{
    ...
    subimage_context.bitmap = TRY(Gfx::Bitmap::create(context.bitmap->format(), { subimage_context.width, subimage_context.height }));
...
static ErrorOr<RefPtr<Gfx::Bitmap>> decode_png_animation_frame_bitmap(PNGLoadingContext& context, AnimationFrame& animation_frame)
{
    if (context.color_type == PNG::ColorType::IndexedColor && context.palette_data.is_empty())
        return Error::from_string_literal("PNGImageDecoderPlugin: Didn't see a PLTE chunk for a palletized image, or it was empty.");
...
Gfx::IntSize PNGImageDecoderPlugin::size()
{
...
static ErrorOr<RefPtr<Gfx::Bitmap>> render_animation_frame(AnimationFrame const& prev_animation_frame, AnimationFrame& animation_frame, Gfx::Bitmap const& decoded_frame_bitmap)
{
    auto rendered_bitmap = TRY(prev_animation_frame.bitmap->clone());
    Gfx::Painter painter(rendered_bitmap);
...
ErrorOr<ImageFrameDescriptor> PNGImageDecoderPlugin::frame(size_t index, Optional<Gfx::IntSize>)
{
    if (m_context->state == PNGLoadingContext::State::Error)
        return Error::from_string_literal("PNGImageDecoderPlugin: Decoding failed");
