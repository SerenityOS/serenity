...
static ErrorOr<void> decode_iff_ilbm_header(ILBMLoadingContext& context)
{
    ...
    if (header.form != Gfx::FourCC("FORM") || header.format != Gfx::FourCC("ILBM"))
...
static ErrorOr<RefPtr<Gfx::Bitmap>> chunky_to_bitmap(ILBMLoadingContext& context, ByteBuffer const& chunky)
{
    ...
    RefPtr<Gfx::Bitmap> bitmap = TRY(Gfx::Bitmap::create(Gfx::BitmapFormat::BGRA8888, { width, height }));
...
static ErrorOr<void> decode_iff_chunks(ILBMLoadingContext& context)
{
    ...
    while (!chunks.is_empty()) {
        ...
        } else if (chunk.type == Gfx::FourCC("BODY")) {
        ...
        } else if (chunk.type == Gfx::FourCC("CRNG")) {
            dbgln_if(ILBM_DEBUG, "Chunk:CRNG");
        } else if (chunk.type == Gfx::FourCC("CAMG")) {
...
static ErrorOr<void> decode_bmhd_chunk(ILBMLoadingContext& context)
{
    ...
    if (first_chunk.type != Gfx::FourCC("BMHD"))
...
Gfx::IntSize ILBMImageDecoderPlugin::size()
{
    return Gfx::IntSize { m_context->bm_header.width, m_context->bm_header.height };
}
...
ErrorOr<ImageFrameDescriptor> ILBMImageDecoderPlugin::frame(size_t index, Optional<Gfx::IntSize>)
{
    if (index > 0)
