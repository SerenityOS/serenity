...
template<>
struct AK::Formatter<Media::ISOBMFF::BoxType> : Formatter<FormatString> {
    ErrorOr<void> format(FormatBuilder& builder, Media::ISOBMFF::BoxType const& box_type)
    {
        auto string = Media::ISOBMFF::box_type_to_string(box_type);
        if (string.has_value()) {
...
template<>
struct AK::Formatter<Media::ISOBMFF::BrandIdentifier> : Formatter<FormatString> {
    ErrorOr<void> format(FormatBuilder& builder, Media::ISOBMFF::BrandIdentifier const& brand_identifier)
    {
        auto string = Media::ISOBMFF::brand_identifier_to_string(brand_identifier);
        if (string.has_value()) {
            return Formatter<FormatString>::format(builder, "{}"sv, string.release_value());
        }
