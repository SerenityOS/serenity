...
#include <AK/Vector.h>
#include <LibMedia/ImageFormats/BooleanDecoder.h>
#include <LibMedia/ImageFormats/WebPLoaderLossy.h>
#include <LibMedia/ImageFormats/WebPLoaderLossyTables.h>
...
void convert_yuv_to_rgb(Gfx::Bitmap& bitmap, int mb_x, int mb_y, ReadonlyBytes y_data, ReadonlyBytes u_data, ReadonlyBytes v_data)
{
    for (int y = 0; y < 16; ++y) {
        for (int x = 0; x < 16; ++x) {
            u8 Y = y_data[y * 16 + x];
...
ErrorOr<NonnullRefPtr<Gfx::Bitmap>> decode_webp_chunk_VP8_contents(VP8Header const& vp8_header, bool include_alpha_channel)
{
    ...
    auto bitmap_format = include_alpha_channel ? Gfx::BitmapFormat::BGRA8888 : Gfx::BitmapFormat::BGRx8888;
    auto bitmap = TRY(Gfx::Bitmap::create(bitmap_format, { macroblock_width * 16, macroblock_height * 16 }));
    ...
    if (bitmap->physical_size() == Gfx::IntSize { width, height })
        return bitmap;
