...
template<>
struct AK::Traits<Media::TGAHeader> : public DefaultTraits<Media::TGAHeader> {
...
Gfx::IntSize TGAImageDecoderPlugin::size()
{
    return Gfx::IntSize { m_context->header.width, m_context->header.height };
}
...
ErrorOr<bool> TGAImageDecoderPlugin::validate_before_create(ReadonlyBytes data)
{
    ...
    auto header = TRY(stream.read_value<Media::TGAHeader>());
...
static ErrorOr<Gfx::ARGB32> read_pixel_from_stream(Stream& stream, size_t bytes_size)
{
    ...
    return stream.read_value<Gfx::ARGB32>();
}
...
ErrorOr<ImageFrameDescriptor> TGAImageDecoderPlugin::frame(size_t index, Optional<Gfx::IntSize>)
{
    ...
    case 24:
        bitmap = TRY(Gfx::Bitmap::create(Gfx::BitmapFormat::BGRx8888, { m_context->header.width, m_context->header.height }));
        break;

    case 32:
        bitmap = TRY(Gfx::Bitmap::create(Gfx::BitmapFormat::BGRA8888, { m_context->header.width, m_context->header.height }));
        break;
