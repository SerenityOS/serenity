...
template<>
class AK::Traits<Media::ICONDIR> : public DefaultTraits<Media::ICONDIR> {
public:
    static constexpr bool is_trivially_serializable() { return true; }
};

template<>
class AK::Traits<Media::ICONDIRENTRY> : public DefaultTraits<Media::ICONDIRENTRY> {
public:
    static constexpr bool is_trivially_serializable() { return true; }
};
...
Gfx::IntSize ICOImageDecoderPlugin::size()
{
    return { m_context->images[m_context->largest_index].width, m_context->images[m_context->largest_index].height };
}

ErrorOr<ImageFrameDescriptor> ICOImageDecoderPlugin::frame(size_t index, Optional<Gfx::IntSize>)
{
    if (index > 0)
