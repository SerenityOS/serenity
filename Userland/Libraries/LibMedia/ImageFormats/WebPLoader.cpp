...
#include <LibMedia/ImageFormats/WebPLoaderLossless.h>
#include <LibMedia/ImageFormats/WebPLoaderLossy.h>
...
static ErrorOr<void> decode_webp_header(WebPLoadingContext& context)
{
    ...
    if (header.riff != Gfx::FourCC("RIFF") || header.webp != Gfx::FourCC("WEBP"))
    ...
    // "The file size in the header is the total size of the chunks that follow plus 4 bytes for the 'WEBP' Gfx::FourCC.
...
static ErrorOr<void> decode_webp_chunk_ALPH(Chunk const& alph_chunk, Gfx::Bitmap& bitmap)
{
    VERIFY(alph_chunk.type == Gfx::FourCC("ALPH"));
...
{
    VERIFY(vp8x_chunk.type == Gfx::FourCC("VP8X"));
...
{
    VERIFY(anim_chunk.type == Gfx::FourCC("ANIM"));
...
{
    VERIFY(anmf_chunk.type == Gfx::FourCC("ANMF"));
    ...
    VERIFY(context.first_chunk->type == Gfx::FourCC("VP8X"));
...
static ErrorOr<ImageData> decode_webp_set_image_data(Optional<Chunk> alpha, Optional<Chunk> image_data)
{
    ...
    if (alpha.has_value() && image_data->type == Gfx::FourCC("VP8L")) {
...
static ErrorOr<void> decode_webp_extended(WebPLoadingContext& context, ReadonlyBytes chunks)
{
    ...
    while (!chunks.is_empty()) {
        ...
        if (chunk.type == Gfx::FourCC("ICCP"))
            store(context.iccp_chunk, chunk);
        else if (chunk.type == Gfx::FourCC("ALPH"))
            store(alpha, chunk);
        else if (chunk.type == Gfx::FourCC("ANIM"))
            store(context.animation_header_chunk, chunk);
        else if (chunk.type == Gfx::FourCC("ANMF"))
            TRY(context.animation_frame_chunks.try_append(chunk));
        else if (chunk.type == Gfx::FourCC("EXIF"))
            store(context.exif_chunk, chunk);
        else if (chunk.type == Gfx::FourCC("XMP "))
            store(context.xmp_chunk, chunk);
        else if (chunk.type == Gfx::FourCC("VP8 ") || chunk.type == Gfx::FourCC("VP8L"))
...
static ErrorOr<void> read_webp_first_chunk(WebPLoadingContext& context)
{
    ...
    if (first_chunk.type != Gfx::FourCC("VP8 ") && first_chunk.type != Gfx::FourCC("VP8L") && first_chunk.type != Gfx::FourCC("VP8X"))
    ...
    if (first_chunk.type == Gfx::FourCC("VP8 ") || first_chunk.type == Gfx::FourCC("VP8L"))
...
static ErrorOr<void> decode_webp_first_chunk(WebPLoadingContext& context)
{
    ...
    if (context.first_chunk->type == Gfx::FourCC("VP8 ")) {
        auto vp8_header = TRY(decode_webp_chunk_VP8_header(context.first_chunk->data));
        context.size = Gfx::IntSize { vp8_header.width, vp8_header.height };
        context.state = WebPLoadingContext::State::FirstChunkDecoded;
        return {};
    }
    if (context.first_chunk->type == Gfx::FourCC("VP8L")) {
        auto vp8l_header = TRY(decode_webp_chunk_VP8L_header(context.first_chunk->data));
        context.size = Gfx::IntSize { vp8l_header.width, vp8l_header.height };
        context.state = WebPLoadingContext::State::FirstChunkDecoded;
        return {};
    }
    VERIFY(context.first_chunk->type == Gfx::FourCC("VP8X"));
    context.vp8x_header = TRY(decode_webp_chunk_VP8X(context.first_chunk.value()));
    context.size = Gfx::IntSize { context.vp8x_header.width, context.vp8x_header.height };
    context.state = WebPLoadingContext::State::FirstChunkDecoded;
    return {};
}
...
static ErrorOr<void> decode_webp_chunks(WebPLoadingContext& context)
{
    ...
    if (context.first_chunk->type == Gfx::FourCC("VP8X"))
...
static ErrorOr<ImageData> decode_webp_animation_frame_image_data(ANMFChunk const& frame)
{
    ...
    Optional<Chunk> alpha, image_data;
    if (chunk.type == Gfx::FourCC("ALPH")) {
        alpha = chunk;
        chunk = TRY(decode_webp_advance_chunk(chunks));
    }
    if (chunk.type == Gfx::FourCC("VP8 ") || chunk.type == Gfx::FourCC("VP8L"))
        image_data = chunk;

    return decode_webp_set_image_data(move(alpha), move(image_data));
}

static ErrorOr<NonnullRefPtr<Gfx::Bitmap>> decode_webp_image_data(ImageData const& image_data)
{
    if (image_data.image_data_chunk.type == Gfx::FourCC("VP8L")) {
    ...
    VERIFY(image_data.image_data_chunk.type == Gfx::FourCC("VP8 "));
...
static ErrorOr<ImageFrameDescriptor> decode_webp_animation_frame(WebPLoadingContext& context, size_t frame_index)
{
    ...
    if (context.state < WebPLoadingContext::State::BitmapDecoded) {
        start_frame = 0;
        auto format = context.vp8x_header.has_alpha ? Gfx::BitmapFormat::BGRA8888 : Gfx::BitmapFormat::BGRx8888;
        context.bitmap = TRY(Gfx::Bitmap::create(format, { context.vp8x_header.width, context.vp8x_header.height }));
        context.bitmap->fill(clear_color);
    } else if (frame_index < context.current_frame) {
        start_frame = 0;
    }

    Gfx::Painter painter(*context.bitmap);
...
{
    return m_context->first_chunk->type == Gfx::FourCC("VP8X") && m_context->vp8x_header.has_animation;
}
...
ErrorOr<ImageFrameDescriptor> WebPImageDecoderPlugin::frame(size_t index, Optional<Gfx::IntSize>)
{
    ...
    auto decode_frame = [this](size_t index) -> ErrorOr<ImageFrameDescriptor> {
        ...
        if (m_context->state < WebPLoadingContext::State::BitmapDecoded) {
            ...
            if (m_context->first_chunk->type == Gfx::FourCC("VP8X")) {
