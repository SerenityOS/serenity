...
#include <LibMedia/ImageFormats/PortableImageLoaderCommon.h>
...
template<typename TContext>
Gfx::IntSize PortableImageDecoderPlugin<TContext>::size()
{
...
template<typename TContext>
ErrorOr<ImageFrameDescriptor> PortableImageDecoderPlugin<TContext>::frame(size_t index, Optional<Gfx::IntSize>)
{
    if (index > 0)
