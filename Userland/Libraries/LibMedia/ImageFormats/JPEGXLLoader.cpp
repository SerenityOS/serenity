...
#include <LibMedia/ImageFormats/ExifOrientedBitmap.h>
...
class Image {
public:
    static ErrorOr<Image> create(Gfx::IntSize size, ImageMetadata const& metadata)
    {
        Image image {};
    ...
    ErrorOr<NonnullRefPtr<Gfx::Bitmap>> to_bitmap(ImageMetadata& metadata) const
    {
        ...
        auto oriented_bitmap = TRY(ExifOrientedBitmap::create(Gfx::BitmapFormat::BGRA8888, { width, height }, orientation));
...
Gfx::IntSize JPEGXLImageDecoderPlugin::size()
{
...
ErrorOr<ImageFrameDescriptor> JPEGXLImageDecoderPlugin::frame(size_t index, Optional<Gfx::IntSize>)
{
    if (index > 0)
