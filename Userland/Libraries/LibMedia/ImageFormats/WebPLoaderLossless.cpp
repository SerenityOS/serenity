...
static ErrorOr<NonnullRefPtr<Gfx::Bitmap>> decode_webp_chunk_VP8L_image(ImageKind image_kind, Gfx::BitmapFormat format, Gfx::IntSize const& size, LittleEndianInputBitStream& bit_stream)
{
    ...
    Vector<Gfx::ARGB32, 32> color_cache;
    ...
    if (image_kind == ImageKind::SpatiallyCoded) {
        ...
        if (has_meta_prefix) {
            ...
            Gfx::IntSize prefix_size { ceil_div(size.width(), block_size), ceil_div(size.height(), block_size) };

            entropy_image = TRY(decode_webp_chunk_VP8L_image(ImageKind::EntropyCoded, Gfx::BitmapFormat::BGRx8888, prefix_size, bit_stream));
            ...
            for (Gfx::ARGB32& pixel : *entropy_image) {
    ...
    auto bitmap = TRY(Gfx::Bitmap::create(format, size));
    ...
    // https://developers.google.com/speed/webp/docs/webp_lossless_bitstream_specification#623_decoding_entropy-coded_image_data
    Gfx::ARGB32* begin = bitmap->begin();
    Gfx::ARGB32* end = bitmap->end();
    Gfx::ARGB32* pixel = begin;

    auto prefix_group = [prefix_bits, begin, &groups, size, &entropy_image](Gfx::ARGB32* pixel) -> PrefixCodeGroup const& {
        if (!prefix_bits)
            return groups[0];
    ...
    auto emit_pixel = [&pixel, &color_cache, color_cache_size, color_cache_code_bits](Gfx::ARGB32 color) {
        // https://developers.google.com/speed/webp/docs/webp_lossless_bitstream_specification#523_color_cache_coding
...
static Gfx::ARGB32 add_argb32(Gfx::ARGB32 a, Gfx::ARGB32 b)
{
...
public:
    ...
    virtual ErrorOr<NonnullRefPtr<Gfx::Bitmap>> transform(NonnullRefPtr<Gfx::Bitmap>) = 0;
};
...
public:
    static ErrorOr<NonnullOwnPtr<PredictorTransform>> read(LittleEndianInputBitStream&, Gfx::IntSize const& image_size);
    virtual ErrorOr<NonnullRefPtr<Gfx::Bitmap>> transform(NonnullRefPtr<Gfx::Bitmap>) override;

private:
    PredictorTransform(int size_bits, NonnullRefPtr<Gfx::Bitmap> predictor_bitmap)
...
#undef ALPHA
    ...
    static Gfx::ARGB32 average2(Gfx::ARGB32 a, Gfx::ARGB32 b)
    {
        return average2(Color::from_argb(a), Color::from_argb(b)).value();
    }

    static ErrorOr<Gfx::ARGB32> predict(u8 predictor, Gfx::ARGB32 TL, Gfx::ARGB32 T, Gfx::ARGB32 TR, Gfx::ARGB32 L);

    int m_size_bits;
    NonnullRefPtr<Gfx::Bitmap> m_predictor_bitmap;
};

ErrorOr<NonnullOwnPtr<PredictorTransform>> PredictorTransform::read(LittleEndianInputBitStream& bit_stream, Gfx::IntSize const& image_size)
{
    ...
    Gfx::IntSize predictor_image_size { ceil_div(image_size.width(), block_size), ceil_div(image_size.height(), block_size) };

    auto predictor_bitmap = TRY(decode_webp_chunk_VP8L_image(ImageKind::EntropyCoded, Gfx::BitmapFormat::BGRx8888, predictor_image_size, bit_stream));

    return adopt_nonnull_own_or_enomem(new (nothrow) PredictorTransform(size_bits, move(predictor_bitmap)));
}

ErrorOr<NonnullRefPtr<Gfx::Bitmap>> PredictorTransform::transform(NonnullRefPtr<Gfx::Bitmap> bitmap_ref)
{
    Gfx::Bitmap& bitmap = *bitmap_ref;
    ...
    Gfx::ARGB32* bitmap_previous_scanline = bitmap.scanline(0);
    for (int y = 1; y < bitmap.height(); ++y) {
        Gfx::ARGB32* bitmap_scanline = bitmap.scanline(y);

        Gfx::ARGB32 TL = bitmap_previous_scanline[0];
        Gfx::ARGB32 T = bitmap_previous_scanline[1];
        Gfx::ARGB32 TR = 2 < bitmap.width() ? bitmap_previous_scanline[2] : bitmap_previous_scanline[0];

        Gfx::ARGB32 L = bitmap_scanline[0];

        int predictor_y = y >> m_size_bits;
        Gfx::ARGB32* predictor_scanline = m_predictor_bitmap->scanline(predictor_y);
        ...
        for (int x = 1; x < bitmap.width(); ++x) {
            ...
            Gfx::ARGB32 predicted = TRY(predict(predictor, TL, T, TR, L));
...
ErrorOr<Gfx::ARGB32> PredictorTransform::predict(u8 predictor, Gfx::ARGB32 TL, Gfx::ARGB32 T, Gfx::ARGB32 TR, Gfx::ARGB32 L)
{
    switch (predictor) {
    case 0:
...
public:
    static ErrorOr<NonnullOwnPtr<ColorTransform>> read(LittleEndianInputBitStream&, Gfx::IntSize const& image_size);
    virtual ErrorOr<NonnullRefPtr<Gfx::Bitmap>> transform(NonnullRefPtr<Gfx::Bitmap>) override;

private:
    ColorTransform(int size_bits, NonnullRefPtr<Gfx::Bitmap> color_bitmap)
        : m_size_bits(size_bits)
    ...
    static Gfx::ARGB32 inverse_transform(Gfx::ARGB32 pixel, Gfx::ARGB32 transform);

    int m_size_bits;
    NonnullRefPtr<Gfx::Bitmap> m_color_bitmap;
};

ErrorOr<NonnullOwnPtr<ColorTransform>> ColorTransform::read(LittleEndianInputBitStream& bit_stream, Gfx::IntSize const& image_size)
{
    ...
    int block_size = 1 << size_bits;
    Gfx::IntSize color_image_size { ceil_div(image_size.width(), block_size), ceil_div(image_size.height(), block_size) };

    auto color_bitmap = TRY(decode_webp_chunk_VP8L_image(ImageKind::EntropyCoded, Gfx::BitmapFormat::BGRx8888, color_image_size, bit_stream));

    return adopt_nonnull_own_or_enomem(new (nothrow) ColorTransform(size_bits, move(color_bitmap)));
}

ErrorOr<NonnullRefPtr<Gfx::Bitmap>> ColorTransform::transform(NonnullRefPtr<Gfx::Bitmap> bitmap_ref)
{
    Gfx::Bitmap& bitmap = *bitmap_ref;
    ...
    for (int y = 0; y < bitmap.height(); ++y) {
        ...
        Gfx::ARGB32* color_scanline = m_color_bitmap->scanline(color_y);
...
Gfx::ARGB32 ColorTransform::inverse_transform(Gfx::ARGB32 pixel, Gfx::ARGB32 transform)
{
    // https://developers.google.com/speed/webp/docs/webp_lossless_bitstream_specification#51_roles_of_image_data
...
ErrorOr<NonnullRefPtr<Gfx::Bitmap>> SubtractGreenTransform::transform(NonnullRefPtr<Gfx::Bitmap> bitmap)
{
    for (Gfx::ARGB32& pixel : *bitmap) {
...
private:
    ColorIndexingTransform(int pixels_per_pixel, int original_width, NonnullRefPtr<Gfx::Bitmap> palette_bitmap)
    ...
    NonnullRefPtr<Gfx::Bitmap> m_palette_bitmap;
};
...
ErrorOr<NonnullOwnPtr<ColorIndexingTransform>> ColorIndexingTransform::read(LittleEndianInputBitStream& bit_stream, int original_width)
{
    ...
    Gfx::IntSize palette_image_size { color_table_size, 1 };
    auto palette_bitmap = TRY(decode_webp_chunk_VP8L_image(ImageKind::EntropyCoded, Gfx::BitmapFormat::BGRA8888, palette_image_size, bit_stream));
    ...
    for (Gfx::ARGB32* pixel = palette_bitmap->begin() + 1; pixel != palette_bitmap->end(); ++pixel)
...
ErrorOr<NonnullRefPtr<Gfx::Bitmap>> ColorIndexingTransform::transform(NonnullRefPtr<Gfx::Bitmap> bitmap)
{
    ...
    Gfx::IntSize unbundled_size = { m_original_width, bitmap->size().height() };
    auto new_bitmap = TRY(Gfx::Bitmap::create(Gfx::BitmapFormat::BGRA8888, unbundled_size));

    unsigned bits_per_pixel = 8 / pixels_per_pixel();
    unsigned pixel_mask = (1 << bits_per_pixel) - 1;
    for (int y = 0; y < bitmap->height(); ++y) {
        Gfx::ARGB32* bitmap_scanline = bitmap->scanline(y);
        Gfx::ARGB32* new_bitmap_scanline = new_bitmap->scanline(y);
...
// https://developers.google.com/speed/webp/docs/riff_container#simple_file_format_lossless
// https://developers.google.com/speed/webp/docs/webp_lossless_bitstream_specification#7_overall_structure_of_the_format
ErrorOr<NonnullRefPtr<Gfx::Bitmap>> decode_webp_chunk_VP8L_contents(VP8LHeader const& vp8l_header)
{
    ...
    auto stored_size = Gfx::IntSize { vp8l_header.width, vp8l_header.height };
    ...
    auto format = vp8l_header.is_alpha_used ? Gfx::BitmapFormat::BGRA8888 : Gfx::BitmapFormat::BGRx8888;
