...
public:
    ...
    ErrorOr<Gfx::FloatPoint> read_point()
    {
        return Gfx::FloatPoint { TRY(read_unit()), TRY(read_unit()) };
    }
    ...
    ErrorOr<TinyVGDecodedImageData::Style> read_style(StyleType type)
    {
        ...
        auto read_gradient = [&]() -> ErrorOr<NonnullRefPtr<Gfx::SVGGradientPaintStyle>> {
            ...
            auto svg_gradient = TRY([&]() -> ErrorOr<NonnullRefPtr<Gfx::SVGGradientPaintStyle>> {
                if (type == StyleType::LinearGradient)
                    return TRY(Gfx::SVGLinearGradientPaintStyle::create(point_0, point_1));
                auto radius = point_1.distance_from(point_0);
                return TRY(Gfx::SVGRadialGradientPaintStyle::create(point_0, 0, point_0, radius));
            }());
    ...
    ErrorOr<Gfx::FloatRect> read_rectangle()
    {
        return Gfx::FloatRect { TRY(read_unit()), TRY(read_unit()), TRY(read_unit()), TRY(read_unit()) };
    }

    ErrorOr<Gfx::FloatLine> read_line()
    {
        return Gfx::FloatLine { TRY(read_point()), TRY(read_point()) };
    }

    ErrorOr<Gfx::Path> read_path(u32 segment_count)
    {
        Gfx::Path path;
...
ErrorOr<NonnullRefPtr<TinyVGDecodedImageData>> TinyVGDecodedImageData::decode(Stream& stream, TinyVGHeader const& header)
{
    ...
    auto rectangle_to_path = [](Gfx::FloatRect const& rect) -> Gfx::Path {
        Gfx::Path path;
...
void TinyVGDecodedImageData::draw_transformed(Gfx::Painter& painter, Gfx::AffineTransform transform) const
{
    ...
    for (auto const& command : draw_commands()) {
        ...
        if (command.fill.has_value()) {
            ...
            command.fill->visit(
                [&](Color color) { aa_painter.fill_path(fill_path, color, Gfx::Painter::WindingRule::EvenOdd); },
                [&](NonnullRefPtr<Gfx::SVGGradientPaintStyle> style) {
                    const_cast<Gfx::SVGGradientPaintStyle&>(*style).set_gradient_transform(transform);
                    aa_painter.fill_path(fill_path, style, 1.0f, Gfx::Painter::WindingRule::EvenOdd);
                });
        }
...
Gfx::IntSize TinyVGImageDecoderPlugin::size()
{
    return { m_context->header.width, m_context->header.height };
}

ErrorOr<ImageFrameDescriptor> TinyVGImageDecoderPlugin::frame(size_t, Optional<Gfx::IntSize> ideal_size)
{
