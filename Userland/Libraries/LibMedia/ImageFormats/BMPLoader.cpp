...
template<typename T>
struct Formatter<Media::Endpoint<T>> : Formatter<StringView> {
    ErrorOr<void> format(FormatBuilder& builder, Media::Endpoint<T> const& value)
    {
...
static ErrorOr<void> decode_bmp_pixel_data(BMPLoadingContext& context)
{
    ...
    Gfx::BitmapFormat format = [&]() -> Gfx::BitmapFormat {
        ...
        case 32:
            return Gfx::BitmapFormat::BGRA8888;
        default:
            return Gfx::BitmapFormat::Invalid;
        }
    }();

    if (format == Gfx::BitmapFormat::Invalid) {
    ...
    context.bitmap = TRY(Gfx::Bitmap::create(format, { static_cast<int>(width), static_cast<int>(height) }));
...
Gfx::IntSize BMPImageDecoderPlugin::size()
{
...
ErrorOr<ImageFrameDescriptor> BMPImageDecoderPlugin::frame(size_t index, Optional<Gfx::IntSize>)
{
    if (index > 0)
