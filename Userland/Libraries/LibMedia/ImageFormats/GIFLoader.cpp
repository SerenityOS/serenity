...
static void copy_frame_buffer(Gfx::Bitmap& dest, Gfx::Bitmap const& src)
{
    VERIFY(dest.size_in_bytes() == src.size_in_bytes());
    memcpy(dest.scanline(0), src.scanline(0), dest.size_in_bytes());
}

static void clear_rect(Gfx::Bitmap& bitmap, Gfx::IntRect const& rect, Color color)
{
    auto intersection_rect = rect.intersected(bitmap.rect());
    if (intersection_rect.is_empty())
        return;

    Gfx::ARGB32* dst = bitmap.scanline(intersection_rect.top()) + intersection_rect.left();
    size_t const dst_skip = bitmap.pitch() / sizeof(Gfx::ARGB32);
...
static ErrorOr<void> decode_frame(GIFLoadingContext& context, size_t frame_index)
{
    ...
    if (context.state < GIFLoadingContext::State::FrameComplete) {
        start_frame = 0;
        context.frame_buffer = TRY(Gfx::Bitmap::create(Gfx::BitmapFormat::BGRA8888, { context.logical_screen.width, context.logical_screen.height }));
        context.prev_frame_buffer = TRY(Gfx::Bitmap::create(Gfx::BitmapFormat::BGRA8888, { context.logical_screen.width, context.logical_screen.height }));
...
Gfx::IntSize GIFImageDecoderPlugin::size()
{
...
ErrorOr<ImageFrameDescriptor> GIFImageDecoderPlugin::frame(size_t index, Optional<Gfx::IntSize>)
{
