            ...
            segment.value().visit(
                [&](DataSection::Data::Active const& data) {
                    ...
                    if (!maybe_data_address.has_value()) {
                        instantiation_result = InstantiationError { "Failed to allocate a data instance for an active data segment" };
                        return;
                    }
                ...
                [&](DataSection::Data::Passive const& passive) {
                    ...
                    if (!maybe_data_address.has_value()) {
                        instantiation_result = InstantiationError { "Failed to allocate a data instance for a passive data segment" };
                        return;
                    }
                    main_module_instance.datas().append(*maybe_data_address);
                });
