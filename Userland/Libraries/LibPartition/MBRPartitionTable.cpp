...
#else
ErrorOr<NonnullOwnPtr<MBRPartitionTable>> MBRPartitionTable::try_to_initialize(NonnullRefPtr<Core::Stream> device_file)
{
...
#else
OwnPtr<MBRPartitionTable> MBRPartitionTable::try_to_initialize(NonnullRefPtr<Core::Stream> device_file, u32 start_lba)
{
...
#else
MBRPartitionTable::MBRPartitionTable(NonnullRefPtr<Core::Stream> device_file, u32 start_lba)
...
MBRPartitionTable::MBRPartitionTable(Kernel::StorageDevice const& device)
    : PartitionTable(device)
#else
MBRPartitionTable::MBRPartitionTable(NonnullRefPtr<Core::Stream> device_file)
    : PartitionTable(move(device_file))
#endif
    , m_start_lba(0)
    , m_cached_header(ByteBuffer::create_zeroed(m_block_size).release_value_but_fixme_should_propagate_errors()) // FIXME: Do something sensible if this fails because of OOM.
