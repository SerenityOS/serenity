...
#else
    static ErrorOr<NonnullOwnPtr<EBRPartitionTable>> try_to_initialize(NonnullRefPtr<Core::Stream>);
    explicit EBRPartitionTable(NonnullRefPtr<Core::Stream>);
#endif
...
#else
    void search_extended_partition(NonnullRefPtr<Core::Stream>, MBRPartitionTable&, u64, size_t limit);
#endif
