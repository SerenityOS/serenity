...
public:
    AudioWidget(NonnullRefPtr<Core::ConfigFile> config)
        : m_audio_client(Audio::ClientConnection::construct())
        , m_config(move(config))
    {
        m_audio_volume = m_config->read_num_entry("Mix", "Volume", 100);
        m_audio_muted = m_config->read_bool_entry("Mix", "Mute", false);
        m_show_percent = m_config->read_bool_entry("Applet", "ShowPercent", false);
        m_audio_client->set_main_mix_volume(m_audio_volume);
        ...
        m_audio_client->on_muted_state_change = [this](bool muted) {
            ...
            m_config->write_bool_entry("Mix", "Mute", muted);
            if (!m_config->sync())
                warnln("Could not write audio configuration.");
        };
        ...
        m_audio_client->on_main_mix_volume_change = [this](int volume) {
            ...
            m_config->write_num_entry("Mix", "Volume", volume);
            if (!m_config->sync())
                warnln("Could not write audio configuration.");
        };
...
int main(int argc, char** argv)
{
    ...
    window->set_main_widget<AudioWidget>(config);
    window->show();
