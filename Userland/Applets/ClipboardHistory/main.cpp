...
ErrorOr<int> serenity_main(Main::Arguments arguments)
{
    ...
    auto debug_dump_action = GUI::Action::create(TRY("Dump to Debug Console"_string), [&](const GUI::Action&) {
        table_view->selection().for_each_index([&](GUI::ModelIndex& index) {
            dbgln("{}", model->data(index, GUI::ModelRole::Display).as_string());
        });
    });

    auto clear_action = GUI::Action::create(TRY("Clear History"_string), TRY(Gfx::Bitmap::load_from_file("/res/icons/16x16/trash-can.png"sv)), [&](const GUI::Action&) {
        model->clear();
        GUI::Clipboard::the().clear();
    });
