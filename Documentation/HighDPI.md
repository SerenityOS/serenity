...
- macOS: Only integer scale factors at the app level. Can stretch final composited frame buffer at the very end for non-integer display scales.
  - advantages: simple programming model, still fairly flexible; scaling at the end produces a coherent final image
  - disadvantages: needs 4x memory even for 1.5x scale; scaling at very makes the final image less detailed than it could be
...
While a 1x 32x32 bitmap and a 2x 16x16 bitmap both have 32x32 pixels, they don't have to look the same: The 2x 16x16 should look exactly like the corresponding 1x 16x16, just with smoother edges. The 1x 32x32 pixel resource could instead pick a different crop. As a concrete example, the 1x 7x10 ladybug emoji image currently just has the ladybug's shell (for space reasons), and so should the 2x version of that emoji. On the other hand, if we add a higher-res 1x 14x20 ladybug emoji at some point, we might want to show the ladybug's legs on it, instead of just a smoother rendition of just the shell. (The 2x version of that 14x20 emoji would then have legs and shell in less jagged.)
...
4. Use suffixes on a directory instead of subdirectory:
...
- have BitmapCollection that stores high-res and low-res path and load lazily when needed
  ...
  + puts complexity in framework, the app doesn't have to care
...
- eagerly load both and use the right one at paint time
  ...
  + puts (less) complexity in framework, the app doesn't have to care
  + can transparently paint UI at both 1x and 2x into different back buffers (eg for multiple screens that have different scale factors)
...
The plan is to have all applications use highdpi back buffers eventually. It'll take some time to get there though, so here's a plan for getting there incrementally.

0. Add some scaling support to Painter. Make it do 2x nearest neighbor scaling of everything at paint time for now.
1. Add scale factor concept to WindowServer. WindowServer has a scaled frame buffer/back buffer. All other bitmaps (both other bitmaps in WindowServer, as well as everything WindowServer-client-side) are always stored at 1x and scaled up when they're painted to the framebuffer. Things will look fine at 2x, but pixely (but window gradients will be smooth already).
2. Let DisplaySettings toggle it WindowServer scale. Now it's possible to switch to and from HighDPI dynamically, using UI.
3. Come up with a system to have scale-dependent bitmap and font resources. Use that to use a high-res cursor bitmap and high-res menu bar text painting in the window server. Menu text and cursor will look less pixely. (And window frames too, I suppose.)
4. Let apps opt in to high-res window framebuffers, and convert all apps one-by-one
5. Remove high-res window frame buffer opt-in since all apps have it now.
