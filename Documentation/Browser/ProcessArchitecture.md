...
Two important aspects of web browsing are further separated from the **WebContent** process: *network downloads* and *image decoding*, segregated to the **DownloadServer** and **ImageDecoder** processes respectively.
...
This process hosts the main HTML/CSS engine (**LibWeb**.) It also runs JavaScript (**LibJS**.) It gets input events from **Browser** and paints the web content into shared bitmaps. It can only communicate with the outside world via **DownloadServer**.

### Process: DownloadServer

This process can use networking protocols (like HTTP, HTTPS, and Gemini) to download files from the outside world. Each **WebContent** process gets its own **DownloadServer** to do downloading on its behalf.

For DNS lookups, **DownloadServer** asks for help from the system's global **LookupServer** service, which handles all outgoing DNS requests.
...
The same basic concept applies to **DownloadServer** and **ImageDecoder** as well, except that those services are spawned by **WebContent** as needed, not by **Browser**.
