...
menuentry 'SerenityOS - netboot diskless text mode' {
        ...
        echo 'Loading prekernel...'
        multiboot (tftp)/serenity/prekernel root=/dev/ramdisk0 gpu_subsystem_mode=off
        echo 'Loading kernel...'
        module (tftp)/serenity/kernel
        echo 'Loading ramdisk...'
        module (tftp)/serenity/ramdisk
...
you might not see any output, so add the `gpu_subsystem_mode=off` argument as well to boot into VGA text mode.
